...
public class ReCFrameView extends FrameView implements ReCApplicationListener, ItemListener {
	...
	javax.swing.Action toggleConnectionStateAction = getContext().getActionMap(ReCFrameView.class, this).get(
	...
	popMenu
		.show(getApparatusTabbedPane(), 0, getApparatusTabbedPane().getHeight()
			- getApparatusTabbedPane().getExperimentActionBar().getHeight()
	...
	getExperimentStatusActionBar().setActionStateText(
		recApplication.getContext().getResourceMap(StatusActionBar.class).getString(
			"lblActionState.apparatusLockable.text", TimeUtils.msToSeconds(millisToLockSuccess)), GREEN);
	...
	getExperimentStatusActionBar().setActionStateText(
		recApplication.getContext().getResourceMap(StatusActionBar.class).getString(
			"lblActionState.apparatusConfigured.text"), GREEN);
    }

    private void startingExperiment() {
	getExperimentStatusActionBar().setActionStateText(
		recApplication.getContext().getResourceMap(StatusActionBar.class).getString(
			"lblActionState.apparatusStarting.text"), YELLOW);
    }
	...
	getExperimentStatusActionBar().setActionStateText(
		recApplication.getContext().getResourceMap(StatusActionBar.class).getString(
			"lblActionState.apparatusStarted.text"), YELLOW);
    }
	...
	long lockCountDown = TimeUtils.msToSeconds(millisToLockSuccess
	...
	if (lockCountDownGreaterThanZero) {
	    getExperimentStatusActionBar().setActionStateText(
		    recApplication.getContext().getResourceMap(StatusActionBar.class).getString(
			    "lblActionState.apparatusLockable.text", lockCountDown), GREEN);
	}
	...
	progressCicleTask = new com.linkare.rec.impl.newface.component.ProgressCicle(idleIcon, busyIcons,
		busyAnimationRate);
	...
	toolBarCenterSpaceLayout.setHorizontalGroup(toolBarCenterSpaceLayout.createParallelGroup(
		javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 790, Short.MAX_VALUE));
