 ...
 */
public class SerialPortTranslatorProcessorManager {
	...
	private SerialPortTranslatorProcessorManager() {
	}

	public static void registerTranslator(SerialPortTranslator translator) {
		if (!translators.containsKey(translator.getCommandIdentifier()))
			translators.put(translator.getCommandIdentifier(), translator);
	}

	public static void deregisterTranslator(SerialPortTranslator translator) {
		if (translators.containsKey(translator.getCommandIdentifier()))
			translators.remove(translator.getCommandIdentifier());
	}

	public static SerialPortTranslator getTranslator(SerialPortCommand command) {
		return (SerialPortTranslator) translators.get(command.getCommandIdentifier());
	}

	public static void registerProcessor(SerialPortProcessor processor) {
		if (!processors.containsKey(processor.getCommandIdentifier()))
			processors.put(processor.getCommandIdentifier(), processor);
	}

	public static void deregisterProcessor(SerialPortProcessor processor) {
		if (processors.containsKey(processor.getCommandIdentifier()))
			processors.remove(processor.getCommandIdentifier());
	}

	public static SerialPortProcessor getProcessor(SerialPortCommand command) {
		return (SerialPortProcessor) processors.get(command.getCommandIdentifier());
	}
