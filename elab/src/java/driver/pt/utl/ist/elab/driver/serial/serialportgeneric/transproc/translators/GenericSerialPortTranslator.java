...
package pt.utl.ist.elab.driver.serial.serialportgeneric.transproc.translators;

import pt.utl.ist.elab.driver.serial.serialportgeneric.SerialPortHardwareInfo;
import pt.utl.ist.elab.driver.serial.serialportgeneric.rs232.SerialPortRs232Channel;
import pt.utl.ist.elab.driver.serial.serialportgeneric.rs232.SerialPortRs232Config;
 ...
 * @author bruno
 */
public class GenericSerialPortTranslator extends AbstractStampTranslator {
	...
	public GenericSerialPortTranslator() {
	...
	public boolean translate(StampCommand command) {
		...
		commandTranslated = SerialPortCommandList.DAT.toString();

		if (command.getCommandIdentifier().equalsIgnoreCase(SerialPortCommandList.DAT.toString())) {
			int i = 0;
			SerialPortRs232Channel parameter = null;
			for (Object commandDataKey : SerialPortHardwareInfo.getDataKeys()) {
				i++;
				Double commandDataValue = Double.parseDouble((String) command.getCommandData((String) commandDataKey));
				parameter = SerialPortRs232Config.getChannel(i);
			...
			// fazer setcommand da string para substituir automaticamente à
			// anterior
			command.setCommand(commandTranslated);
		} else if (command.getCommandIdentifier().equalsIgnoreCase(SerialPortCommandList.CFG.toString())) {
			int i = 0;
			// StampRs232Parameter
		 ...
		 * System.out.println("Comparing: " + modeStr + " with " +
		 * RadioactividadeStampDriver.ACQUISITION_MODES[i]);
		 * if(GenericStampDriver.ACQUISITION_MODES[i].equalsIgnoreCase(modeStr))
		 * { mode=i; break; } } for(int
		 * i=0;i<GenericStampDriver.ACQUISITION_MATERIALS.length;i++) {
		 * if(GenericStampDriver
		 * .ACQUISITION_MATERIALS[i].equalsIgnoreCase(materialStr)) {
		 * material=i; break; } }
		 */
