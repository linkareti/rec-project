    ...
    public void validateConfig(final HardwareAcquisitionConfig config) throws WrongConfigurationException {
        ...
        if (config.getSelectedHardwareParameters() == null) {
            ...
            for (int i = 0; i < hardwareParameters.length && params_ok; i++) {
                final String param_value = config.getSelectedHardwareParameterValue(hardwareParameters[i].getParameterName());
                if (param_value != null) {
        ...
        if (!(total_samples >= samplingScale.getMinSamples() && total_samples <= samplingScale.getMaxSamples() && ((total_samples - samplingScale.getMinSamples()) % samplingScale.getStep()) == 0)) {
