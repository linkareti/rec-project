...
@NamedQueries({
		@NamedQuery(name = Reservation.FIND_ALL_QUERYNAME, query = Reservation.FIND_ALL_QUERY),
...
public class Reservation extends DefaultDomainObject implements ScheduleEvent {
	...
	public static final String FIND_BY_EXPERIMENT_NAME_IN_INTERVAL_QUERY = "Select r from Reservation r where (r.startDate between :"
			+ QUERY_PARAM_START_DATE
			+ " and :"
			+ QUERY_PARAM_END_DATE
			+ " or r.endDate between :"
			+ QUERY_PARAM_START_DATE
			+ " and :"
			+ QUERY_PARAM_END_DATE
			+ ") and r.experiment.name = :"
			+ QUERY_PARAM_EXPERIMENT_NAME;

	public static final String FIND_FOR_USER_IN_DATE_QUERYNAME = "Reservation.findForUserInDate";
	public static final String FIND_FOR_USER_IN_DATE_QUERY = "Select r from Reservation r WHERE r.user.username = :"
			+ QUERY_PARAM_USERNAME
			+ " and r.startDate between :"
			+ QUERY_PARAM_START_DATE + " and :" + QUERY_PARAM_END_DATE;

	public static final String FIND_FOR_USER_AFTER_DATE_QUERYNAME = "Reservation.findForUserAfterDate";
	public static final String FIND_FOR_USER_AFTER_DATE_QUERY = "Select r from Reservation r WHERE r.user.username = :"
			+ QUERY_PARAM_USERNAME
			+ " and r.startDate >= :"
			+ QUERY_PARAM_START_DATE;

	public static final String FIND_FOR_USER_QUERYNAME = "Reservation.findForUser";
	public static final String FIND_FOR_USER_QUERY = "Select r from Reservation r WHERE r.user.username = :"
			+ QUERY_PARAM_USERNAME;

	public static final String FIND_IN_INTERVAL_AND_LAB_QUERYNAME = "Reservation.findInIntervalAndLab";
	public static final String FIND_IN_INTERVAL_AND_LAB_QUERY = "Select r from Reservation r where ( :"
			+ QUERY_PARAM_START_DATE
			+ " between r.startDate and r.endDate  or :"
			+ QUERY_PARAM_END_DATE
			+ " between r.startDate and r.endDate ) and r.experiment.laboratory.name = :"
			+ QUERY_PARAM_LAB_NAME;

	public static final String FIND_FOR_DOMAIN_IN_INTERVAL_QUERYNAME = "Reservation.findForDomainInDate";
	public static final String FIND_FOR_DOMAIN_IN_INTERVAL_QUERY = "Select r from Reservation r WHERE r.domain = :"
			+ QUERY_PARAM_DOMAIN
			+ " and r.startDate between :"
			+ QUERY_PARAM_START_DATE + " and :" + QUERY_PARAM_END_DATE;

	public static final int MINIMUM_INTERVAL_IN_MINUTES = 15;
	 ...
	 * Constructor
	 * 
	 * Creates a reservation with the minimum period
	 * 
	 * @param title
	 *            new value of title
	 * @param startDate
	 ...
	 * 
	 */
	public Reservation(String title, Date startDate) {
		this.title = title;
		this.startDate = startDate;
		DateTime dtStart = new DateTime(startDate);
		this.endDate = dtStart.plusMinutes(MINIMUM_INTERVAL_IN_MINUTES)
				.toDate();
	}
	...
	public boolean hasConflicts(
			List<Reservation> reservationsForExperimentNameInInterval) {
		for (final Reservation reservation : reservationsForExperimentNameInInterval) {
			if (!EqualityUtils.equals(this.getIdInternal(),
					reservation.getIdInternal())
					&& EqualityUtils.equals(this.getExperiment()
							.getLaboratory(), reservation.getExperiment()
							.getLaboratory())
	...
	public boolean isValidPeriodInterval() {
		int minutesInInterval = getInterval().toPeriod(PeriodType.minutes())
				.getMinutes();
		return MINIMUM_INTERVAL_IN_MINUTES <= minutesInInterval
				&& minutesInInterval <= MAXIMUM_INTERVAL_IN_MINUTES;
	}
	...
	private static boolean isValidDate(final Date date) {
		if (date != null) {
			...
			c.setTime(date);
			return (c.get(Calendar.MINUTE) % MINUTE_INTERVAL) == 0;
		} else
			return false;
	}

	public BooleanResult isValid(
			final List<Reservation> reservationsForExperimentNameInInterval) {
		...
		if (!isValidDates()) {
			return new BooleanResult(Boolean.FALSE,
					"error.invalid.chosen.dates");
		}
		if (!isValidPeriodInterval()) {
			return new BooleanResult(Boolean.FALSE,
					"error.invalid.period.interval");
		}
