...
public class ScheduleController implements Serializable {
	...
	public ScheduleModel getEventModel() {
		return eventModel = eventModel == null ? new AllocationManagerScheduleModel()
				: eventModel;
	}
	...
	public void removeEvent(ActionEvent actionEvent) {
		if (!StringUtils.isBlank(event.getId())) {
			Reservation reservation = (Reservation) eventModel.getEvent(event
					.getId());
			eventModel.deleteEvent(event);
			reservationService.remove(reservation);
			JsfUtil.addGlobalSuccessMessage(ConstantUtils.BUNDLE,
					ConstantUtils.LABEL_INFO_KEY, ConstantUtils.INFO_REMOVE_KEY);
			refreshUserView();
	...
	public final String createOrUpdate() {
		if (StringUtils.isBlank(event.getId())) {
			...
			event.setStyleClass("myevents");
			// It is necessary to first update the model so that the event has a
			// reservation id
			eventModel.addEvent(event);
			reservationService.create(event);
			JsfUtil.addGlobalSuccessMessage(ConstantUtils.BUNDLE,
					ConstantUtils.LABEL_INFO_KEY, ConstantUtils.INFO_CREATE_KEY);
			refreshUserView();
		} else {
			// It is necessary to get the merged event and only after that,
			// update the event model
			event = reservationService.edit(event);
			eventModel.updateEvent(event);
			JsfUtil.addGlobalSuccessMessage(ConstantUtils.BUNDLE,
					ConstantUtils.LABEL_INFO_KEY, ConstantUtils.INFO_UPDATE_KEY);
			refreshUserView();
		}
		return null;
	}
	...
	private void refreshUserView() {
		...
		userView.setReservations(reservationService.findReservationsFor(
				new Date(), null, userView));
		SessionHelper.setUserView(userView);
	}

	private User getUser() {
		return userService.findByUsername(SessionHelper.getUserView()
				.getFullUsername());
	}
	...
	public void onEventSelect(ScheduleEntrySelectEvent selectEvent) {
		...
		if (!event.isInternal()) {
			event.setExternalCourse(MoodleClientHelper.findCourse(
					event.getReservedTo(), SessionHelper.getLoginDomain(),
					SessionHelper.getLoginReturn()));
		}
	}

	public void onDateSelect(DateSelectEvent selectEvent) {
		Date dateSelected=(Date) selectEvent.getDate();
		event = new Reservation("", dateSelected);
	}
...
//	private void moveEvent(final boolean moveBothDates,
//			final Reservation event, final int minuteDelta) {
//		if (moveBothDates) {
...
//		moveEvent(false, event, selectEvent.getMinuteDelta());
	...
	public SelectItem[] getExternalCourses() {
		return externalCourses = externalCourses == null ? JsfUtil
				.getSelectItems(new ExternalCourseServiceBean().findAll(), true)
				: externalCourses;
	}
	...
	public void setExternalCourses(SelectItem[] externalCourses) {
		this.externalCourses = Arrays.copyOf(externalCourses,
	...
	private final class AllocationManagerScheduleModel extends
			LazyScheduleModel {
		...
		public void loadEvents(Date start, Date end) {
			clear();
			final List<ScheduleEvent> events = reservationService
					.findAllReservationsFor(start, end,
							SessionHelper.getUserView());
