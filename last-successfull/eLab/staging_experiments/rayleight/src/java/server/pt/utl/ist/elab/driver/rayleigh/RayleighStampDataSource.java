...
public class RayleighStampDataSource extends AbstractStampDataSource {
	...
	public void processDataCommand(StampCommand cmd) {
		...
		if (cmd.getCommandIdentifier().equalsIgnoreCase(StampRayleighProcessor.COMMAND_IDENTIFIER)) {
			...
			values[0] = PhysicsValueFactory
					.fromInt(pos, getAcquisitionHeader().getChannelsConfig(0).getSelectedScale());
			values[1] = PhysicsValueFactory.fromDouble(value, getAcquisitionHeader().getChannelsConfig(1)
					.getSelectedScale());
			if (first) {
				division = value;
				first = false;
			...
			values[2] = PhysicsValueFactory.fromDouble(lnvalue, getAcquisitionHeader().getChannelsConfig(2)
					.getSelectedScale());
