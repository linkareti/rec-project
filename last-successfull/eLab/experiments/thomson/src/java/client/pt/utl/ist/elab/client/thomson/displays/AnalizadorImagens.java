...
public class AnalizadorImagens {
	...
	protected java.awt.Image[] im_feixe = null; // so o feixe num funco branco
	// ou preto (preto e branco :)
	protected java.awt.Image img640480 = null; // imagem final com o raio
	// tracado....
	...
	protected double[] circunferencia = new double[3]; // circunferencia[0]=centro
	// X circunferencia[1] =
	// centro Y e
	// circunferencia[2] =
	// raio
	protected double[] recta = new double[3]; // recta[0]=m recta[1] = b e
	// recta[2] = R quadrado y=mx+b
	protected double[] distancias_referencia = new double[2]; // Dx e Dy por
	// cada pixel
	protected boolean isUp = false; // feixe � orientado para cima? serve para
	// construir o display final na imagem em
	// 640 x 480
	protected boolean allDone = false; // todos os calculos feitos
	protected int i = 0;
	protected int j = 0;
	protected boolean circ_rect = false; // desenhar para o equilibrio de
	// forcas(recta)(false) ou so para o
	// campo
	// magnetico(circunferencia)(true)

	// Variáveis de sistema
	/*
	 * private static final int DIST = 50; private static final int CROSS = 19;
	 * private static final int D1 = 19; private static final int D2 = 80;
	 * private static final int D3 = 90; private static final int PRETO_R1 =
	 * 120; private static final int PRETO_G1 = 112; private static final int
	 * PRETO_B1 = 13; private static final int PRETO_R2 = 40; private static
	 * final int PRETO_G2 = 44; private static final int PRETO_B2 = 0; private
	 * static final int AZUL_R = 162; private static final int AZUL_G = 178;
	 * private static final int AZUL_B = 229; private static final int AZUL_RAIO
	 * = 55;
	 */
	...
	public double[] getCircunferenciaPontos(java.awt.Image img) {
		try {
			 ...
			 * 
			 * 
			 * for (j = 5; j < (new_height - 6); j++) { //erode para tira pontos
			 * deslinhados for (i = 5; i < (new_width - 5); i++) { // if
			 * (img_BW[i+j*new_width] == 0xffffffff & img_BW[i+1+j*new_width] ==
			 * 0xffffffff & img_BW[i-1+j*new_width] == 0xffffffff &
			 * img_BW[i+(j+1)*new_width] == 0xffffffff &
			 * img_BW[i+(j-1)*new_width] == 0xffffffff){ if
			 * (img_BW[i+j*new_width] == 0xffffffff & img_BW[i+1+j*new_width] ==
			 * 0xffffffff & img_BW[i-1+j*new_width] == 0xffffffff) {
			 * img_BW_temp[i+j*new_width] = 0xffffffff; } else {
			 * img_BW_temp[i+j*new_width] = 0xff000000; } } } for (j = 5; j <
			 * (new_height - 6); j++)for (i = 5; i < (new_width - 5);
			 * i++)img_BW[i+j*new_width] =img_BW_temp[i+j*new_width];
			 * 
			 * 
			 * // numero de pontos total -> p p=0; for (i = 5; i < (new_width -
			 * 5); i++) for (j = 5; j < (new_height - 6); j++) if
			 * (img_BW[i+j*new_width] == 0xffffffff) p++;
			 * 
			 * }
			 */
			int p_temp = 20;
				...
				while (p_temp > P) {
					...
					for (i = 1; i < (new_width - 2); i++) {
						if (img_BW[i + j * new_width] == 0xffffffff & img_BW[i + 1 + j * new_width] == 0xffffffff
								& img_BW[i - 1 + j * new_width] == 0xffffffff) {
			...
			for (int alfa = 0; alfa < med1; alfa++) { // x1 y1
				for (int beta = med2; beta < med3; beta++) { // x2 y2
					for (int gama = med4; gama < sup; gama++) { // x3 y3

						var3 = 2 * ((p_y[alfa] - p_y[beta]) * (p_x[gama] - p_x[alfa]) - (p_y[alfa] - p_y[gama])
								* (p_x[beta] - p_x[alfa]));

						if (var3 != 0) { // para evitar divisoes por 0

							var1 = p_x[beta] * p_x[beta] + p_y[beta] * p_y[beta] - p_x[alfa] * p_x[alfa] - p_y[alfa]
									* p_y[alfa];
							var2 = p_x[gama] * p_x[gama] + p_y[gama] * p_y[gama] - p_x[alfa] * p_x[alfa] - p_y[alfa]
									* p_y[alfa];
							...
							// System.out.println( String.valueOf(Ri ) + " = " +
							// String.valueOf(((p_x[alfa]-Ai)*(p_x[alfa]-Ai)+(p_y[alfa]-Bi)*(p_y[alfa]-Bi))/(Ri*Ri)
							// ) + " = " +
							// String.valueOf((p_x[beta]-Ai)*(p_x[beta]-Ai)+(p_y[beta]-Bi)*(p_y[beta]-Bi)
							// /(Ri*Ri)) + " = " +
							// String.valueOf((p_x[gama]-Ai)*(p_x[gama]-Ai)+(p_y[gama]-Bi)*(p_y[gama]-Bi)/(Ri*Ri)
							// ));

							n = 0; // ver o numero de pontos k se ajustam ao
							// modelo
							for (int pontos = 0; pontos < p; pontos++) {

								if (((p_x[pontos] - Ai) * (p_x[pontos] - Ai) + (p_y[pontos] - Bi) * (p_y[pontos] - Bi) - (Ri * Ri)) < RANSAC_ERR
										* (Ri * Ri)
										& ((p_x[pontos] - Ai) * (p_x[pontos] - Ai) + (p_y[pontos] - Bi)
												* (p_y[pontos] - Bi) - (Ri * Ri)) > -RANSAC_ERR * (Ri * Ri)) {
									n++;
									// System.out.println( String.valueOf(n ) +
									// " = " +
									// String.valueOf(Math.abs((p_x[pontos]-Ai)*(p_x[pontos]-Ai)+(p_y[pontos]-Bi)*(p_y[pontos]-Bi))
									// - (Ri*Ri) ) );
			...
			System.out.println(String.valueOf(p) + " " + String.valueOf(n_max) + "  " + String.valueOf(A) + "  "
					+ String.valueOf(B) + "  " + String.valueOf(R));
	...
	public double[] getEquilibrioData(java.awt.Image img) {
		...
		int p_temp = 20;
		for (i = 0; i < (new_width - 1); i++) { // erode para tira pontos
			// deslinhados

			while (p_temp > 1) {

				for (j = 1; j < (new_height - 2); j++) {
					if (img_BW[i + j * new_width] == 0xffffffff & img_BW[i + (j + 1) * new_width] == 0xffffffff
							& img_BW[i + (j - 1) * new_width] == 0xffffffff) {
						img_BW_temp[i + j * new_width] = 0xffffffff;
					} else {
						img_BW_temp[i + j * new_width] = 0xff000000;
		...
		System.out.println("p=" + p);
		// im_alinhada[2] = toolkit.createImage(new
		// java.awt.image.MemoryImageSource(new_width, new_height, img_BW, 0,
		// new_width));

		/*
		 * 
		 * int[] p_x = new int [p]; int[] p_y = new int [p];
		 */
		 ...
		 * (img_BW[i+j*new_width] == 0xffffffff & img_BW[i+1+j*new_width] ==
		 * 0xffffffff & img_BW[i-1+j*new_width] == 0xffffffff &
		 * img_BW[i+(j+1)*new_width] == 0xffffffff & img_BW[i+(j-1)*new_width]
		 * == 0xffffffff) {
		 * 
		 * img_BW_temp[i+j*new_width] = 0xffffffff; } else {
		 * img_BW_temp[i+j*new_width] = 0xff000000; } } } for (j = 5; j <
		...
		/*
		 * // encontrar o R quadrado for (j = 5; j < (new_height - 6); j++) {
		 * for (i = 5; i < (new_width - 5); i++) {
		 * 
		 * if (img_BW[i+j*new_width] == 0xffffffff) {
		 * 
		 * SSyy += (j-sy/p)*(j-sy/p); SEE += (j-recta[0]*i -
		 * recta[1])*(j-recta[0]*i - recta[1]); } } } recta[2] = 1-(SEE/SSyy);
		 * //r quadrado
		 */
		System.out.println("rect ->" + String.valueOf(recta[0]) + "  " + String.valueOf(recta[1]) + "  "
				+ String.valueOf(recta[2]));
		...
		try {
			...
			for (i = 1; i < (IMAGE_WIDTH - 2); i++) { // onde varia a funcao.
				...
				for (j = 1; j < (IMAGE_HEIGHT - 1); j++) {
					...
					/*
					 * if (getDistancia(r,g,b,re,ge,be) < D1 &
					 * getDistancia(r,g,b,rd,gd,bd) < D1 &
					 * getDistancia(r,g,b,rc,gc,bc) < D1 &
					...
					/*
					 * else if( getDistancia(r,g,b,PRETO_R2, PRETO_G2, PRETO_B2)
					 * < D2 ) { temp_pixels[i + j*IMAGE_WIDTH] = 0xffffffff;
					 * 
					 * if (getDistancia(r,g,b,re,ge,be) < D1 &
					 * getDistancia(r,g,b,rd,gd,bd) < D1 &
					 * getDistancia(r,g,b,rc,gc,bc) < D1 &
					 * getDistancia(r,g,b,rb,gb,bb) < D1 )
					 * 
					 * { temp_pixels[i + j*IMAGE_WIDTH] = 0xff000000; }
					 * 
					 * }
					 ...
					 * ) { temp_pixels[i + j*IMAGE_WIDTH] = 0xffffffff; if
					 * (getDistancia(r,g,b,re,ge,be) < D4 &
					 * getDistancia(r,g,b,rd,gd,bd) < D4 &
					 * getDistancia(r,g,b,rc,gc,bc) < D4 &
					 * getDistancia(r,g,b,rb,gb,bb) < D4 ) { temp_pixels[i +
					 * j*IMAGE_WIDTH] = 0xff000000; }
					 * 
					 * 
					 * }
					 * 
			...
			for (int k = 0; k < 2; k++) { // dialate x 2

				for (i = 2; i < (IMAGE_WIDTH - 3); i++) { // dialate
				...
				for (i = 31; i < (IMAGE_WIDTH - 32); i++) {
					...
					if (image_pixels[i + j * IMAGE_WIDTH] == 0xffffffff) {
						...
						if (p > 45 && n < 5) {
							...
							// System.out.println(String.valueOf(n) + "  " +
							// String.valueOf(m_horiz[n]) + "  " +
							// String.valueOf(b_horiz[n]));
							n++;
			...
			/*
			 * if(b_horiz[4] == 0 & m_horiz[4] == 0 ) { // se nao forem
			 * detectadas as 5 rectas
			 * 
			 * m_horiz[4] = m_horiz[3]; b_horiz[4] = b_horiz[3]; m_horiz[3] =
			 * m_horiz[2]; b_horiz[3] = m_horiz[2]; }
			 */
				...
				for (j = 31; j < (IMAGE_HEIGHT - 31); j++) {
					...
					if (image_pixels[i + j * IMAGE_WIDTH] == 0xffffffff) {
						...
						if (p > 15 && n < 9) {
							...
							// System.out.println(String.valueOf(n) + "  " +
							// String.valueOf(m_vert[n]) + "  " +
							// String.valueOf(b_vert[n]));
							n++;
			...
			for (i = 0; i < (IMAGE_WIDTH - 1); i++) {
				for (j = 0; j < IMAGE_HEIGHT; j++) {
					...
					for (int alfa = 0; alfa < 5; alfa++) { // horizontais y=mx+b
					...
					for (int alfa = 0; alfa < 9; alfa++) { // verticais x=my+b
			...
			for (int alfa = 0; alfa < 5; alfa++) { // horizontais y=mx+b
				...
				for (int beta = 0; beta < 9; beta++) { // pontos de interseccao

					pontos_x[alfa * 9 + beta] = (int) Math.round((m_vert[beta] * b_horiz[alfa] + b_vert[beta])
							/ (1 - m_horiz[alfa] * m_vert[beta]));
					pontos_y[alfa * 9 + beta] = (int) Math.round((m_horiz[alfa] * b_vert[beta] + b_horiz[alfa])
							/ (1 - m_horiz[alfa] * m_vert[beta]));
					// System.out.println("->" + String.valueOf(alfa*9 + beta) +
					// "  " + String.valueOf(pontos_x[alfa*9 + beta]) + "  " +
					// String.valueOf(pontos_y[alfa*9 + beta]));
				}
			}
				...
				for (int beta = 0; beta < 4; beta++) {
					for (i = 0; i < 50; i++) {
						for (j = 0; j < 50; j++) {
							...
							peso2 = (1 / dist2) / dist5;
							peso3 = (1 / dist3) / dist5;
							peso4 = 1 - peso1 - peso2 - peso3; // para somatorio
							// se igual a 1
							// //(1/dist4)/dist5;

							rgb[0] = (int) Math
									.round(peso1
											* ((img_orig[pontos_x[alfa + beta * 9] + Math.round(i * cx / 50)
													+ (pontos_y[alfa + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x00ff0000) >> 16)
											+ peso2
											* ((img_orig[pontos_x[alfa + 1 + beta * 9] + Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x00ff0000) >> 16)
											+ peso3
											* ((img_orig[pontos_x[alfa + (beta + 1) * 9]
													+ Math.round(i * cx / 50)
													+ (pontos_y[alfa + (beta + 1) * 9] + Math.round((j - 50) * cy / 50))
													* IMAGE_WIDTH] & 0x00ff0000) >> 16)
											+ peso4
											* ((img_orig[pontos_x[alfa + 1 + (beta + 1) * 9]
													+ Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + (beta + 1) * 9] + Math.round((j - 50) * cy
															/ 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16));

							rgb[1] = (int) Math
									.round(peso1
											* ((img_orig[pontos_x[alfa + beta * 9] + Math.round(i * cx / 50)
													+ (pontos_y[alfa + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x0000ff00) >> 8)
											+ peso2
											* ((img_orig[pontos_x[alfa + 1 + beta * 9] + Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x0000ff00) >> 8)
											...
											* ((img_orig[pontos_x[alfa + 1 + (beta + 1) * 9]
													+ Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + (beta + 1) * 9] + Math.round((j - 50) * cy
															/ 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8));

							rgb[2] = (int) Math
									.round(peso1
											* (img_orig[pontos_x[alfa + beta * 9] + Math.round(i * cx / 50)
													+ (pontos_y[alfa + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x000000ff)
											+ peso2
											* (img_orig[pontos_x[alfa + 1 + beta * 9] + Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + beta * 9] + Math.round(j * cy / 50))
													* IMAGE_WIDTH] & 0x000000ff)
											...
											* (img_orig[pontos_x[alfa + 1 + (beta + 1) * 9]
													+ Math.round((i - 50) * cx / 50)
													+ (pontos_y[alfa + 1 + (beta + 1) * 9] + Math.round((j - 50) * cy
															/ 50)) * IMAGE_WIDTH] & 0x000000ff));

							new_img[i + 50 * alfa + (25 + j + 50 * beta) * new_width] = 0xff000000 + rgb[0] * 256 * 256
									+ rgb[1] * 256 + rgb[2];
						}
					}
				}
			...
			for (int alfa = 0; alfa < 8; alfa++) { // parte acima e abaixo da
				// grelha
				for (i = 0; i < 50; i++) {
					for (j = 0; j < 25; j++) {
						...
						rgb[0] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[alfa] + Math.round(i * cx / 50)
												+ (pontos_y[alfa] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16)
										+ peso2
										* ((img_orig[pontos_x[alfa + 1] + Math.round((i - 50) * cx / 50)
												+ (pontos_y[alfa + 1] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16));

						rgb[1] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[alfa] + Math.round(i * cx / 50)
												+ (pontos_y[alfa] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8)
										+ peso2
										* ((img_orig[pontos_x[alfa + 1] + Math.round((i - 50) * cx / 50)
												+ (pontos_y[alfa + 1] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8));

						rgb[2] = (int) Math.round(peso1
								* (img_orig[pontos_x[alfa] + Math.round(i * cx / 50)
										+ (pontos_y[alfa] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x000000ff)
								+ peso2
								* (img_orig[pontos_x[alfa + 1] + Math.round((i - 50) * cx / 50)
										+ (pontos_y[alfa + 1] + Math.round(-j * cy / 50)) * IMAGE_WIDTH] & 0x000000ff));

						new_img[i + 50 * alfa + (25 - j) * new_width] = 0xff000000 + rgb[0] * 256 * 256 + rgb[1] * 256
								+ rgb[2];

						rgb[0] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[alfa + 36] + Math.round(i * cx / 50)
												+ (pontos_y[alfa + 36] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16)
										+ peso2
										* ((img_orig[pontos_x[alfa + 37] + Math.round((i - 50) * cx / 50)
												+ (pontos_y[alfa + 37] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16));

						rgb[1] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[alfa + 36] + Math.round(i * cx / 50)
												+ (pontos_y[alfa + 36] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8)
										+ peso2
										* ((img_orig[pontos_x[alfa + 37] + Math.round((i - 50) * cx / 50)
												+ (pontos_y[alfa + 37] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8));

						rgb[2] = (int) Math.round(peso1
								* (img_orig[pontos_x[alfa + 36] + Math.round(i * cx / 50)
										+ (pontos_y[alfa + 36] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x000000ff)
								+ peso2
								* (img_orig[pontos_x[alfa + 37] + Math.round((i - 50) * cx / 50)
										+ (pontos_y[alfa + 37] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x000000ff));

						new_img[i + 50 * alfa + (225 + j) * new_width] = 0xff000000 + rgb[0] * 256 * 256 + rgb[1] * 256
								+ rgb[2];
					}
				}
			}

			for (int alfa = 0; alfa < 4; alfa++) { // parte � direita da grelha
				for (i = 400; i < (new_width); i++) {
					for (j = 0; j < 50; j++) {
						...
						rgb[0] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[8 + alfa * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + alfa * 9] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x00ff0000) >> 16)
										+ peso2
										* ((img_orig[pontos_x[8 + (alfa + 1) * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + (alfa + 1) * 9] + Math.round((j - 50) * cy / 50))
												* IMAGE_WIDTH] & 0x00ff0000) >> 16));

						rgb[1] = (int) Math
								.round(peso1
										* ((img_orig[pontos_x[8 + alfa * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + alfa * 9] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x0000ff00) >> 8)
										+ peso2
										* ((img_orig[pontos_x[8 + (alfa + 1) * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + (alfa + 1) * 9] + Math.round((j - 50) * cy / 50))
												* IMAGE_WIDTH] & 0x0000ff00) >> 8));
						...
						rgb[2] = (int) Math
								.round(peso1
										* (img_orig[pontos_x[8 + alfa * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + alfa * 9] + Math.round(j * cy / 50)) * IMAGE_WIDTH] & 0x000000ff)
										+ peso2
										* (img_orig[pontos_x[8 + (alfa + 1) * 9] + Math.round((i - 400) * cx / 50)
												+ (pontos_y[8 + (alfa + 1) * 9] + Math.round((j - 50) * cy / 50))
												* IMAGE_WIDTH] & 0x000000ff));

						new_img[i + (j + 25 + alfa * 50) * new_width] = 0xff000000 + rgb[0] * 256 * 256 + rgb[1] * 256
								+ rgb[2];

					}
				}
			}
			for (i = 400; i < new_width; i++) { // dois cantos k restam ( canto
				// superior direito e canto
				// inferior direito
				for (j = 0; j < 25; j++) {

					new_img[i + (25 - j) * new_width] = img_orig[pontos_x[8] + Math.round((i - 400) * cx / 50)
							+ (pontos_y[8] + Math.round(-j * cy / 50)) * IMAGE_WIDTH];
					new_img[i + (j + 225) * new_width] = img_orig[pontos_x[44] + Math.round((i - 400) * cx / 50)
			...
			// for (i = 0; i < (new_width - 1); i++) for (j = 0; j < new_height;
			// j++) image_pixels[i+j*new_width] = temp_pixels[i+j*new_width];

			/*
			 * for (int k = 0; k < 3; k++){ //dialate x 2
			 * 
			 * for (i = 4; i < (new_width - 5); i++){ //dialate for (j = 4; j <
			 * (new_height - 4); j++){
			 * 
			 * if (new_img[i+j*new_width] == 0xffffffff){ for (int alfa =-2;
			 * alfa < 3; alfa ++){ new_tmp[i + (j+alfa)*new_width]= 0xffffffff;
			 * new_tmp[i + alfa + j*new_width]= 0xffffffff; } } } }
			 * 
			 * for (i = 0; i < (new_width - 1); i++) for (j = 0; j < new_height;
			 * j++) new_img[i+j*new_width] = new_tmp[i+j*new_width]; }
			 * 
			 * 
			 ...
			 * new_img[i+1+j*new_width] == 0xffffffff & new_img[i-1+j*new_width]
			 * == 0xffffffff & new_img[i+(j+1)*new_width] == 0xffffffff &
			 * new_img[i+(j-1)*new_width] == 0xffffffff){ new_tmp[i+j*new_width]
			 * = 0xffffffff; } else new_tmp[i+j*new_width] = 0xff000000; } }
			 * 
			 * for (i = 0; i < (new_width - 1); i++) for (j = 0; j < new_height;
			 * j++) new_img[i+j*new_width] = new_tmp[i+j*new_width]; }
			 */

			im_alinhada[3] = toolkit.createImage(new java.awt.image.MemoryImageSource(new_width, new_height, new_img,
					0, new_width));
	...
	public double[] getDistanciasReferencia() { // distancias reais por pixel

		distancias_referencia[0] = 1.0 / 50; // a imagem tem uma definicao de
		// 400 x 250 com 50 pixeis a
		// representarem 1 cm
	...
	public boolean getIsUp() { // parametro de entrada: imagem depois do erode
		return (isUp);
	}
