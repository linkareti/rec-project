...
void rec_generic_driver(void ){		
	...
	if(_T1IF){
		...
		//printf("%d\r", adc_to_kPa_pressure_converter(read_adc(ADC_PRESSURE)));
...
//If the computer send a command this code see what command was sended
	...
	the parameters[N][]*/
		if(strncmp(Serial.rbuf,"cfg",3) == 0){
			sscanf(Serial.rbuf,"%*s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\r",&param_1,&param_2,&param_3,&param_4,&param_5, &param_6, &param_7);
			printf("CFG\t%d\t%d\t%d\t%d\t%d\t%d\t%d\r",param_1,param_2,param_3,param_4,param_5,param_6,param_7);
	...
	//cur - current configuration command
		if(strncmp(Serial.rbuf,"cur",3) == 0){
			printf("CUR\t");
			printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\r",param_1,param_2,param_3,param_4,param_5,param_6,param_7);
		}
