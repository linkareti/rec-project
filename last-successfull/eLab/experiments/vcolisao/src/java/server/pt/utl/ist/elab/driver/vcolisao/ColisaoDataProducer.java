...
public class ColisaoDataProducer extends VirtualBaseDataSource {
	...
	private final int NUM_CHANNELS = 14;

	private final float x1, y1, x2, y2, v1, v2, r1, r2, m1, m2;
	private final int a, elasticCollision;
	...
	public ColisaoDataProducer(final VirtualBaseDriver driver, final float v1, final float v2, final float r1,
			final float r2, final float m1, final float m2, final int a, final int elasticCollision, final int tbs,
			final int nSamples) {
		this.driver = driver;
		x1 = 0;
		y1 = 0;
	...
	public Esfera computeInitialPosition1(final Esfera esfera) {
		double dt = ColisaoDataProducer.roundToInt(nSamples / 2);
		dt = dt * tbs;
	...
	public Esfera computeInitialPosition(final Esfera esfera) {
		double dt = ColisaoDataProducer.roundToInt(nSamples / 2);
		dt = dt * tbs;
	...
	public boolean checkColision(final Esfera[] spheres) {
		/*
		 * if( Esfera.magnitude(new double[] {spheres[0].x - spheres[1].x ,
		 * spheres[0].y - spheres[1].y }) <( spheres[0].raio + spheres[1].raio
		 * )){return true;} else{return false;}
		 */
		return (Esfera.magnitude(new double[] { spheres[0].x - spheres[1].x, spheres[0].y - spheres[1].y }) <= (spheres[0].raio + spheres[1].raio));
	}

	public Esfera[] advance(final Esfera[] espheras, final double dt) {

		espheras[0] = new Esfera(espheras[0].x + espheras[0].vX * dt, espheras[0].y + espheras[0].vY * dt,
				espheras[0].vX, espheras[0].vY, espheras[0].raio, espheras[0].m);
		espheras[1] = new Esfera(espheras[1].x + espheras[1].vX * dt, espheras[1].y + espheras[1].vY * dt,
				espheras[1].vX, espheras[1].vY, espheras[1].raio, espheras[1].m);

		return espheras;
	}

	public Esfera[] colidir(final Esfera[] esferas_) {
		switch (elasticCollision) {
		case (0): {
			final double vY1 = esferas_[0].vY;
			final double vY2 = esferas_[1].vY;
			final double vX1 = esferas_[0].vX;
			final double vX2 = esferas_[1].vX;

			esferas_[0] = new Esfera(esferas_[0].x, esferas_[0].y, (esferas_[0].m * vX1 + esferas_[1].m * vX2)
					/ (esferas_[0].m + esferas_[1].m), (esferas_[0].m * vY1 + esferas_[1].m * vY2)
					/ (esferas_[0].m + esferas_[1].m), esferas_[0].raio, esferas_[0].m);

			esferas_[1] = new Esfera(esferas_[1].x, esferas_[1].y, (esferas_[0].m * vX1 + esferas_[1].m * vX2)
					/ (esferas_[0].m + esferas_[1].m), (esferas_[0].m * vY1 + esferas_[1].m * vY2)
					/ (esferas_[0].m + esferas_[1].m), esferas_[1].raio, esferas_[1].m);
			break;
		}
		...
		case (1): {
			...
			final double a1 = Esfera.dot(new double[] { esferas_[0].vX, esferas_[0].vY }, dR);
			final double a2 = Esfera.dot(new double[] { esferas_[1].vX, esferas_[1].vY }, dR);
			final double optP = (2 * (a1 - a2)) / (esferas_[0].m + esferas_[1].m);
	...
	private class ProducerThread extends Thread {
		...
		public void run() {
			try {
				...
				final double xf = (esferas[0].raio + esferas[1].raio) * Math.cos(Math.toRadians(a));
				final double yf = (esferas[0].raio + esferas[1].raio) * (-Math.sin(Math.toRadians(a)));
				esferas[0] = new Esfera(-esferas[0].vX * dt_, -esferas[0].vY * dt_, esferas[0].vX, esferas[0].vY,
						esferas[0].raio, esferas[0].m);
				esferas[1] = new Esfera(xf - esferas[1].vX * dt_, yf - esferas[1].vY * dt_, esferas[1].vX,
						esferas[1].vY, esferas[1].raio, esferas[1].m);
				...
				while (currentSample <= nSamples && !stopped) {
					...
					value[8] = new PhysicsValue(PhysicsValFactory.fromFloat((float) (esferas[1].vY * esferas[1].m)),
							...
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// pxt
					value[9] = new PhysicsValue(
							PhysicsValFactory.fromFloat((float) (esferas[0].vX * esferas[0].m + esferas[1].vX
									* esferas[1].m)), getAcquisitionHeader().getChannelsConfig(1).getSelectedScale()
									.getDefaultErrorValue(), getAcquisitionHeader().getChannelsConfig(1)
									.getSelectedScale().getMultiplier());
					// pyt
					value[10] = new PhysicsValue(
							PhysicsValFactory.fromFloat((float) (esferas[0].vY * esferas[0].m + esferas[1].vY
									* esferas[1].m)), getAcquisitionHeader().getChannelsConfig(1).getSelectedScale()
									.getDefaultErrorValue(), getAcquisitionHeader().getChannelsConfig(1)
									.getSelectedScale().getMultiplier());
					// Ec1
					value[11] = new PhysicsValue(
							PhysicsValFactory.fromFloat((float) (0.5 * esferas[0].m * moduloV1 * moduloV1)),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getDefaultErrorValue(),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// Ec2
					value[12] = new PhysicsValue(
							PhysicsValFactory.fromFloat((float) (0.5 * esferas[1].m * moduloV2 * moduloV2)),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getDefaultErrorValue(),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// Ect
					value[13] = new PhysicsValue(PhysicsValFactory.fromFloat((float) (0.5 * esferas[1].m * moduloV2
							* moduloV2 + 0.5 * esferas[0].m * moduloV1 * moduloV1)), getAcquisitionHeader()
