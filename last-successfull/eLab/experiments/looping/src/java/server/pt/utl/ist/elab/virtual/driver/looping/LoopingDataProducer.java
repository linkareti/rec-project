...
public class LoopingDataProducer extends VirtualBaseDataSource implements ODE {
	...
	private class ProducerThread extends Thread {
		...
		public void setData() {
			...
			value[1] = new PhysicsValue(PhysicsValFactory.fromFloat((float) x_), getAcquisitionHeader()
			...
			value[2] = new PhysicsValue(PhysicsValFactory.fromFloat((float) y_), getAcquisitionHeader()
			...
			value[3] = new PhysicsValue(PhysicsValFactory.fromFloat((float) gimmeEnergy()[0]), getAcquisitionHeader()
			...
			value[4] = new PhysicsValue(PhysicsValFactory.fromFloat((float) gimmeEnergy()[1]), getAcquisitionHeader()
			...
			value[5] = new PhysicsValue(PhysicsValFactory.fromFloat((float) gimmeEnergy()[2]), getAcquisitionHeader()
		...
		public void run() {
			try {
				...
				} while (loop.getState()[0] <= 2 * Math.PI && loop.getState()[0] > 0 && currentSample <= nSamples
						&& !stopped);

				if (turningBack) {
					loop.getState()[1] = -loop.getState()[1];
				}
				passLoop = true;
	...
	public void whereAreYou() {
		...
		case TAG_DROP: {
			y_ = h1 * 0.5 * (Math.cos(x_) + 1);
			break;
		}

		case TAG_WAVE: {
			y_ = h2 * 0.5 * (Math.cos(x_) + 1);
			break;
		}
	...
	/*
	 * public static void main(String args[]) { LoopingDataProducer dp = new
	 * LoopingDataProducer(null, 0f,10f,5f,3f,0.1f, 9.8f, 80,10000 );
	 * dp.startProduction(); }
	 */
