...
public class SessionGraphDisplay extends javax.swing.JPanel implements
	...
	public void newSamples(com.linkare.rec.impl.client.experiment.NewExpDataEvent evt) {
		...
		for (int i = firstSample; i < lastSample + 1; i++) {
			if (sessionX == null) {
				/*
				 * byte[] configurationByteArray; if
				 * (model.getValueAt(lastSample,
				 * model.getChannelIndex("Configuration")) != null) {
				 * configurationByteArray = model.getValueAt(lastSample,
				 * model.getChannelIndex
				 * ("Configuration")).getValue().getByteArrayValue().getData();
				 * int[] configurationValues =
				...
				if (model.getValueAt(i, model.getChannelIndex("XX")) != null) {
					...
					// while
					// (sessionGraphDisplaySingleton.checkNewValueAvailableSess());
					 ...
					 * - xx[0]]++;
					 * 
					 * double lambda = 0.01; //0.001 double termEpsilon = 0.01;
					 * int maxIter = 100; double[] params =
					 * pt.utl.ist.elab.client
					 * .aleatorio.utils.LMFit.solve(sessionX
					 * ,sessionY,mu,sigma,y0,Amp,lambda,termEpsilon,maxIter);
					 * graphPanel.updateGraph(sessionX, sessionY, mu, sigma, y0,
					 * Amp);
					 * 
					 * muText.setText(String.valueOf(params[0]));
					 * sigmaText.setText(String.valueOf(params[1]));
	...
	/*
	 * private int getIndexOfMax(double[] array) { if(array==null) return -1;
	 * 
	 * double value = array[0]; int valueIndex = 0; for(int index = 1; index <
	 * array.length; index++) if(array[index] > value) { valueIndex = index;
	 * value = array[index]; } return valueIndex; }//getIndexOfMax(double[]
	 * array)
	 */
