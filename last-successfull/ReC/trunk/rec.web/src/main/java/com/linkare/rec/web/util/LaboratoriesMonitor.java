...
public final class LaboratoriesMonitor {
	...
	private void connectWithLabs(boolean isToForceInitialization) {
		...
		for (final Entry<String, LabJMXConnetionHandler> lab : labsJMXConnectionHandler
				.entrySet()) {
			...
			final JMXConnectionHandler jmxConnectionHandler = lab.getValue()
	...
	public List<MultiThreadLaboratoryWrapper> getActiveLabs() {
		...
		if (!labsJMXConnectionHandler.isEmpty()) {
			labs = new ArrayList<MultiThreadLaboratoryWrapper>(
					labsJMXConnectionHandler.size());
			for (final Entry<String, LabJMXConnetionHandler> jmxHandler : labsJMXConnectionHandler
					.entrySet()) {
				final MultiThreadLaboratoryWrapper laboratory = labsNotificationListener
						.getLaboratory(jmxHandler.getValue().getLaboratory()
								.getName());
				if (laboratory != null) {
					laboratory.setAvailable(jmxHandler.getValue()
							.getJmxConnectionHandler().isConnected());
					labs.add(laboratory);
				}
			}
		}
	...
	private MbeanProxy<IMultiCastControllerMXBean, Laboratory> getMBeanProxy(
		...
		final IMultiCastControllerMXBean proxy = labJMXConnetionHandler
				.getJmxConnectionHandler()
				.getMbeanProxy(
						MBeanObjectNameFactory
								.getMultiCastControllerObjectName(),
						IMultiCastControllerMXBean.class);
		return new MbeanProxy<IMultiCastControllerMXBean, Laboratory>(
				labJMXConnetionHandler.getLaboratory(), proxy);
	}
			...
			synchronized (this) {
				if (needRefresh(lab)) {
					...
					final LabJMXConnetionHandler previous = labsJMXConnectionHandler
							.put(lab.getName(),
									createLabJMXConnectionHandler(lab));
	...
	private boolean needRefresh(final Laboratory lab) {
		...
		if (labJMXConnetionHandler != null) {
			...
			// validate version we can received an out of order request to
			// change
			if (lab.getVersion() > labJMXConnetionHandler.getLaboratory()
					.getVersion()) {
				final String jmxUrl = labJMXConnetionHandler
						.getJmxConnectionHandler().getJmxURL();
				final String jmxUser = labJMXConnetionHandler
						.getJmxConnectionHandler().getJmxUser();
				final String jmxPass = labJMXConnetionHandler
						.getJmxConnectionHandler().getJmxPass();

				final boolean hasJMXUrlChanged = !StringUtils.equalsIgnoreCase(
						jmxUrl, lab.getJmxURL());
				final boolean hasJMXUserChanged = !StringUtils
						.equalsIgnoreCase(jmxUser, lab.getJmxUser());
				final boolean hasJMXPassChanged = !StringUtils
						.equalsIgnoreCase(jmxPass, lab.getJmxPass());

				result = hasJMXUrlChanged || hasJMXUserChanged
						|| hasJMXPassChanged;
			}
