...
public class ReCApplication extends SingleFrameApplication implements ApparatusListSourceListener,
        ...
        try {
            ...
            for (final Lab lab : recFaceConfig.getLab()) {
                ...
                for (final Apparatus apparatus : lab.getApparatus()) {
                    if (apparatus.getLocation().equals(getApparatusAutoConnectID())) {
    ...
    public void connect() {
        if (currentState.canGoTo(LAB_CONNECT_PERFORMED)) {
            ...
            if (isAutoConnectLab()) {
                //TODO: Chamar um splash screen que informe o utilizador que esta tentando conectar ao apparatus
                while (labClientBean.getApparatusByID(apparatusAutoConnectID) == null) {};
                toggleApparatusState();
                if (getCurrentState().equals(CONNECTED_TO_APPARATUS)) {
                    //TODO:
                }
            }
        }
