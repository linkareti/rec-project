...
public class DataConfigInternationalization {
	...
	public static String toString(ChannelAcquisitionConfig config) {
		...
		if (config.getChannelName() != null)
			strBufOut.append("\t" + CHANNEL_NAME).append(config.getChannelName()).append(linesep);
		...
		if (config.getSelectedScale() != null)
			strBufOut.append("\t" + CHANNEL_SCALE).append(config.getSelectedScale()).append(linesep);
		...
		if (config.getChannelsConfig() != null) {
			...
			for (int i = 0; i < config.getChannelsConfig().length; i++) {
				if (config.getChannelsConfig(i) != null)
					strBufOut.append("\t" + CHANNEL + i + CONFIGURATION).append(linesep).append(config.getChannelsConfig(i))
							.append(linesep);
				else
