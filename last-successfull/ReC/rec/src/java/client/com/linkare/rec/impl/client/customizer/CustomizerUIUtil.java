...
public class CustomizerUIUtil {
	...
	public static ICustomizer loadCustomizer(final String url) {
		try {
			...
			LoggerUtil.logThrowable("Couldn't load Customizer from URL :" + url, e2, LOGGER);
			return null;
		}

		/*
		 * try { DownloadService ds =
		 ...
		 * 
		 * if(o instanceof ICustomizer) return (ICustomizer)o; else return null;
		 * 
		 * }catch(Exception e2) {LoggerUtil.logThrowable(
		 * "Couldn't load Customizer (using DownloadService) from URL :"+
		 * url,e2,Logger.getLogger(UI_CLIENT_LOGGER)); return null; }
		 */

		/*
		 * try { URL fulljarURL=new URL(url);
		 * 
		 ...
		 * certs=con.getJarEntry().getCertificates();
		 * System.out.println("Certificates:" + certs); if(certs!=null &&
		 * certs.length>0) { java.security.cert.X509Certificate
		 * cer=(java.security.cert.X509Certificate)certs[0]; String
		 * message="Do you want to run the configuration utility signed by:";
		 * message+="\n\rSigned by: "+cer.getSubjectDN().getName();
		 * message+="\n\rCertified by: "+cer.getIssuerDN().getName(); try {
		 * cer.checkValidity(); message+="\n\rand with a valid date!";
		 ...
		 * 
		 * }catch(Exception e) {
		 * LoggerUtil.logThrowable("Couldn't load Customizer from URL "+
		 * url,e,Logger.getLogger(UI_CLIENT_LOGGER));
		 * 
		 * try { DownloadService ds =
		 * (DownloadService)ServiceManager.lookup("javax.jnlp.DownloadService");
		 * 
