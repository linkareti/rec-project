...
public class STDMAPDataProducer extends VirtualBaseDataSource implements Runnable {
	...
	// HIST + MAP
	public STDMAPDataProducer(VirtualBaseDriver driver, float k, float theta, float iMapa, int iter, float pcor, int w,
			int h, byte pixSize, boolean staticImg) {
		this.driver = driver;
		this.nSamples = 1;
	...
	// ANIMA + MAP
	public STDMAPDataProducer(VirtualBaseDriver driver, float theta, float thetaDot, float length, float mass,
			float force, int forceDt, int nSamples) {
		this.driver = driver;
	...
	// JUST MAP
	public STDMAPDataProducer(VirtualBaseDriver driver, float k, float theta, int nTheta, float dTheta, float iMapa,
			int nIMapa, float dIMapa, int iter, float pcor, int w, int h, byte pixSize, boolean staticImg) {
		this.driver = driver;
		this.nSamples = 1;
		...
		for (int t = 0; t < nTheta; t++) {
			for (int i = 0; i < nIMapa; i++) {
				...
				for (int j = 0; j < iter; j++) {
					...
					g.drawOval((int) Math.round(theta * w / (2 * Math.PI)), h
							- (int) Math.round(iMapa * h / (2 * Math.PI)), this.pixSize, this.pixSize);
			...
			try {
				...
				if (mass != 0) { // Anima
					...
					if (nTheta * nIMapa == 1) { // histogram
						...
						while (currentSample < nSamples && !stopped) {
							...
							for (int j = 0; j < iter; j++) {
								...
								iMapaData[dataCounter++] = (float) iMapa;
								g.drawOval((int) Math.round(theta * w / (2 * Math.PI)), h
										- (int) Math.round(iMapa * h / (2 * Math.PI)), pixSize, pixSize);
							}
							...
							value[3] = new PhysicsValue(PhysicsValFactory.fromByteArray(ByteUtil
									.floatArrayToByteArray(iMapaData), "data/raw"), null,
					...
					while (currentSample < nSamples && !stopped) {
						...
						PhysicsVal val = PhysicsValFactory.fromByteArray(ByteUtil.floatArrayToByteArray(mData),
								"data/raw");
