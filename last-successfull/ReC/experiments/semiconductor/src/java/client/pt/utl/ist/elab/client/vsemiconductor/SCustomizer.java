...
/**
 * Classe que vai conter o painel de defini��es da simula��o a correr. Esta
 * classe apresenta uma colec��o de JSliders e JFormattedTextFields que
 * possibilitam o input dos v�rios par�metros a considerar para a execu��o da
...
public class SCustomizer extends JPanel implements com.linkare.rec.impl.client.customizer.ICustomizer {
	...
	// Painel para conter alguns conjuntos JSlider/JFormattedTextField, para
	// efeitos de organiza��o
	...
	// Vari�veis v�rias. As dimens�es servir�o para desenhar o Graphical User
	// Interface,
	...
	/**
	 * Construtor do painel, onde se desenham os v�rios elementos e se definem
	 * as ac��es que cada elemento vai desempenhar.
	 */
	public SCustomizer() {

		/**
		 * DefiniwCentro - pWidth)��o dos valores iniciais - a menos que variem
		 * ao longo da experiencia, obviamente, ser�o tamb�m os definitivos.
		 */
		// Cria��o do Slider
		...
		// Change Listener para o Slider, definido de forma a que ele interaja
		// com a Formatted Text Field
		sliderConcDadores.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				JSlider source = (JSlider) e.getSource(); // Vai buscar o Slider
				// que disparou o
				// Event
				double valor = (double) source.getValue(); // E recolhe-se o
				// novo valor (ap�s
				// a mudan�a)
				if (!source.getValueIsAdjusting()) { // Se o utilizador acabou
					...
					txtConcDadores.setText(String.valueOf(valor / 100)); // Se
					// nao,
					// apresenta-se
					// apenas
					// o
		...
		/**
		 * Cria��o da JFormattedTextField que vai fazer uso do formatador. Esta
		 * ir� interagir com o JSlider de forma a permitir a introdu��o de um
		 * valor espec�fico de forma mais simples e directa que o slider
		 * permite, utilizando para controlo de valores o formatador criado
		 * anteriormente.
		 */
		...
		/**
		 * Defini��o do InputMap da Text Field, de forma a que esta reaja ao
		 * pressionar da tecla enter al�m da perda de focus.
		 */
		txtConcDadores.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");

		/**
		 * Defini��o do ActionMap da Text Field. Este mapa ir� conter as ac��es
		 * que a Text Field executa para cada input definido no input map.
		 */
		txtConcDadores.getActionMap().put("check", new AbstractAction() {
			public void actionPerformed(ActionEvent e) { // ActionEvent (para
				// alguma ac��o
				// executada)
				if (!txtConcDadores.isEditValid()) { // Se o texto introduzido
					// for inv�lido,
					Toolkit.getDefaultToolkit().beep(); // avisa-se o utilizador
					// com um beep
					txtConcDadores.selectAll(); // e selecciona-se o texto
					// introduzido.
				} else
					try { // Se o texto for v�lido,
						txtConcDadores.commitEdit(); // aprova-se a edi��o do
						// valor da Text Field.
		...
		/**
		 * Property Change Listener para a Text Field. Este bloco de c�digo vai
		 * conter as instru��es necess�rias para a defini��o dos valores da
		 * simula��o de acordo com o escolhido pelo utilizador.
		 */
		txtConcDadores.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent e) {
				if ("value".equals(e.getPropertyName())) { // Se a propriedade
					// alterada for a do
					// valor (diferente
					// do texto!)
					Number value = (Number) e.getNewValue(); // vai-se buscar o
					// novo valor
					// introduzido.
					if (sliderConcDadores != null && value != null) { // Por uma
						// quest�o
						// de
						// seguran�a
						// verifica-se
						...
						// dois
						// objectos
						sliderConcDadores.setValue((int) (value.doubleValue() * 100));// e
						// altera-se
						// o
						// valor
						// condignamente.
						...
						if (!concDadoresCheck.isSelected())
							sliderConcDadoresFinal.setValue(sliderConcDadores.getValue());
					}
				}
			}
		});
		...
		/**
		 * Cria��o do JPanel que vai conter o Slider e a Text Field, al�m de uma
		 * border identificadora do par�metro que vai ser definido nesses
		 * componentes.
		 */
		panelConcDadores = new JPanel();
		title = BorderFactory.createTitledBorder("Concentra\u00e7\u00e3o de Dadores Inicial (10^x) (cm^-3)");// Cria��o
		// da
		// border
		// e
		// do
		// t�tulo
		title.setTitleJustification(TitledBorder.CENTER); // Posicionamento do
		// t�tulo
		title.setTitlePosition(TitledBorder.TOP);
		panelConcDadores.setBorder(title); // Defini��o do painel; border,
		// dimens�o, layout e componentes
		...
		/**
		 * Dado que o que se segue � apenas uma repeti��o para cada par�metro do
		 * que j� foi feito at� agora, julga-se desnecess�rio a repeti��o dos
		 * coment�rios e explica��es oferecidas dado que ser� sempre uma simples
		 * quest�o de procurar o segmento correspondente do j� oferecido. Assim
		 * sendo comentaremos apenas aquelas passagens que forem novas e que
		 * ainda n�o tenham sido explicadas/comentadas ao longo do c�digo.
		 */
		...
		txtConcAceitadores.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
				...
				if ("value".equals(e.getPropertyName())) {
					...
					if (sliderConcAceitadores != null && value != null) {
						...
						if (!concAceitadoresCheck.isSelected())
							sliderConcAceitadoresFinal.setValue(sliderConcAceitadores.getValue());
					}
				}
			}
		});
		...
		txtTemperatura.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
				...
				if ("value".equals(e.getPropertyName())) {
					...
					if (sliderTemperatura != null && value != null) {
						...
						if (!temperaturaCheck.isSelected())
							sliderTemperaturaFinal.setValue(sliderTemperatura.getValue());
					}
				}
			}
		});
		...
		txtTempoAmostras.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtNumAmostras.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtCampoElectrico.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
				...
				if ("value".equals(e.getPropertyName())) {
					...
					if (sliderCampoElectrico != null && value != null) {
						...
						if (!campoElectricoCheck.isSelected())
							sliderCampoElectricoFinal.setValue(sliderCampoElectrico.getValue());
					}
				}
			}
		});
		...
		/**
		 * Defini��o dos valores finais - apenas ser�o definidos se as
		 * checkboxes correspondentes estiverem activadas. Nesse caso ser�
		 * definido n�o s� o valor inicial como tamb�m o final, e de acordo com
		 * esses limites e o n�mero de amostras ser� ent�o calculado o n�mero e
		 * o tamanho dos intervalos em que variam as respectivas grandezas.
		 */
		// Cria��o do Slider
		...
		sliderConcDadoresFinal.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (!source.getValueIsAdjusting()) { // Se o utilizador acabou
					...
					txtConcDadoresFinal.setText(String.valueOf(valor / 100)); // Se
					// nao,
					// apresenta-se
					// apenas
					// o
		...
		// ganhar algum espa�o).
		txtConcDadoresFinal.setMaximumSize(txtFieldDim); // Tamanho m�ximo da
		// caixa.
		 ...
		 * pressionar da tecla enter al�m da perda de focus.
		 */
		txtConcDadoresFinal.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtConcDadoresFinal.getActionMap().put("check", new AbstractAction() {
			public void actionPerformed(ActionEvent e) { // ActionEvent (para
				// alguma ac��o
				// executada)
				if (!txtConcDadoresFinal.isEditValid()) { // Se o texto
					// introduzido for
					// inv�lido,
					Toolkit.getDefaultToolkit().beep(); // avisa-se o utilizador
					// com um beep
					txtConcDadoresFinal.selectAll(); // e selecciona-se o texto
					// introduzido.
				} else
					try { // Se o texto for v�lido,
						txtConcDadoresFinal.commitEdit(); // aprova-se a edi��o
						// do valor da Text
						// Field.
					} catch (java.text.ParseException exc) {
					}
		...
		txtConcDadoresFinal.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent e) {
				if ("value".equals(e.getPropertyName())) { // Se a propriedade
					...
					// novo valor
					// introduzido.
					if (sliderConcDadoresFinal != null && value != null) { // Por
						// uma
						// quest�o
						// de
						// seguran�a
						...
						// dois
						// objectos
						sliderConcDadoresFinal.setValue((int) (value.doubleValue() * 100.0)); // e
						// altera-se
						// o
						// valor
						// condignamente
		...
		title = BorderFactory.createTitledBorder("Concentra\u00e7\u00e3o de Dadores Final (10^x) (cm^-3)");// Cria��o
		// da
		// border
		// e
		// do
		...
		// t�tulo
		title.setTitlePosition(TitledBorder.TOP);
		panelConcDadoresFinal.setBorder(title); // Defini��o do painel; border,
		// dimens�o, layout e
		// componentes
		...
		txtConcAceitadoresFinal.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtTemperaturaFinal.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtCampoElectricoFinal.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		sliderGradienteImpurezas.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (!source.getValueIsAdjusting()) { // Se o utilizador acabou
					...
					txtGradienteImpurezas.setText(String.valueOf(valor / 100));// Se
					// nao,
					// apresenta-se
					// apenas
					// o
		...
		// ganhar algum espa�o).
		txtGradienteImpurezas.setMaximumSize(txtFieldDim); // Tamanho m�ximo da
		// caixa.
		 ...
		 * pressionar da tecla enter al�m da perda de focus.
		 */
		txtGradienteImpurezas.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "check");
		...
		txtGradienteImpurezas.getActionMap().put("check", new AbstractAction() {
			public void actionPerformed(ActionEvent e) { // ActionEvent (para
				// alguma ac��o
				// executada)
				if (!txtGradienteImpurezas.isEditValid()) { // Se o texto
					// introduzido for
					// inv�lido,
					Toolkit.getDefaultToolkit().beep(); // avisa-se o utilizador
					// com um beep
					txtGradienteImpurezas.selectAll(); // e selecciona-se o
					// texto introduzido.
				} else
					try { // Se o texto for v�lido,
						txtGradienteImpurezas.commitEdit(); // aprova-se a
						// edi��o do valor
						// da Text Field.
					} catch (java.text.ParseException exc) {
					}
		...
		txtGradienteImpurezas.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent e) {
				if ("value".equals(e.getPropertyName())) { // Se a propriedade
					...
					// novo valor
					// introduzido.
					if (sliderGradienteImpurezas != null && value != null) { // Por
						// uma
						// quest�o
						// de
						// seguran�a
						...
						// dois
						// objectos
						sliderGradienteImpurezas.setValue((int) (value.doubleValue() * 100));// e
						// altera-se
						// o
						// valor
						// condignamente.
		...
		title = BorderFactory.createTitledBorder("Gradiente de Impurezas (10^x) (cm^-4)");// Cria��o
		// da
		// border
		// e
		// do
		...
		// t�tulo
		title.setTitlePosition(TitledBorder.TOP);
		panelGradienteImpurezas.setBorder(title); // Defini��o do painel;
		// border, dimens�o, layout
		// e componentes
		...
		// M�todos Action Performed para os dois bot�es; chamam ambos o mesmo
		// m�todo por uma quest�o de simplicidade.
		...
		// Painel para conter os pain�is dos bot�es das l�nguas, com o t�tulo,
		// dimens�o e layout apropriados
		...
		// Painel para conter os pain�is individuais do material, com border,
		...
		correrButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				acqConfig.setSelectedFrequency(new Frequency((double) sliderTempoAmostras.getValue(), hardwareInfo
				...
				acqConfig.getSelectedHardwareParameter("N Dadores INI").setParameterValue(
						"" + Double.parseDouble(txtConcDadores.getText()));
				acqConfig.getSelectedHardwareParameter("N Dadores END").setParameterValue(
						"" + Double.parseDouble(txtConcDadoresFinal.getText()));
				acqConfig.getSelectedHardwareParameter("N Aceitadores INI").setParameterValue(
						"" + Double.parseDouble(txtConcAceitadores.getText()));
				acqConfig.getSelectedHardwareParameter("N Aceitadores END").setParameterValue(
						"" + Double.parseDouble(txtConcAceitadoresFinal.getText()));
				acqConfig.getSelectedHardwareParameter("Temp INI").setParameterValue(
						"" + Double.parseDouble(txtTemperatura.getText()));
				acqConfig.getSelectedHardwareParameter("Temp END").setParameterValue(
						"" + Double.parseDouble(txtTemperaturaFinal.getText()));
				acqConfig.getSelectedHardwareParameter("V ext INI").setParameterValue(
						"" + Double.parseDouble(txtCampoElectrico.getText()));
				acqConfig.getSelectedHardwareParameter("V ext END").setParameterValue(
						"" + Double.parseDouble(txtCampoElectricoFinal.getText()));
				acqConfig.getSelectedHardwareParameter("Imp Gradient").setParameterValue(
						"" + Double.parseDouble(txtGradienteImpurezas.getText()));
				/*
				 * if(portuguesButton.isSelected()) {
				 * motor.setLingua("portugues"); } else
				 * if(englishButton.isSelected()) { motor.setLingua("english");
				 * }
				 */
		...
		// Defini��o da border a utilizar no painel (com o t�tulo
		// correspondente)
		esquerda.setLayout(new GridLayout(0, 1)); // Defini��o do layout e
		// adi��o dos componentes ao
		// painel
		...
		// Defini��o da border a utilizar no painel (com o t�tulo
		// correspondente)
		direita.setLayout(new GridLayout(0, 1)); // Defini��o do layout e adi��o
		// dos componentes ao painel
	...
	/**
	 * M�todo de actualiza��o dos valores das taxas de varia��o por itera��o dos
	 * v�rios par�metros. Actualiza todos os valores por uma quest�o de
	 * simplicidade de c�digo.
	 */
	private void actualizarLabels() {
		double numAmostras = Double.parseDouble(txtNumAmostras.getText());

		double deltaDadores = (Math.pow(10, Double.parseDouble(txtConcDadoresFinal.getText())) - Math.pow(10, Double
				.parseDouble(txtConcDadores.getText())));
		if (concDadoresCheck.isSelected())
			deltaDadores /= numAmostras * (Double.parseDouble(txtTempoAmostras.getText()) / 1000);
		else
			deltaDadores = 0;
		...
		double deltaAceitadores = (Math.pow(10, Double.parseDouble(txtConcAceitadoresFinal.getText())) - Math.pow(10,
				Double.parseDouble(txtConcAceitadores.getText())));
		if (concAceitadoresCheck.isSelected())
			deltaAceitadores /= numAmostras * (Double.parseDouble(txtTempoAmostras.getText()) / 1000);
		else
			deltaAceitadores = 0;

		double deltaTemperatura = (Double.parseDouble(txtTemperaturaFinal.getText()) - Double
				.parseDouble(txtTemperatura.getText()));
		if (temperaturaCheck.isSelected())
			deltaTemperatura /= numAmostras * (Double.parseDouble(txtTempoAmostras.getText()) / 1000);
		else
			deltaTemperatura = 0;

		double deltaPotencial = (Double.parseDouble(txtCampoElectricoFinal.getText()) - Double
				.parseDouble(txtCampoElectrico.getText()));
		if (campoElectricoCheck.isSelected())
			deltaPotencial /= numAmostras * (Double.parseDouble(txtTempoAmostras.getText()) / 1000);
		else
			deltaPotencial = 0;
	...
	/**
	 * M�todo para mudar al�ngua em que � apresentado o painel de defini��es.
	 * Este m�todo limita-se a alterar as borders nos v�rios pain�is, dado que
	 * seria um desperd�cio de recursos criar uma nova colec��o de objectos s�
	 * para alterar a l�ngua do painel, j� que o funcionamento do programa se
	 * mant�m.
	 */
	private void mudarLingua() {
		if (portuguesButton.isSelected()) { // Se a l�ngua escolhida for o
			// Portugu�s, muda-se tudo para
			// Portugu�s
		...
		} else { // Se a l�ngua for outra - Ingl�s, exclusivamente - muda-se
			// apropriadamente
	 ...
	 * Notifies all registered listeners about the event.
	 * 
	 * @param param1 ParameterDPendulum.zip #1 of the
	 *            <CODE>EventObject<CODE> constructor.
	 */
	...
	public void setHardwareAcquisitionConfig(HardwareAcquisitionConfig acqConfig) {
		 ...
		 * jSliderTBS.setValue(freq); jTextFieldTBS.setText("" + freq);
		 * 
		 * int a1 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P1 ini angle")); jSliderAng1.setValue(a1); jTextFieldAng1.setText(""
		 * + a1);
		 * 
		 * int a2 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P2 ini angle")); jSliderAng2.setValue(a2); jTextFieldAng2.setText(""
		 * + a2);
		 * 
		 * int l1 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P1 length")); jSliderL1.setValue(l1); jTextFieldL1.setText("" + l1);
		 * 
		 * int l2 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P2 length")); jSliderL2.setValue(l2); jTextFieldL2.setText("" + l2);
		 * 
		 * int m1 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P1 Mass")); jSliderM1.setValue(m1); jTextFieldM1.setText("" + m1);
		 * 
		 * int m2 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P2 Mass")); jSliderM2.setValue(m2); jTextFieldM2.setText("" + m2);
		 * 
		 * int w1 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P1 vel ang ini")); jSliderW1.setValue(w1); jTextFieldW1.setText("" +
		 * w1);
		 * 
		 * int w2 =
		 * Integer.parseInt(acqConfig.getSelectedHardwareParameterValue(
		 * "P2 vel ang ini")); jSliderW2.setValue(w2); jTextFieldW2.setText("" +
		 * w2); }
		 */
	}
	...
	/**
	 * M�todo main. Este m�todo vai criar uma frame provis�ria para apresentar o
	 * painel e vai-lhe adicionar o dito painel, apenas para uma quest�o de
	 * testes.
	 */
	public static void main(String args[]) {
		 ...
		 * UIManager.getSystemLookAndFeelClassName() ); } catch (Exception exc)
		 * { }
		 * 
