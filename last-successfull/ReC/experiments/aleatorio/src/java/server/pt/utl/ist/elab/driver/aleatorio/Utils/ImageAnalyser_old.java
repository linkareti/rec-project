...
public class ImageAnalyser_old extends javax.swing.JPanel implements java.lang.Cloneable {
	...
	public void setParams(int BWThreshold, int radius, int houghThreshold1, int houghThreshold2, int houghThreshold3,
		 ...
		 * imagemDialog.setTitle("Imagem Capturada"); ImagePanel
		 * imagemCapturadaPanel = new ImagePanel(originalImage);
	...
	public java.awt.Image conversionBW() {
		...
		/*
		 * javax.swing.JFrame imagemDialog = new javax.swing.JFrame();
		 * imagemDialog.setTitle("BW Image"); ImagePanel imagemCapturadaPanel =
		 * new ImagePanel(BWImage);
		 * imagemDialog.getContentPane().add(imagemCapturadaPanel); int[] dimIm
		 * = imagemCapturadaPanel.imageSize(); imagemDialog.setSize(dimIm[0]+8,
		 * dimIm[1]+27); //imagemDialog.pack();
		 * imagemDialog.setDefaultCloseOperation
	...
	public java.awt.Image houghTransform() {
		...
		/*
		 * javax.swing.JFrame imagemDialog = new javax.swing.JFrame();
		 * imagemDialog.setTitle("Hough Image"); ImagePanel imagemCapturadaPanel
		 * = new ImagePanel(houghImage);
		 * imagemDialog.getContentPane().add(imagemCapturadaPanel); int[] dimIm
		 * = imagemCapturadaPanel.imageSize(); imagemDialog.setSize(dimIm[0]+8,
		 * dimIm[1]+27); //imagemDialog.pack();
		 * imagemDialog.setDefaultCloseOperation
	...
	public java.awt.Image houghCount() {
					...
					for (int indexa = 0; indexa <= center_counter; indexa++) {
						if (Math.abs(houghCircles[indexa][0] - x) < Math.ceil((double) radius / 2.)
							...
							if (info[indexa] <= media && (BWPixels[index] & 0x000000ff) != 255) {
								...
								// circulos[indexa][0] + ";" +
								// circulos[indexa][1] + " m:" + info[indexa] +
								// " with " + x + ";" + y + " m:" + media);
		...
		/*
		 * javax.swing.JFrame imagemDialog = new javax.swing.JFrame();
		 * imagemDialog.setTitle("Imagem houghCount"); ImagePanel
		 * imagemCapturadaPanel = new ImagePanel(houghCountImage);
		 * imagemDialog.getContentPane().add(imagemCapturadaPanel); int[] dimIm
		 * = imagemCapturadaPanel.imageSize(); imagemDialog.setSize(dimIm[0]+8,
		 * dimIm[1]+27); //imagemDialog.pack();
		 * imagemDialog.setDefaultCloseOperation
	...
	public java.awt.Image refineCount() {
		...
		double theta;
		int[] outPixels = null;
		// if (originalPixels == null)//for some stupid reason, this may not
		...
		for (int clusterIndex = 0; clusterIndex < cluster_counter; clusterIndex++) {
			...
			for (int dieIndex = 0; dieIndex < vDie[clusterIndex].size() - 1; dieIndex++) {
				...
				indice = centers[((Integer) vDie[clusterIndex].get(dieIndex)).intValue()][1] * imageWidth
						+ centers[((Integer) vDie[clusterIndex].get(dieIndex)).intValue()][0];
				// houghCircles[((Integer)vDie[clusterIndex].get(dieIndex)).intValue()][1]
				// * imageWidth +
				// houghCircles[((Integer)vDie[clusterIndex].get(dieIndex)).intValue()][0];
		...
		/*
		 * javax.swing.JFrame imagemDialog = new javax.swing.JFrame();
		 * imagemDialog.setTitle("Imagem fullCount"); ImageStorePanel
		 * imagemCapturadaPanel = new ImageStorePanel(houghCountImage);
		 * imagemDialog.getContentPane().add(imagemCapturadaPanel); int[] dimIm
		 * = imagemCapturadaPanel.imageSize(); imagemDialog.setSize(dimIm[0]+8,
		 * dimIm[1]+27); //imagemDialog.pack();
		 * imagemDialog.setDefaultCloseOperation
