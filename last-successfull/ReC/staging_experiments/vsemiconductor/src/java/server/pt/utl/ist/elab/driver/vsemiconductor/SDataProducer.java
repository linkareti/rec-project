...
/**
 * Classe que cont�m a simula��o propriamente dita. Este motor de dados vai
 * gerar uma colec��o de resultados de acordo com o input do recebido do
 * utilizador atrav�s do painel de defini��es ap�s se executar a simula��o,
 * apresentando estes resultados numa s�rie de formatos v�rios.
 */
public class SDataProducer extends BaseDataSource {
	 ...
	 * 
	 * a -> lattice constant (cm^-4 => m^-4) dc (eS/eO) -> dielectric constant
	 * (adimensional) eF -> Fermi energy level (eV) eG -> energy bandgap (eV =>
	 * V) el -> electrical field (V/cm => V/m) elM -> maximum field (talvez nao
	 * venha a ser usada...) e0 -> permittivity in vacuum (F/cm => F/m) eS ->
	 * semiconductor permittivity (F/cm => F/m) h -> Planck constant (J-s) k ->
	 * Boltzmann constant (J/K) kT (k*t) => kT nao e' utilizado como variavel de
	 * instancia) -> thermal energy m0 -> electron rest mass (Kg) mn -> electron
	 * effective mass (Kg) mp -> hole effective mass (Kg) ni2 -> intrinsic
	 * density^2 (cm^-3 => m^-3) nA -> acceptor impurity density (cm^-3 => m^-3)
	 * nC -> effective density of states in conduction band (cm^-3 => m^-3) nD
	 * -> donor impurity density (cm^-3 => m^-3) nV -> effective density of
	 * states in valence band (cm^-3 => m^-3) q -> magnitude of electronic
	 * charge (C) t -> absolute temperature (K) vBI -> built-in potencial (V) v
	 * -> voltage (V) w -> thickness (cm or microm => m) x -> position (same as
	 * w) xp -> first extreme position (same as w) xn -> second extreme position
	 * (same as w)
	 */
	...
	final static double e0 = 8.85418 * Math.pow(10, -14), m0 = 0.91095 * Math.pow(10, -30), h = 6.62617 * Math.pow(10,
			-34), k = 1.38066 * Math.pow(10, -23), q = 1.60218 * Math.pow(10, -19);
	...
	public void setMaterial(String material) {
		...
		// Calculo de mn e mp
		// a 300K, nV = 1.04 x 10^19 e nC = 2.8 x 10^19 (para o silicio) e nV =
		// 7.0 x 10^18 e nC = 4.7 x 10^17 (para o GaAs)
		if (silicio) {
	...
	/**
	 * M�todo para definir o n�mero de amostras a tomar. Basicamente, ser� o
	 * n�mero de itera��es para o qual se vai executar a simula��o. Servir� para
	 * determinar o ritmo de varia��o dos par�metros a considerar (caso variem
	 * ao longo da experi�ncia), assim como o tempo total (tempo "virtual") da
	 * simula��o.
	 */
	 ...
	 * servir� para avan�ar a simula��o no tempo de acordo com o definido pelo
	 * utilizador, servindo para determinar n�o s� o tempo total "virtual" da
	 * simula��o (com o n�mero de amostras) como para determinar a velocidade da
	 * anima��o e para calcular certas grandezas dependentes do tempo.
	 */
	...
	/**
	 * M�todo para configurar as vari�veis referentes � concentra��o de dadores.
	 * O primeiro argumento determina se h� alguma varia��o ao longo da
	 * simula��o desse par�metro e os seguintes o seu valor inicial e final (que
	 * s� ser� considerado se o primeiro par�metro for verdadeiro).
	 */
	 ...
	 * longo da simula��o desse par�metro e os seguintes o seu valor inicial e
	 * final (que s� ser� considerado se o primeiro par�metro for verdadeiro).
	 */
	...
	/**
	 * M�todo para configurar as vari�veis referentes � temperatura. O primeiro
	 * argumento determina se h� alguma varia��o ao longo da simula��o desse
	 * par�metro e os seguintes o seu valor inicial e final (que s� ser�
	 * considerado se o primeiro par�metro for verdadeiro).
	 */
	 ...
	 * aplicado. O primeiro argumento determina se h� alguma varia��o ao longo
	 * da simula��o desse par�metro e os seguintes o seu valor inicial e final
	 * (que s� ser� considerado se o primeiro par�metro for verdadeiro).
	 */
	...
	class Produtor extends Thread {
		...
		public void run() {
			...
			while (running && cSample < numeroAmostras) {
				...
				/*
				 * Calculo do nivel de Fermi - k e' dividido por q de forma a
				 * que a sua unidade seja eV
				 */
				eF = eG - (k / q) * t * Math.log(nC / nD);

				/*
				 * System.out.println("*************************");
				 * System.out.println("Ef = " + eF); System.out.println("EG = "
				 * + eG); System.out.println("k/q = " + (k/q));
				...
				if (linear) {
					vBI = (2.0 / 3.0) * ((k * t) / q)
							* Math.log((gradiente * gradiente * eS * (k * t) / q) / (8 * q * ni * ni * ni));

					if (v >= vBI)
						w = 0;
					else
						w = Math.pow(((12 * eS * (vBI - v)) / (q * gradiente)), 1.0 / 3.0);
					...
					if (v >= vBI)
						...
						w = Math.sqrt(((2 * eS) / q) * ((nA + nD) / (nA * nD)) * (vBI - v));
				...
				if (dadoresVaria)
					nD += dadoresVariacao;
				if (aceitadoresVaria)
					nA += aceitadoresVariacao;
				if (temperaturaVaria)
					t += temperaturaVariacao;
				if (potencialVaria)
					v += potencialVariacao;

				PhysicsValue[] value = new PhysicsValue[9];
				value[0] = new PhysicsValue(PhysicsValFactory.fromDouble(w), getAcquisitionHeader()
				...
				value[1] = new PhysicsValue(PhysicsValFactory.fromDouble(eG), getAcquisitionHeader().getChannelsConfig(
						1).getSelectedScale().getDefaultErrorValue(), getAcquisitionHeader().getChannelsConfig(1)
						.getSelectedScale().getMultiplier());
				value[2] = new PhysicsValue(PhysicsValFactory.fromDouble(vBI), getAcquisitionHeader()
				...
				value[3] = new PhysicsValue(PhysicsValFactory.fromDouble(e), getAcquisitionHeader()
				...
				if (temperaturaVaria) {
					value[4] = new PhysicsValue(PhysicsValFactory.fromDouble(t), getAcquisitionHeader()
				...
				value[5] = new PhysicsValue(PhysicsValFactory.fromDouble(eF), getAcquisitionHeader().getChannelsConfig(
						5).getSelectedScale().getDefaultErrorValue(), getAcquisitionHeader().getChannelsConfig(5)
						.getSelectedScale().getMultiplier());
				if (dadoresVaria) {
					value[6] = new PhysicsValue(PhysicsValFactory.fromDouble(nD), getAcquisitionHeader()
				...
				if (aceitadoresVaria) {
					value[7] = new PhysicsValue(PhysicsValFactory.fromDouble(nA), getAcquisitionHeader()
				...
				if (potencialVaria) {
					value[8] = new PhysicsValue(PhysicsValFactory.fromDouble(v), getAcquisitionHeader()
