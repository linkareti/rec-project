...
public class GraphPlot extends JPanel {
	...
	public GraphPlot(int plotStyle) {
	...
	public void setPlotValues(short[] values) {
	...
	public void drawAxes() {
		...
		// vertical axis:
		graphics.drawLine(plotRect.x, plotRect.y, plotRect.x, plotRect.y + plotRect.height);
		// horizontal axis:
		graphics.drawLine(plotRect.x, xAxisPos, plotRect.x + plotRect.width, xAxisPos);
	}

	public void plotPoints() {
		if (nPoints != 0) {
			graphics.setColor(plotColor);
			// default plot type has x axis in middle of plot with + and - y
			// axes:
			xAxisPos = plotRect.y + plotRect.height / 2;
			float xScale = plotRect.width / (float) nPoints;
			float yScale = 0.5f * plotRect.height / ymax;
			if (plotStyle == SPECTRUM) {
				xAxisPos = plotRect.y + plotRect.height; // x axis at bottom of
				// plot
				yScale = plotRect.height / ymax; // vertical scale based on full
				// plot height
			}
			if (logScale)
				xAxisPos = plotRect.y; // x axis at top of plot (0 dB)
	...
	public void paintComponent(Graphics g) {
		...
		plotRect.setBounds(horzInset, vertInset, getSize().width - 2 * horzInset, getSize().height - 2 * vertInset);
		drawAxes();
		if (logScale)
			drawGrid();
		plotPoints();
