				...
				if (getComponentOrientation().isLeftToRight()) {
					for (int i = 0; i < widths.length; i++) {
						...
						for (int j = 0; j < heights.length; j++) {
							...
							if (i == 0 || j == 0) {
								g.setColor(new Color(Math.max((int) (background.getRed() * FACTOR), 0), Math.max((int) (background
										.getGreen() * FACTOR), 0), Math.max((int) (background.getBlue() * FACTOR), 0)));
							} else {
				...
				for (int j = 0; j < heights.length; j++) {
					...
					if (j == 0) {
						g.setColor(new Color(Math.max((int) (background.getRed() * FACTOR), 0), Math.max(
								(int) (background.getGreen() * FACTOR), 0), Math.max((int) (background.getBlue() * FACTOR), 0)));
					} else {
						g.setColor(background);
					}
