...
public class ClientQueue implements QueueLogger {
    ...
    public void sendChatMessage(final UserInfo user, final String clientTo, final String message,
            final IResource resource, boolean fromElab) throws NotRegistered, NotAuthorized {
        synchronized (queueOrg) {
            if (!contains(user) && fromElab) {
                getClientQueueListener().log(
                        Level.INFO,
    ...
    public void sendChatMessage(final UserInfo user, final String clientTo, final String message,
        ...
        sendChatMessage(user, clientTo, message, resource, true);
    }

    public void sendMulticastChatMessage(final String clientTo, final String message, final boolean fromElab) {
        synchronized (queueOrg) {
            UserInfo otherUser = userNameToUserInfo(clientTo);

            if (!fromElab) {
                otherUser = new UserInfo(ChatMessageEvent.EVERYONE_USER);
            } else if (otherUser == null || otherUser.getUserName().equals(ChatMessageEvent.EVERYONE_USER_ALIAS)) {
                return;
            }
    ...
    public void sendMulticastChatMessage(final String clientTo, final String message) {
        sendMulticastChatMessage(clientTo, message, true);
