...
public class DataClientForQueue
{
	...
	if(!dcw.isConnected())
	{
	    getDataClientForQueueListener().log(Level.SEVERE,"Error getting username in DataClientForQueue - Throwing not authorized...");
    ...
    {
	//Just creating a Thread so that we don't call shutdown on the messageQueue from its own Thread...
	//otherwise we would block because the queue is waiting for a join to it's dispatching Thread...
        (new Thread()
        {
            public void run()
            {
	...
	getDataClientForQueueListener().log(Level.INFO,"client "+getUserName()+ " - informing dataClientForQueueListener that I'm gone!");
	...
	if(evt.getUserTo()==null || evt.getUserTo().getUserName()==null || evt.getUserTo().getUserName().equals(getUserName()))
	    messageQueue.addEvent(evt);
    }
     ...
     * @param dataClientForQueueListener New value of property dataClientForQueueListener.
     *
     */
    public void setDataClientForQueueListener(com.linkare.rec.impl.multicast.IDataClientForQueueListener dataClientForQueueListener)
    {
	...
	public void dispatchEvent(Object o)
	{
		...
		if(o instanceof ChatMessageEvent)
		{
		    ...
		    if(evt.getUserFrom()==null || evt.getUserTo()==null) return;
		...
		if(o instanceof HardwareLockEvent)
		{
		    HardwareLockEvent evt=(HardwareLockEvent)o;
                    //aqui o evento de HardwareLock que esta na Queue e desmultiplicado 
                    //para true CORBA way
	    ...
	    {
		getDataClientForQueueListener().logThrowable("Oooppss.. client gone? - Error dispatching event to client! Why? Gone?",e);
		if(!isConnected())
		{
		    shutdownAsSoonAsPossible();
		    return;
