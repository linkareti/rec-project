...
public class ReCFrameView extends FrameView implements ReCApplicationListener, ItemListener {
    ...
    private ApplicationActionMap getActionMap() {
        return org.jdesktop.application.Application.getInstance(ReCApplication.class).getContext().getActionMap(ReCFrameView.class, ReCFrameView.this);
    }
    ...
    public DefaultDialog<ApparatusTabbedHistoryPane> getNewExperimentHistoryDialogBox(final ExperimentUIData experimentUIData) {
        ...
        historyResultsPane.setExperimentResults(experimentUIData.getHistoryUINode(), experimentUIData.getDataModel(), experimentUIData.getDataDisplays());
        ...
        // Return dialog
        final DefaultDialog<ApparatusTabbedHistoryPane> dialog = new DefaultDialog<ApparatusTabbedHistoryPane>(getFrame(), experimentUIData.getHistoryUINode().getApparatusName(),
                apparatusTabbedPane);
        dialog.pack();
    ...
    private javax.swing.Action toggleConnectionStateActionData(final boolean connected) {
        final javax.swing.Action toggleConnectionStateAction = getContext().getActionMap(ReCFrameView.class, this).get("toggleConnectionState");
    ...
    private void askForVLC(boolean ask) {
        ...
        if (ask) {
            if (vlcPath == null) {
                ...
                final int result = JOptionPane.showConfirmDialog(getFrame(), "Não foi possível iniciar a reprodução de vídeo. " + LINE_SEPARATOR
                        + "No entanto, pode visualizar a experiência instalando " + LINE_SEPARATOR
                        + "o VLC. Deseja especificar a directoria de instalação do VLC?", "", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    private File getVLCExecutableFile() {
        ...
        do {
            ...
            if (!isVLCExecutable(selected)) {
                if (JOptionPane.showConfirmDialog(getFrame(), getResourceMap().getString("vlcexecutablefile.text"), "", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION) {
                    proceed = false;
                }
            }
        } while (!proceed);
    ...
    /**
     * Verifies if the specified file is in fact the vlc executable. Guarantees that no other executable file of other application is selected and launched by
     * eLab.
     * 
    ...
    private void customizerDone() {
        ...
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.customizerDone.text"), YELLOW);
    }
    ...
    private void connectToApparatus() {
        ...
        updateStatus(getResourceMap().getString("lblTaskMessage.connectedToApparatus.text", recApplication.getCurrentApparatusHardwareFamiliarName()));
        getApparatusDescriptionPane().setApparatusConfig(recApplication.getSelectedApparatusConfig());
    ...
    private void maxUsers() {
        final String errorMessage = ReCResourceBundle.findStringOrDefault("ReCUI$rec.ui.status.maxUsers", "Sorry, the lab is full. Please try again later...");
    ...
    private void lockableApparatus(final ApparatusConnectorEvent evt) {
        ...
        if (!recApplication.getExperimentAutoplay()) {
            ...
            getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusLockable.text",
    ...
    private void configuringApparatus() {
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusConfiguring.text"), GREEN);
        lockApparatus();
    ...
    private void configuredApparatus() {
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusConfigured.text"), GREEN);
    }

    private void startingExperiment() {
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusStarting.text"), YELLOW);
    }
    ...
    private void startedExperiment() {
        ...
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusStarted.text"), YELLOW);
        setExperimentAutoplay(false);
        setPlayButtonEnabled(false);
        setStopButtonEnabled(true);
    }
    ...
    private void stopedExperiment() {
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusStoped.text"), RED);
        setStopButtonEnabled(false);
    }
    ...
    private void configError() {
        ...
        progressCicleTask.stop();
        setStopButtonEnabled(false);
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusConfigErrorState.text"), RED);
        getApparatusTabbedPane().getExperimentActionBar().setPlayStopButtonEnabled(false);

        // showing this message can be necessary because the status bar message
        // is going to be hidden with stopped message
        final String errorMessage = ReCResourceBundle.findStringOrDefault("ReCUI$rec.ui.status.apparatus.config.error.state",
    ...
    private void incorrectStateExperiment() {
        ...
        progressCicleTask.stop();
        setStopButtonEnabled(false);
        getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusIncorrectState.text"), RED);
        getApparatusTabbedPane().getExperimentActionBar().setPlayStopButtonEnabled(false);

        final String errorMessage = ReCResourceBundle.findStringOrDefault("ReCUI$rec.ui.status.apparatus.incorrect.state",
    ...
    private void showExperimentResults(final ExperimentUIData experimentUIData) {
        ...
        getResultsPane().setExperimentResults(experimentUIData.getHistoryUINode(), experimentUIData.getDataModel(), experimentUIData.getDataDisplays());
    ...
    private void apparatusLockTimerTick() {
        final long lockCountDown = TimeUtils.msToSeconds(millisToLockSuccess - (TimeUtils.getSystemCurrentTimeMs() - apparatusLockInitialTimeMs));

        final boolean lockCountDownGreaterThanZero = lockCountDown > 0;

        if (lockCountDownGreaterThanZero) {
            getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusLockable.text", lockCountDown),
                    GREEN);
        }

        if (getExperimentActionBar().isPlayStopButtonEnabled() != lockCountDownGreaterThanZero) {
            getExperimentStatusActionBar().setActionStateText(getStatusActionBarResourceMap().getString("lblActionState.apparatusLockable.text", 0), GREEN);
	...
	progressCicleTask = new com.linkare.rec.impl.newface.component.ProgressCicle(ReCFrameView.idleIcon, ReCFrameView.busyIcons, busyAnimationRate);
	...
	final javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(com.linkare.rec.impl.newface.ReCApplication.class)
										    .getContext().getActionMap(ReCFrameView.class, this);
	toolBtnConnect.setAction(actionMap.get("toggleConnectionState")); // NOI18N
	toolBtnConnect.setBackground(null);
	final org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(com.linkare.rec.impl.newface.ReCApplication.class)
												     .getContext().getResourceMap(ReCFrameView.class);
    ...
    public void refreshView() {
        ...
        toolBtnConnect.getAccessibleContext().setAccessibleName(getResourceMap().getString("toolBtnConnect.AccessibleContext.accessibleName"));
    ...
    public void updateMenu(JMenuBar menuBar) {
        ...
        for (JMenu jMenu : menuFather) {
            jMenu.setText(ResourceBundle.getBundle("com.linkare.rec.impl.newface.resources.ReCFrameView").getString(jMenu.getName() + ".text"));
        }
