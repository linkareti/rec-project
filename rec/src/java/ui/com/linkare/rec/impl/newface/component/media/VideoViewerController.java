...
public class VideoViewerController {
	...
	private void refreshPausedVideo() {
		...
		if (state == MediaPlayerState.PAUSED) {
			...
			try {
				...
				// Catarino: esta excepção é mesmo para ignorar totalmente ou devemos apresentar o log pelo menos?
			}
			pause();
		}
	}
	...
	private void fireMediaTimeChangedEvent(MediaTimeChangedEvent evt) {
		...
		MediaApplicationEventListener[] listeners = eventListeners.getListeners(
				MediaApplicationEventListener.class);
	...
	private void fireMediaNotConnectedEvent(MediaNotConnectedEvent evt) {
		MediaApplicationEventListener[] listeners = eventListeners.getListeners(
				MediaApplicationEventListener.class);
		for (int i = 0; i < listeners.length; i++) {
