...
public class ReCBaseUI extends javax.swing.JFrame implements ICustomizerListener, ExpHistoryDisplayFactory {
	...
	public ReCBaseUI() {
		...
		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.commPlat",
				"Initializing Communication Platform..."));

		ORBBean.getORBBean();

		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.labWindow",
				"Initializing Main Laboratory Window..."));
		JDialog.setDefaultLookAndFeelDecorated(true);

		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.readPref",
				"Reading preferences..."));
		...
		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.webresource",
				"Loading Web Resources..."));

		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.ui",
				"Starting User Interface..."));

		reCSplash.setStatusMessage(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.init.done",
				"All Done!"));
	...
	private void initComponents()// GEN-BEGIN:initComponents
	{
		...
		jInternalFrameLabTree.setTitle(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.lbl.apparatuslist",
				"Apparatus List"));
		jInternalFrameLabTree.setFrameIcon(ReCResourceBundle.findImageIconOrDefault("ReCBaseUI$rec.bui.icon.tree",
		...
		jInternalFrameTabs.setTitle(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.title.historyAndUser",
				"History & Users List"));
		...
		jTabbedPaneInfoUsers.addTab(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.title.apparatusUsersList",
				"Apparatus Users List"), ReCResourceBundle.findImageIconOrDefault("ReCBaseUI$rec.bui.icon.users",
				new ImageIcon(getClass().getResource("/com/linkare/rec/impl/baseUI/resources/UserList16_2.gif"))),
				usersListPanel, ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.tip.users",
		...
		statusPanelApparatus.setLblStatus(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.apparatus",
				"Apparatus Status:"));
		...
		statusPanelLab.setLblStatus(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.lab",
				"Laboratory Status:"));
		...
		jLabel1.setIcon(ReCResourceBundle.findImageIconOrDefault("ReCBaseUI$rec.bui.icon.sponsor", new ImageIcon(
				getClass().getResource("/com/linkare/rec/impl/baseUI/resources/RecLogo2Gray.png"))));
		gridBagConstraints = new java.awt.GridBagConstraints();
	...
	{// GEN-HEADEREND:event_apparatusClientBeanApparatusStateStarted
		...
		statusPanelApparatus.setStatus(ReCResourceBundle.findStringOrDefault("ReCBaseUI$rec.bui.status.started",
				"Started..."));
	...
	{// GEN-HEADEREND:event_apparatusClientBeanApparatusLockable
		...
		timerLock.scheduleAtFixedRate((lockerTask = new java.util.TimerTask() {
			public void run() {
				if (locktried) {
					cancel();
			...
			if (currentCustomizer != null) {
				...
				if (!showedMessageConfig) {
					...
					String message = ReCResourceBundle
							...
							+ url.toExternalForm()
							+ ReCResourceBundle
									.findStringOrDefault("ReCBaseUI$rec.bui.lbl.helpSelectConfigAfter",
											"'><br>to access the experiment <br>configuration utility!</font></center></body></html>");
	...
	{// GEN-HEADEREND:event_laboratoryTreeDisplaySelectionChange
		...
		for (int i = 0; i < offlineApparatusDisplaysList.size(); i++) {
			if (((OfflineInternalFrame) offlineApparatusDisplaysList.get(i)).getHardwareID().equals(
					evt.getApparatus().getLocation())) {
	...
	public void canceled() {// silent noop - Customizer canceled...
	}
	...
	public void showExperimentHeader(ExpHistory expHistory) {
		...
		frame.pack();
		frame.setLocation((getLocation().x + getWidth()) / 2 - frame.getWidth() / 2, (getLocation().y + getHeight())
				/ 2 - frame.getHeight() / 2);
	...
	public void startExperiment(ExpHistory abstractExpHistory) {
		...
		try {
			// /factoryLocation =
			// ReCResourceBundle.findStringOrDefault(currentApparatusConfig.getDisplayFactoryClassLocationBundleKey(),
			// null);
	...
	/*
	 * private void
	 * disableApparatus(com.linkare.rec.impl.baseUI.config.Apparatus app) {
	 * app.setEnabled(false); Display[] dps = app.getDisplay(); for(int i=0;
	 * i<dps.length; i++) { if(!dps[i].getOfflineCapable())
	 * dps[i].setEnabled(false); }
	 * 
	 * DefaultAcquisitionConfig[] dfac = app.getDefaultAcquisitionConfig();
	 * for(int j=0; j<dfac.length; j++) { dfac[j].setEnabled(false); } }
	 */
