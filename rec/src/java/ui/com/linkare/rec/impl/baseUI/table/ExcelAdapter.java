...
public class ExcelAdapter implements ActionListener {
	...
	public void copyToClipBoard() {
		...
		/*
		 * StringBuffer sbf=new StringBuffer(); // Check to ensure we have
		 * selected only a contiguous block of // cells int
		 * numcols=jTable1.getSelectedColumnCount(); int
		 * numrows=jTable1.getSelectedRowCount(); int[]
		 * rowsselected=jTable1.getSelectedRows(); int[]
		 ...
		 * sbf.append(jTable1.getColumnName(colsselected[j])); if (j<numcols-1)
		 * sbf.append("\t"); } sbf.append("\n"); for (int i=0;i<numrows;i++) {
		 * for (int j=0;j<numcols;j++) {
		 * sbf.append(jTable1.getValueAt(rowsselected[i],colsselected[j])); if
