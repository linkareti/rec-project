...
public class ResultsPane extends AbstractContentPane implements ExpDataModelListener, ActionListener {
    ...
    private void addExperimentDataDisplays(final List<ExpDataDisplay> displays) {
        ...
        for (final ExpDataDisplay expDataDisplay : displays) {
            if (autoAcquireResult) {
                if (DataDisplayEnum.CHART.equals(expDataDisplay.getDisplayType())) {
                    firstDisplay = expDataDisplay.getName();
                    break;
                } else if (DataDisplayEnum.DATA_TABLE.equals(expDataDisplay.getDisplayType())) {
                    firstDisplay = expDataDisplay.getName();
                    continue;
                } else if (DataDisplayEnum.SENSOR.equals(expDataDisplay.getDisplayType()) && firstDisplay == null) {
                    firstDisplay = expDataDisplay.getName();
                    continue;
                } else if (DataDisplayEnum.ANIMATION.equals(expDataDisplay.getDisplayType()) && firstDisplay == null) {
                    firstDisplay = expDataDisplay.getName();
                    continue;
                }
            } else if (firstDisplay == null) {
                firstDisplay = expDataDisplay.getName();
            }
        }
