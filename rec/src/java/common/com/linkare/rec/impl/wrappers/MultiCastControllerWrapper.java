...
/**
 * This class works as a wrapper to the MCController Server
 * It keeps checking the connection state on an error
 * A client may use the method isConnected if in need to determine the current status
 * of the connection to the server
 * @see com.linkare.rec.acquisition.MultiCastControllerOperations
 * 
 * @author José Pedro Pereira - Linkare TI
 */
public class MultiCastControllerWrapper implements
		MultiCastControllerOperations {

	/*The name of the logger to use*/
	private static String MCCONTROLLER_LOGGER = "MultiCastController.Logger";

	/*The internal reference to the MultiCastController - the one that actually does the work*/
	 ...
	 *	@param delegate The real MultiCastController that must do the work
	 *	Note that this will check the connection state on start,
	 *	so delegate should be a valid reference as obtained from CORBA 
	 */
	...
	private void checkConnect() {
		...
		try {
			...
			logThrowable("Couldn't determine remote existence of MultiCastController. Assuming disconnected...",
							e);
	...
	/**
	 * This method verifies the connection to the server by invoking 
	 * internal CORBA methods on the delegate reference
	 * @return true if the connection can be determined from a non_existent CORBA call, otherwise false
	 */
	...
	/**
	 * This method determines if other wrapper is just another reference 
	 * to the same server...
	 * @param other the MultiCastControllerWrapper that needs to be checked against this one
	 * @return true if this MultiCastControllerWrapper has a reference to the same server as the other wrapper
	...
	/**
	 * This method determines if other multicastcontroller reference is just another reference 
	 * to the same server...
	 * @param other the MultiCastController that needs to be checked against this one's delegate
	 * @return true if this MultiCastControllerWrapper has a reference to the same server as the other delegate
	 * @see org.omg.CORBA.Object#_is_equivalent(org.omg.CORBA.Object)
	 */
	 ...
	 */
	public UserInfo[] getClientList(UserInfo user) throws NotRegistered,
			NotAuthorized {
		if (delegate == null) {
			log(Level.WARNING,
							"MultiCastController has not been set! Please set it first...");
