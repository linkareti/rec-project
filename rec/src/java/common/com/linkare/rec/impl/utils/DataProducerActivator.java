...
public class DataProducerActivator extends LocalObject implements ServantActivator
{
    ...
    public void etherealize(byte[] oid, POA poa, Servant servant, boolean param, boolean param4)
    {
        ...
        try
        {
            Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Etherealizing object "+ (new String(oid)) + " in POA "+poa.the_name());
            ...
            if(!objdataser.alreadySerialized())
            {
                ...
                Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Deactivating object "+ filename + " in POA "+poa.the_name());
                SerializationHelper.writeObject(filename,dir,objdataser);
                Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Serializing for the first time...!");                
                Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Deactivated object "+ filename + " in POA "+poa.the_name());
            }
            else
            {
                Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Object already serialized no needed to serialize again!");
            }            
            
        }catch(java.io.IOException e)
        {
            LoggerUtil.logThrowable("Error deactivating dataproducer... - Throwing CORBA OBJ_ADAPTER Error",e,Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER));
    ...
    public org.omg.PortableServer.Servant incarnate(byte[] oid, POA poa) throws ForwardRequest
    {
        ...
        try
        {
            ...
            Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Activating object "+ filename + " in POA "+poa.the_name());
            
            Object readed = SerializationHelper.readObject(filename,dir);
            //System.out.println("is readed an instance of ReCMultiCastDataProducer?? " + (readed instanceof ReCMultiCastDataProducer));
            ...
            Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER).log(Level.FINE,"Activated object "+ filename + " in POA "+poa.the_name());
            return s;
        }catch(java.io.IOException e)
        {
            LoggerUtil.logThrowable("Error activating dataproducer... - Throwing CORBA OBJ_ADAPTER Error",e,Logger.getLogger(RECMULTICAST_DATAPRODUCER_LOGGER));
            throw new org.omg.CORBA.OBJ_ADAPTER(e.getMessage());
        }
