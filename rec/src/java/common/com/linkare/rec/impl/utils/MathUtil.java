...
public class MathUtil
{
	...
	System.err.println("value " + value +" should be in scale ["+min_value+" - " + max_value + "] step=" + step_value);
	if(step_value==0.) 
	{
	    System.err.println("step_value is 0., returning (value<=max_value && value>=min_value)=" + (value<=max_value && value>=min_value));
	...
	/*int dec_places_step=0;
	double step_value_temp=step_value;
	while(Math.floor(step_value_temp)<step_value_temp)
	{
	    dec_places_step++;
	    step_value_temp=step_value_temp*10.;
	}
	
	double div=(value-min_value)/step_value;
	double power_mult=Math.pow(10.,dec_places_step);
