package com.linkare.rec.acquisition;


/**
* com/linkare/rec/acquisition/IncorrectStateException.java .
* Generated by the IDL-to-Java compiler (portable), version "3.1"
* from I:/Projects/REC/IdlCompile/ReC7.idl
* Sexta-feira, 2 de Janeiro de 2004 15H12m GMT
*/

public final class IncorrectStateException extends org.omg.CORBA.UserException
{
  public int errorCode = (int)0;
  public com.linkare.rec.acquisition.HardwareState actualState = null;
  public com.linkare.rec.acquisition.HardwareState requiredState = null;

  public IncorrectStateException()
  {
    super(IncorrectStateExceptionHelper.id());
  } // ctor

  public IncorrectStateException(int _errorCode, com.linkare.rec.acquisition.HardwareState _actualState, com.linkare.rec.acquisition.HardwareState _requiredState)
  {
    super(IncorrectStateExceptionHelper.id());
    errorCode = _errorCode;
    actualState = _actualState;
    requiredState = _requiredState;
  } // ctor


  public IncorrectStateException(String $reason, int _errorCode, com.linkare.rec.acquisition.HardwareState _actualState, com.linkare.rec.acquisition.HardwareState _requiredState)
  {
    super(IncorrectStateExceptionHelper.id() + "  " + $reason);
    errorCode = _errorCode;
    actualState = _actualState;
    requiredState = _requiredState;
  } // ctor

} // class IncorrectStateException
