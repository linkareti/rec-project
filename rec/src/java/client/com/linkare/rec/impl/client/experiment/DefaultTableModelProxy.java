...
public class DefaultTableModelProxy extends javax.swing.table.DefaultTableModel implements ExpDataModelListener
{
	...
	public String getColumnName(int columnIndex)
	{
		if(expDataModel==null || !expDataModel.isDataAvailable())
		{
			if(columnIndex==0) return "No data available...";
			return null;
		}
		if(columnIndex==0) return "Sample Num";
		if(columnIndex==1) return "Date";
		if(columnIndex==2) return "Time";
		if(columnIndex==3) return "Milliseconds";
	...
	public Object getValueAt(int rowIndex, int columnIndex)
	{
		...
		if(columnIndex==1) return expDataModel.getTimeStamp(rowIndex).getDate().toString();
		if(columnIndex==2) return expDataModel.getTimeStamp(rowIndex).getTime().toSimpleTimeString();
		if(columnIndex==3) return ""+expDataModel.getTimeStamp(rowIndex).getTime().getMilis();
