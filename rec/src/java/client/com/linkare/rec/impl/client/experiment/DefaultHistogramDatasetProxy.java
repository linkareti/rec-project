...
public class DefaultHistogramDatasetProxy extends org.jfree.data.xy.AbstractIntervalXYDataset implements
	...
	private void recalculateAll() {
		...
		for (int i = 0; i < nSamples; i++) {
			if (expDataModel.getValueAt(i, channelDisplay) != null) {
				...
				int block_to_add = (int) Math.floor((xValue - xMean) / deltaX + ((double) nClasses) / 2.); // adicionar
				// ao
				// bloco
				// em
				// que
				...
				oCountHits.set(block_to_add, new Long(((Long) oCountHits.get(block_to_add)).longValue() + 1));// adicionar
				// ao
				// valor
				// anterior...
			}
	 ...
	 * @return the x-value.
	 */
	public double getXValue(int series, int item) {// item varia entre 0 e
		// nClasses-1
