...
public final class ChannelInfo implements IDLEntity
{
    ...
    // @param	actual_selected_scale_index	actual_selected_scale_index struct member
    // @param	frequencies	frequencies struct member
    // @param	actual_selected_frequency_index	actual_selected_frequency_index struct member
    ...
    //
    public ChannelInfo(String channel_name, com.linkare.rec.data.metadata.Scale[] scales, Scale actual_selected_scale, com.linkare.rec.data.metadata.FrequencyScale[] frequencies, Frequency selected_frequency, com.linkare.rec.data.metadata.ChannelParameter[] channel_parameters, com.linkare.rec.data.metadata.ChannelDirection channel_direction, boolean is_channel_independent, SamplesNumScale samplingScale)
    {
	...
	this.setScales(temp);temp=null;System.gc();
	...
	this.setFrequencies(temp2);temp2=null;System.gc();
	...
	this.setChannelParameters(temp3);temp3=null;System.gc();
	...
	else
	{
	    ...
	    temp[index]=scales;
	    this.scales=temp;temp=null;System.gc();
	}
	...
	else
	{
	    ...
	    this.frequencies=temp;temp=null;System.gc();
	}
     ...
     * @param actualSelectedFrequencyIndex New value of property actualSelectedFrequencyIndex.
     */
	...
	else
	{
	    ...
	    this.channelParameters=temp;temp=null;System.gc();
	}
	...
	/*
	 public void updateFromChannelAcquisitionHeader(ChannelAcquisitionConfig ch_acq_header)
	{
		for(int i=0;i<channelParameters.length;i++)
		{
		channelParameters[i].setSelectedParameterValue(ch_acq_header.getSelectedChannelParameterValue(channelParameters[i].getParameterName()));
		}
	}
	 */

    public void validateConfig(ChannelAcquisitionConfig config)
	...
	if(samplingScale!=null)
	    if(! ( 
		    total_samples>=samplingScale.getMinSamples() &&
	...
	if(scale_selected_in_conf==null) return;
	...
	if(channelParameters!=null)
	    for(int i=0;i<channelParameters.length;i++)
		config.addSelectedChannelParameter(new ParameterConfig(channelParameters[i].getParameterName(),channelParameters[i].getSelectedParameterValue()));
