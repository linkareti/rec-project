...
public abstract class DataCollector extends Thread implements Serializable
{
    ...
    {
        if(newState.equals(dataCollectorState)) return;
        
        if(newState.equals(DataCollectorState.DP_WAITING))
        {
            log(Level.INFO,"Trying to set Data Collector State to " + newState + " while current DataProducer State is "+dataCollectorState +" - this transition is not possible!");
        }
        ...
        {
            if(!( dataCollectorState.equals(DataCollectorState.DP_WAITING)
            || dataCollectorState.equals(DataCollectorState.DP_STARTED_NODATA)
            )
            )
                log(Level.INFO,"Trying to set Data Collector State to " + newState + " while current DataProducer State is "+dataCollectorState +" - this transition is not possible!");
            else
        ...
        else if(newState.equals(DataCollectorState.DP_ENDED))
        {
            if(!( dataCollectorState.equals(DataCollectorState.DP_WAITING)
            || dataCollectorState.equals(DataCollectorState.DP_STARTED_NODATA)
            || dataCollectorState.equals(DataCollectorState.DP_STARTED)
            )
            )
                log(Level.INFO,"Trying to set Data Collector State to " + newState + " while current DataProducer State is "+dataCollectorState +" - this transition is not possible!");
            else
    ...
    public void run()
    {
        ...
        while(samplesPackets.size()-1<=largestPacketKnown && remoteDataProducerState.equals(DataProducerState.DP_STARTED) && !exit)
        {
            while(pause && !exit)
            {
                synchronized(synchWaitFetchData)
            ...
            while(remoteDataProducerState.equals(DataProducerState.DP_STARTED) && samplesPackets.size()-1==largestPacketKnown && !exit)
            {
                synchronized(synchWaitFetchData)
                {
                    try
    ...
    private void fetchPackets()
    {
        ...
        if(lastPacket>largestPacketKnown || largestPacketKnown == MaxPacketNumUnknown.value) return;
        try
        {
        ...
        }catch(NotAnAvailableSamplesPacketException e)
        {
            ...
            if(lastPacket>largestPacketKnown) return;
            try
            {
                addSamplesPackets(getRemoteDataProducer().getSamples(lastPacket, largestPacketKnown));
    ...
    public void fetchLastPackets()
    {
        try
        {
            ...
            System.out.println("***********************************");
            System.out.println("***********************************");
            System.out.println("Fetching last packets... largest packet known after direct query to the remote data producer is now " +largestPacketKnown);
            System.out.println("***********************************");
            System.out.println("***********************************");
            System.out.println("***********************************");*/
