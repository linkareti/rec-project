...

{
    ...
    //Added by André... the free mem should be defined by in the init scripts...
	    ...
	    
	{
		...
		throw new NotAnAvailableSamplesPacketException(NotAnAvailableSamplesPacketExceptionConstants.PACKET_NOT_FOUND_IN_CACHE, packet_num);
	    ...
	    
	{
		...
		LoggerUtil.logThrowable("Tryed to serialize packets but error occurred", e,Logger.getLogger("SamplesPacketIO"));
		
		Logger.getLogger("SamplesPacketIO").log(Level.WARNING,"Couldn't serialize SamplePackets to file... Defaulting to in memory ArrayList!");
	...
	return ((double)Runtime.getRuntime().freeMemory()<=percentFreeThreshold/100.*(double)Runtime.getRuntime().maxMemory());
	...
	com.linkare.rec.impl.data.SamplesPacketSourceEvent event=new com.linkare.rec.impl.data.SamplesPacketSourceEvent(this, packetLargestIndex);
	    ...
	    
	{
		...
		//System.out.println("Firing newSamplesPackets at SamplesPacketMatrix to registered only " + packetLargestIndex);
	    ...
	    
	{
		...
		try
		{
		    ...
		    samplesStream.flush();samplesStream.close();
