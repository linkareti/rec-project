...
public class SamplesSourcePacketizer implements SamplesPacketSource
{
	...
	if(packetsLocations==null || getSamplesSource()==null)
	    throw new SamplesPacketReadException("Error packetizing... probably source is not set yet", packetStartIndex);
	...
	for(int i=0;i<retVal.length;i++)
	{
	    ...
	    {
		throw new SamplesPacketReadException("Error reading sample index " + e.getErrorSampleNumber() + " for this packet",packetStartIndex+i);
	    }
	...
	int[] startAndEndLoc=new int[]{currentPacketSampleStartIndex,currentPacketSampleStartIndex+countSamplesCurrentPacket-1};
	...
	do
	{
	    int[] startAndEndLoc=new int[]{currentPacketSampleStartIndex,currentPacketSampleStartIndex+getPacketSize()-1};
	    packetsLocations.add(startAndEndLoc);
    ...
    {
        System.out.println("--->>>>>>>>>Firing new samples Packet at Samples Source Packetizer refreshState " + packetLargestIndex);
