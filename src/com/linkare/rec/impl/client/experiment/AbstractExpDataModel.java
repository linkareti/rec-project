...
public abstract class AbstractExpDataModel extends DataCollector implements ExpDataModel, DataReceiverOperations
{
    ...
    private DataReceiver getDataReceiver()
    {
        ...
        try
        {
            return (_thisDataReceiver=DataReceiverHelper.narrow(ORBBean.getORBBean().getAutoIdRootPOA().servant_to_reference(ORBBean.getORBBean().registerAutoIdRootPOAServant(DataReceiver.class,this,(oid=new ObjectID())))));
            
        }catch(Exception e)
        {
            e.printStackTrace();
            LoggerUtil.logThrowable("DataReceiver not registered with ORB...",e,LogManager.getLogManager().getLogger(DATA_RECEIVER_LOGGER));
            return null;
        }
    ...
    public void setDpwDataSource(DataProducerWrapper remoteDataProducer) throws MaximumClientsReached
    {
        ...
        {
            Logger.getLogger(DATA_RECEIVER_LOGGER).log(Level.SEVERE,"DataProducer is disconnected! Check it out, please...");
        }
    ...
    public HardwareAcquisitionConfig getAcquisitionConfig()
    {
        ...
        else
        {
            ...
            {
                LoggerUtil.logThrowable("Couldn't get Acquisition Header!",nae,LogManager.getLogManager().getLogger(DATA_RECEIVER_LOGGER));
            }
        }
    ...
    {
        if(apparatusName!=null) return apparatusName;
        ...
        {
            LoggerUtil.logThrowable("Couldn't get Apparatus Name from Acquisition Header!",npe,LogManager.getLogManager().getLogger(DATA_RECEIVER_LOGGER));
        }
    ...
    private void fireExpDataModelListenerDataModelStartedNoData()
    {
        ...
        for (int i = listeners.length-2; i>=0; i-=2)
        {
            if (listeners[i]==com.linkare.rec.impl.client.experiment.ExpDataModelListener.class)
            {
                ((com.linkare.rec.impl.client.experiment.ExpDataModelListener)listeners[i+1]).dataModelStartedNoData();
            }
        }
    }
    ...
    public void fireStateChanged()
    {
        ...
        if(getDataCollectorState().equals(com.linkare.rec.impl.utils.DataCollectorState.DP_ENDED))
        {
            ...
            if(dataProducerRunningCheck!=null)
            {
                ...
                //System.out.println("Took me "+(System.currentTimeMillis()-millis)/1000 + " s to shutdown the Thread checking if remote data producer is alive...");
            }
        }
     ...
     * @param listener The listener to register.
     */
    public synchronized void addSamplesPacketSourceEventListener(com.linkare.rec.impl.data.SamplesPacketSourceEventListener listener)
    {
        if (listenerList == null )
        {
            listenerList = new javax.swing.event.EventListenerList();
     ...
     * @param listener The listener to remove.
     */
    public synchronized void removeSamplesPacketSourceEventListener(com.linkare.rec.impl.data.SamplesPacketSourceEventListener listener)
    {
     ...
     * @param event The event to be fired
     */
    private void fireSamplesPacketSourceEventListenerNewSamplesPackets(com.linkare.rec.impl.data.SamplesPacketSourceEvent event)
    {
        if (listenerList == null) return;
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length-2; i>=0; i-=2)
        {
            ...
            {
                ((com.linkare.rec.impl.data.SamplesPacketSourceEventListener)listeners[i+1]).newSamplesPackets(event);
            }
        }
    }
    ...
    public com.linkare.rec.data.acquisition.PhysicsValue getValueAt(int sampleIndex, int channelIndex)
    {
        ...
        {
            e.printStackTrace();
            LoggerUtil.logThrowable("Exception when getValueAt() from AbstractDataModel",e,Logger.getLogger(DATA_RECEIVER_LOGGER));
            return null;
        }
    }
    ...
    private class SamplesDepacketizingAdapter implements SamplesSourceEventListener
    {
        public void newSamples(SamplesSourceEvent evt)
        {
            ...
            {
                LoggerUtil.logThrowable("Exception when newSamples(SamplesSourceEvent evt) from AbstractDataModel",e,Logger.getLogger(DATA_RECEIVER_LOGGER));
            }
        }
    }
            ...
            while(!shutdown)
            {
                ...
                if(millisChecked+5000<System.currentTimeMillis())
                {
                    try
                    {
                        if(!remoteDataProducer.isConnected())
                        {
                            Logger.getLogger(DATA_RECEIVER_LOGGER).log(Level.WARNING,"remote Data Producer is gone... tested connected and it returned false...");
                    ...
                    {
                        LoggerUtil.logThrowable("remote Data Producer is gone... tested connected and it has blown with an exception...",e,LogManager.getLogManager().getLogger(DATA_RECEIVER_LOGGER));
                        remoteDataProducerGone();
                        return;
                    }
                }
