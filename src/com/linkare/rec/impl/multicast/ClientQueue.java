...
public class ClientQueue
{
    ...
    public UserInfo[] getUsers(UserInfo user,IResource resource)
    throws NotRegistered,NotAuthorized
    {
        ...
        if(!contains(user))
        {
            getClientQueueListener().log(Level.INFO,"ClientQueue - Client " + user.getUserName()+ " doesn't exist here! - Going to send a NotRegistered Exception!");
            throw new NotRegistered();
        }
        
        if(!SecurityManagerFactory.authorize(resource,securityUser,op1))
        {
            getClientQueueListener().log(Level.INFO,"ClientQueue - Client " + user.getUserName()+ " is not authorized to make this operation! - Going to send a NotAuthorized Exception!");
        ...
        System.arraycopy(userInfosOut,0, retVal, 0, retVal.length);
        return retVal;*/
    ...
    throws NotRegistered,NotAuthorized
    {
        synchronized(queueOrg)
        {
            if(!contains(user))
            {
                getClientQueueListener().log(Level.INFO,"ClientQueue -  The user " + user.getUserName() + " is not registered! - Going to send a NotRegistered Exception");
                throw new NotRegistered();
            }
            ...
            {
                getClientQueueListener().log(Level.INFO,"ClientQueue - The user " + user.getUserName() + " is not authorized to send messages to the user " + (clientTo==IChatServer.EVERYONE_USER?IChatServer.EVERYONE_USER_ALIAS:clientTo) +  " ! - Going to send a NotAuthorized Exception!");
                throw new NotAuthorized(NotAuthorizedConstants.NOT_AUTHORIZED_OPERATION);
            }
    ...
    public boolean add(DataClient dc,IResource resource)
    throws MaximumClientsReached,NotAuthorized
    {
        ...
        synchronized(queueOrg)
        {
            ...
            {
                getClientQueueListener().log(Level.INFO,"ClientQueue - Maximum capacity reached... Going to deny client's request for registration! Maybe try later?");
                throw new MaximumClientsReached(MaximumClientsReachedConstants.MAXIMUM_CLIENTS_REACHED_IN_HARDWARE_QUEUE,getMaximumClients());
            }
            else
            {
        ...
        getClientQueueListener().log(Level.INFO,"ClientQueue : checking to see if DataClient " + dcfq.getUserName() + " is allready registered!");
        ...
        if(contains(dcfq.getUserInfo()))
        {
            ...
            {
                //Let's wait for the other client to unregister shall we? Come on give the guy a break...
                synchronized(this)
                {
                    try
                    {
                        ...
                        {
                            //Waiting for other to finish...please be patient...at least 20 seconds of the messageQueue shutdown...
                            wait(1000);
                        }
                    }
                    catch(Exception e)
            ...
            {
                getClientQueueListener().log(Level.INFO,"ClientQueue : client " + otherDcfq.getUserName() + " is not conencted - shutting it down!");
                otherDcfq.shutdown();
		synchronized(this)
                {
                    try
            ...
            {
                getClientQueueListener().log(Level.INFO,"ClientQueue : There is allready a user with that name : " + otherDcfq.getUserName() + " - Sending a NotAuthorized Exception!");
        ...
        synchronized(queueOrg)
        {
            getClientQueueListener().log(Level.INFO,"ClientQueue - Going to register client " + dcfq.getUserName() + " !");
            retVal=queueOrg.add(dcfq);
            getClientQueueListener().log(Level.INFO,"ClientQueue - registered client " + dcfq.getUserName() + (retVal?"successfully!":"failed!"));
        }
    ...
    public boolean add(DataClientForQueue dcfq,IResource resource)
    throws MaximumClientsReached,NotAuthorized
    {
        getClientQueueListener().log(Level.INFO,"ClientQueue - trying to register new client!");
        boolean retVal=false;
        ...
        synchronized(queueOrg)
        {
            try
            {
                if(queueOrg.contains(dcfq))
                {
                    ...
                    if(!cyclingQueue)
                    {
                        getClientQueueListener().log(Level.INFO,"ClientQueue - "+ (returnVal?"Removed":"Failed to remove")  +" client " + dcfq.getUserName() +"!");
                        if(returnVal)
                        {
                            getClientQueueListener().log(Level.INFO,"ClientQueue - informing client is gone " + dcfq.getUserName() +"!");
                            clientQueueListener.dataClientForQueueIsGone(dcfq);
                        }
                    }
                }
                else
                    getClientQueueListener().log(Level.INFO,"ClientQueue - client " + dcfq.getUserName() +" isn't registered here!");
                
            }
            catch(Exception e)
            {
                getClientQueueListener().logThrowable("ClientQueue - Error " + (cyclingQueue?"cycling":"removing") + " client " + dcfq.getUserName() +"!",e);
            }
        }
    ...
    public DataClientForQueue getWrapperForDataClient(DataClient client)
    {   //????Isto ´´e estupido agora... mas nao sei o que mais vai quebrar...
        //pode ser que esteja a usar isto noutros sitions... antes de apagar convem
        //perceber onde... e verificar se afinal nao faz assim tanto sentido...
        //pode eventualmente fazer sentido para o chat...
        //TODO check with JP, this wasn't working well...I'm going to check with username because it works
        //apparently the delegates aren't the same...
        /* exactly - the wrappers shouldn't be the same... this is highly stupid from me...
         *the fact is that LabClientBeans is one client and Apparatus clientbean ´´e outro... very very stupid from me...
        Iterator iter=iterator();
        while(iter.hasNext())
        {
            DataClientForQueue dcfq=(DataClientForQueue)iter.next();
            if(dcfq.getDataClient().isSameDelegate(client))
                return dcfq;
        }
        return null;*/
    ...
    private class ClientQueueDispatcher implements EventQueueDispatcher
    {
        ...
        public void dispatchEvent(Object o)
        {
            ...
            {
                if(cachedHardwareState==null || !cachedHardwareState.equals(((HardwareStateChangeEvent)o).getNewState()))
                {
                    cachedHardwareState=((HardwareStateChangeEvent)o).getNewState();
                    
                    getClientQueueListener().log(Level.INFO,"ClientQueue - dispatching Hardware State change event. New State is: " + ((HardwareStateChangeEvent)o).getNewState());
                    ...
                    while(iter.hasNext())
                    {
                        ...
                        {
                            getClientQueueListener().logThrowable("ClientQueue - Error dispatching Hardware State change events to clients!",e);
                        }
                    }
            ...
            {
                getClientQueueListener().log(Level.INFO,"The current free memory is: " +  Runtime.getRuntime().freeMemory() );
                
                getClientQueueListener().log(Level.INFO,"ClientQueue - dispatching Hardware Lock event to client " +  ((HardwareLockEvent)o).getLockerClient().getUserName() );
                ...
                {
                    getClientQueueListener().logThrowable("ClientQueue - Error dispatching Hardware Lock event to client "+ ((HardwareLockEvent)o).getLockerClient().getUserName(),e);
                }
            }
            else if(o instanceof HardwareChangeEvent)
            {
                ...
                while(iter.hasNext())
                {
                    ...
                    {
                        getClientQueueListener().logThrowable("ClientQueue - Error dispatching Hardware Change event!",e);
                    }
                }
            }
