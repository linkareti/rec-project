...
public final class SecurityManagerFactory
{
    ...
    //public static final String MCCONTROLLER_SECURITYMANAGER_LOGGER=ReCMultiCastController.MCCONTROLLER_LOGGER;
	...
	{
	    Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"SecurityManager System Property not found... Loading DefaultSecurityManager!");
	...
	{
	    Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"Unable to load SecurityManager defined at system : "+secManagerClassName+" - loading DefaultSecurityManager!");
	    LoggerUtil.logThrowable("Error loading specified SecurityManager", e, Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER));
	    secManager=new DefaultSecurityManager();
	}
	...
	if(secManager==null)
	{
	    Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"Setting SecurityManager to null... Loading DefaultSecurityManager!");
	    this.secManager=new DefaultSecurityManager();
	}
	else
	{
	    Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"Setting SecurityManager to instance of class "+secManager.getClass().getPackage().getName()+"."+secManager.getClass().getName());
	...
	{
	    LoggerUtil.logThrowable("Exception occured authenticating user! Returning false!", e, Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER));
	    return false;
	}
    }
	...
	{
	    LoggerUtil.logThrowable("Exception occured authorizing user for operation! Returning false!", e, Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER));
	    return false;
	}
    }
	...
	{
	    //Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"Authenticating access for Resource: " + resource + " to User: " + user +" OK!");
	    return true;
	}
	
	public boolean authorize(IResource resource, IUser user, IOperation op)
	{
	    //Logger.getLogger(MCCONTROLLER_SECURITYMANAGER_LOGGER).log(Level.INFO,"Authorizing Operation " + op + " for Resource: " + resource + " to User: " + user +" OK!");
	    return true;
	}
    }
