...
public class SaveData {
                        ...
                        pt.utl.ist.elab.client.webrobot.customizer.Comps.IVPWM ivpwm, int maxRow, int maxColumn) 
    {
        ...
        pt.utl.ist.elab.client.webrobot.customizer.Models.ModelIVPWM modelIVPWM=(pt.utl.ist.elab.client.webrobot.customizer.Models.ModelIVPWM)ivpwm.getDataModel();        
        try
        {
            if(file!=null)
            {
            ...
            for(int iCol=1;iCol<maxAllowedColumn+1;iCol++)
            {
                ...
                for(int iRow=2;iRow<maxRow+1;iRow++)
                {
                    ...
                    if(comp==null||comp.getClass().getName().startsWith("pt.utl.ist.elab.client.webrobot.customizer.Comps.HorLine"))
                    {
                        continue;
                    }
                    write("coluna="+iCol+
                                 ", nivel="+iRow+
                                 ", tipo="+comp.getTipo()+
                                 ", d1="+comp.getDataModel().getD1()+
                                 " ,d2="+comp.getDataModel().getD2()+
                                 " ,d3="+comp.getDataModel().getD3()+
                                 ", valor="+comp.getDataModel().getValor()+
                                 ", valor2="+comp.getDataModel().getValor2()+
                                 ", flag="+comp.getDataModel().getFlag()
                          );
                    ...
                    {
                        /**Only start counting horlines, after it finds a component
                         *different from an Horline between two HorLines
                         */
                        if(matrix[iRow][cCol-1]!=null&&
                            matrix[iRow][cCol-1].getClass().getName().startsWith("pt.utl.ist.elab.client.webrobot.customizer.Comps.HorLine")&&
                            isFirstTime)
                        {
                        }
                        else
            ...
            write(confInOut.getModel().getA1()+" "+confInOut.getModel().getA2()+" "+
                                confInOut.getModel().getA3()+" "+confInOut.getModel().getA4()+" 0"+
                                lineSeparator
                             );
            ...
            for(int iRow=0;iRow<iValues.length;iRow++)
            {
                ...
                write(convertSpaces(firstSize)+String.valueOf((Integer)iValues[iRow][1])+"\t"+
                        convertSpaces(secondSize)+String.valueOf((Integer)iValues[iRow][2])+
                        lineSeparator);
                        
            }
            write(modelIVPWM.getI0Value()+" "+modelIVPWM.getI1Value()+" "+
                                modelIVPWM.getI2Value()+" "+modelIVPWM.getI3Value()+" "+
                                modelIVPWM.getI4Value()+" "+modelIVPWM.getI5Value()+" "+
                                modelIVPWM.getI6Value()+" "+modelIVPWM.getI7Value()+lineSeparator
                             );
            write(modelIVPWM.getI0State()+" "+modelIVPWM.getI1State()+" "+
                                modelIVPWM.getI2State()+" "+modelIVPWM.getI3State()+" "+
                                modelIVPWM.getI4State()+" "+modelIVPWM.getI5State()+" "+
