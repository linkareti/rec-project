...
public class ObjectoCeleste
{
    ...
    public ObjectoCeleste(String _nomeObjecto, String _nome ,String _comentario, int _ar_h, double _ar_m, int _dec_g ,double _dec_m, double _dt)
    {
    ...
    public static boolean visivel(int ar_h,double ar_m, int dec_g, double dec_m)
    {
         ...
         /*Local de observa��o
         Lisboa PORTUGAL  Latitude: 38.7� Longitude -9.2�
          */
        ...
        if(dec_g<0)dec=degToRad(-(Math.abs(dec_g)+dec_m/60));
        else dec=degToRad(dec_g +dec_m/60);
       ...
       /* while(ha<0){
        ...
        if(Math.sin(ha)<0)azim=a;
        else azim= 2*Math.PI -a;
        
        /*Excluir os objetos a baixo de 10� e perto do meridiano local (5�)*/
        if((alt <10) || (Math.abs(azim)<0.1)) return true;
        else return true;
    }
}