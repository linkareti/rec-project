...
{    
	...
	String ch_nameX=expDataModel.getChannelConfig(getChannelDisplayX()).getChannelName();
        ...
        if(getChannelDisplayYArray().length==0)
        {
            multiplierY=expDataModel.getChannelConfig(getChannelDisplayY()).getSelectedScale().getMultiplier().toString();
            ph_unit_symbolY=expDataModel.getChannelConfig(getChannelDisplayY()).getSelectedScale().getPhysicsUnitSymbol();
