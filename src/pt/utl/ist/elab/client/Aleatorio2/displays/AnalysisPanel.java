 ...
 * AnalysisPanel.java
 *
 * Created on 18 de Junho de 2003, 13:33
 */
...
public class AnalysisPanel extends javax.swing.JPanel implements java.lang.Cloneable {
    ...
    private int houghCenterCounter;  //stores the count of the number of dots detected
    private int hough_cluster_counter;
    private int convolutionCenterCounter;  //stores the count of the number of dots detected
    private int conv_cluster_counter;
    private int center_counter;  //stores the full ammount of centers found with both algorithms
    ...
    /** Creates a new instance of AnalysisPanel */
    public AnalysisPanel()
    {
    }//AnalysisPanel()
    
    public AnalysisPanel(java.awt.Image image)
    {
    ...
    public java.awt.Image houghCount()
    {
                    ...
                    for(int indexa = 0; indexa <= houghCenterCounter; indexa++) {
                        if (Math.abs(houghCircles[indexa][0] - x) < Math.ceil((double)radius/2.) && Math.abs(houghCircles[indexa][1] - y) < Math.ceil((double)radius/2.)) {
                            if (houghInfo[indexa] <= media && (BWPixels[index] & 0x000000ff) != 255) {
                                //System.out.println("Overwriting " + circulos[indexa][0] + ";" + circulos[indexa][1] + " m:" + houghInfo[indexa]  + " with " + x + ";" + y + " m:" + media);
        ...
        java.awt.image.MemoryImageSource mis = new java.awt.image.MemoryImageSource(imageWidth,imageHeight,houghCountPixels,0,imageWidth);
    ...
    public java.awt.Image convolutionTransform()
    {
        ...
        int maskWidth = (radius-1)*2, maskHeight = (radius-1)*2, maskArea = maskWidth * maskHeight;
        
        boolean[] mask = new boolean[maskArea];
        int x, y;
        double theta;
        ...
        for (int i=0; i< maskArea; i++)
        {
            x = i % maskWidth;
            y = i / maskWidth;
            if (y < (radius-1) + java.lang.Math.sqrt((radius-1)*(radius-1) - (x-(radius-1))*(x-(radius-1))) && y > (radius-1) - java.lang.Math.sqrt((radius-1)*(radius-1) - (x-(radius-1))*(x-(radius-1))))
            {
                mask[i] = false;
            }//if
            else
        ...
        */
        ...
        java.awt.image.PixelGrabber pg = new java.awt.image.PixelGrabber(BWImage, 0, 0, imageWidth, imageHeight, inPixels, 0, imageWidth);
        try {pg.grabPixels();}
        catch(InterruptedException e)
        {
            e.printStackTrace();
            ...
            //if(pixelValue > 0)
                for (int maskIndex = 0; maskIndex < maskArea; maskIndex++)                
                    try {//convResult += mask[maskIndex] * (inPixels[imageIndex + (maskIndex / maskWidth * imageWidth) + maskIndex % maskWidth] & 0x000000ff);
                        if (mask[maskIndex] && 0<(inPixels[imageX+maskIndex%maskWidth + (imageY+maskIndex/maskWidth)*imageWidth] & 0x000000ff)
                            || !mask[maskIndex] && 0 == (inPixels[imageX+maskIndex%maskWidth + (imageY+maskIndex/maskWidth)*imageWidth] & 0x000000ff) )
                        convResult+=255;}
                    catch(ArrayIndexOutOfBoundsException e){}
    ...
    public java.awt.Image convolutionCount()
    {
        ...
        java.awt.image.PixelGrabber pg = new java.awt.image.PixelGrabber(convolutionImage, 0, 0, imageWidth, imageHeight, convolutionPixels, 0, imageWidth);
        try {
            pg.grabPixels();
        }catch(InterruptedException e) {
            e.printStackTrace();
        ...
        java.awt.image.PixelGrabber pg1 = new java.awt.image.PixelGrabber(originalImage, 0, 0, imageWidth, imageHeight, convolutionCountPixels, 0, imageWidth);
        try {
            pg1.grabPixels();
        }catch(InterruptedException e) {
            e.printStackTrace();
        ...
        for (int imageIndex = 0; imageIndex < area; imageIndex++) {
            int valor = convolutionPixels[imageIndex] & 0x000000ff;
            if(valor > convolutionThreshold) {
                ...
                for(int indexa = 0; indexa < convolutionCenterCounter; indexa++) {
                    if (Math.abs(convolutionCircles[indexa][0] - x) < radius && Math.abs(convolutionCircles[indexa][1] - y) < radius) {
        ...
        convolutionCountImage = buildImage(convolutionCircles, convolutionCenterCounter);
        /*
        int indice;
        for(int index = 0; index < convolutionCenterCounter; index++) {
        ...
        //Builds the final image
        java.awt.image.MemoryImageSource mis = new java.awt.image.MemoryImageSource(imageWidth,imageHeight,convolutionCountPixels,0,imageWidth);
    ...
    public java.awt.Image refineCount(String type)
    {
        int x, y;
        int[][] refineCenters= null;
        
        if (type == this.IMAGE_CONVOLUTION)
        {
        ...
        for(int clusterIndex = 0; clusterIndex < cluster_counter; clusterIndex++) {
            ...
            for(int dieIndex = 0; dieIndex < vDie[clusterIndex].size()-1; dieIndex++) {
                ...
                indice = refineCenters[((Integer)vDie[clusterIndex].get(dieIndex)).intValue()][1] * imageWidth + refineCenters[((Integer)vDie[clusterIndex].get(dieIndex)).intValue()][0];
    ...
    private int countClusters(int[][] circles)
    {
        //Counting the number of Dice by clustering
        int currentX, currentY;        
        int x, y;
     ...
     *spot Checking
     */
    //checks the pixels of the B&W image and the corners of squares of side 13, 9 & 5, must be white (in either) and the center black, to be accepted as a spot
