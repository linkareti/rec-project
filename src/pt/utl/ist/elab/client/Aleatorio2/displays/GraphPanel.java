...
public class GraphPanel extends JPanel{
    ...
    private int imageHeight = 140;  //maximum height of column = 100 pixels; the rest is for the axes
    ...
    public void updateGraph(double[] x, double[] y, double mu, double sigma, double y0, double Amp)
    {
        ...
        drawData();
        drawFunction();
    }//updateGraph(double[] x, double[] y, double mu, double sigma, double y0, double Amp)
    ...
    private void drawAxes()
    {
        ...
        for (int index = (bottomMargin + extraMargin); index < imageHeight-(topMargin-extraMargin); index+=2*extraMargin)
        {
            ...
            g.drawString(String.valueOf(absValue),imageWidth-(rightMargin - extraMargin),imageHeight-index+fontSize/2);
        }
        ...
        //drawing the X axes
        g.drawLine(leftMargin - extraMargin,imageHeight-(bottomMargin + extraMargin),imageWidth-(rightMargin - extraMargin),imageHeight-(bottomMargin + extraMargin));
        
        for (int index = leftMargin ; index < imageWidth-rightMargin; index+=columnWidth)
        {
            if(x[(index-leftMargin )/columnWidth] % 5 == 0)     //only draws string every 5 columns
                g.drawString(String.valueOf(x[(index-leftMargin )/columnWidth]),index,imageHeight-leftMargin );
            //else g.drawString(String.valueOf(x[(index-10)/4]),index,imageHeight-2);
        }
    ...
    /**
     *Draws rectangles on the graph.
     *These rectangles' height is related to the y[] values
     */
    private void drawData()
    {
        ...
        for (int index = 0; index < x.length; index++)
        {
            ...
            g.drawRect(index*columnWidth+leftMargin,imageHeight-(bottomMargin+extraMargin)-altura,columnWidth,altura);
        }
    }//drawData
    
    /**
     *Draws the Gaussian Function on the graph with the previously fitted parameters
     */
    ...
    private void drawFunction()
    {
        ...
        for(int index = leftMargin; index < imageWidth-rightMargin; index++)
        {
            ...
            double yValue = pt.utl.ist.elab.client.Aleatorio2.utils.LMFit.gaussFunc(xValue,params[0],params[1],params[2],params[3]);
            int yPoint = (int)Math.round((yValue*(-(double)graphHeight))/maxY) + imageHeight-(bottomMargin+extraMargin);
