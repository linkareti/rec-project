...
public class DPendulumDataProducer extends BaseDataSource
{
    ...
    public DPendulumDataProducer(DPendulumDriver dpd, float ini_theta1_deg, float ini_theta2_deg, float ini_omega1, float ini_omega2, float length1, float length2, float mass1, float mass2, int tbs, int nSamples) 
    {
        this.dpd = dpd;
    ...
    private class ProducerThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while(cSample < nSamples && running)
            {
                ...
                value[0] = new PhysicsValue(PhysicsValFactory.fromFloat(x),
                ...
                value[1] = new PhysicsValue(PhysicsValFactory.fromFloat(theta1),
                ...
                value[2] = new PhysicsValue(PhysicsValFactory.fromFloat(theta2),
                ...
                value[3] = new PhysicsValue(PhysicsValFactory.fromFloat(omega1),
                ...
                value[4] = new PhysicsValue(PhysicsValFactory.fromFloat(omega2),
    ...
    private float[] derivs(float x, float[] y, int n)
    {
        ...
        dydx[1] = (float)((mass2 * length1 * y[1] * y[1] * Math.sin(del) * Math.cos(del) + mass2 * G * Math.sin(y[2]) * Math.cos(del)
        + mass2 * length2 * y[3] * y[3] * Math.sin(del) - (mass1 + mass2) * G * Math.sin(y[0])) / den1);

        dydx[2] = y[3];

        den2 = (float)((length2/length1) * den1);
        dydx[3] = (float)((-mass2 * length2 * y[3] *  y[3] * Math.sin(del) * Math.cos(del) + (mass1 + mass2) * G * Math.sin(y[0]) 
        * Math.cos(del) - (mass1 + mass2) * length1 * y[1] * y[1] * Math.sin(del) - (mass1 + mass2) * G * Math.sin(y[2]))/den2);
