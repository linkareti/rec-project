...
public class StampFinder
{
    ...
    public void startSearch()
    {
        ...
        Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO, "Are there COMM Ports on the System? " + commPortIdentifiers.hasMoreElements());
        ...
        Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO, "There are " + portsObj.length +" Serial COMM Ports in the system...");
    ...
    private class StampFinderRunner extends Thread
    {
        ...
        public void cyclePort()
        {
            ...
            {
                Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Serial Port " + cpi.getName() + " is currently owned by another application - " + cpi.getCurrentOwner());
                currentPort++;
                return;
            }
            
            Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Serial Port " + cpi.getName() + " is not Owned - Opening & Locking It! ");
            ...
            try
            {
                Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Sleeping for "+(timeOutPerPort>0?timeOutPerPort:100)+ " ms for port "+ cpi.getName()+  " data events");
                synchronized(this)
                {
                    wait(timeOutPerPort>0?timeOutPerPort:100);
                    Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Ended Sleeping for "+(timeOutPerPort>0?timeOutPerPort:100)+ " ms for port "+ cpi.getName()+  " data events");
                }
                
            }catch(InterruptedException e)
            {
                LoggerUtil.logThrowable("Error waiting for SerialPort identification!",e,Logger.getLogger(STAMP_FINDER_LOGGER));
                return;
            }
            ...
            if(!portFound)
            {
                ...
                {
                    Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"NO id string detected on port "+cpi.getName());
                    ...
                    Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Shuted down stampIO!");
                                ...
                                /*try
                                {
                                        Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Closing port "+cpi.getName()+" IO Stream!");
                                        currentReader.close();
                                }catch(Exception e)
                                {
                                        LoggerUtil.logThrowable("Error closing port "+cpi.getName() +" IO Stream!",e,Logger.getLogger(STAMP_FINDER_LOGGER));
                                }
                                try
                                {
                                        Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Closing port "+cpi.getName()+"!");
                                        currentPortOpen.close();
                                }catch(Exception e)
                                {
                                        LoggerUtil.logThrowable("Error closing port "+cpi.getName()+"!",e,Logger.getLogger(STAMP_FINDER_LOGGER));
                                }*/
            }
        ...
        private class StampFinderRunnerPortListener implements StampCommandListener
        {
                        ...
                        /*public synchronized void serialEvent(gnu.io.SerialPortEvent serialPortEvent)
                        {
                            Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Received port event "+currentPortOpen.getName()+" whith type="+serialPortEvent.getEventType()+"  - DataAvailableEventType="+SerialPortEvent.DATA_AVAILABLE);
                                ...
                                if(serialPortEvent.getEventType()==SerialPortEvent.DATA_AVAILABLE)
                                {
                                        ...
                                        try
                                        {
                                                Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Reading line of data from port "+currentPortOpen.getName()+"!");
                                                line=currentReader.readLine();
                                                Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Data Line is : '"+line+"'!");
                                        }catch(IOException e)
                                        {
                                                LoggerUtil.logThrowable("IO Error reading line from port "+currentPortOpen.getName()+"!",e,Logger.getLogger(STAMP_FINDER_LOGGER));
                                        }
                                        if(line.equals(stampIdentifier))
                                        {
                                            Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Identified STAMP on port "+currentPortOpen.getName()+"!");
                                            //currentPortOpen.removeEventListener();
            ...
            {
                Logger.getLogger(STAMP_FINDER_LOGGER).log(Level.INFO,"Received a command "+command.getCommandIdentifier()+"!");
