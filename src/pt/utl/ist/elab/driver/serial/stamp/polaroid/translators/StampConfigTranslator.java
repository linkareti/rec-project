 ...
 */
package pt.utl.ist.elab.driver.serial.stamp.polaroid.translators;
...
public class StampConfigTranslator extends AbstractStampTranslator {
    ...
    public boolean translate(StampCommand command) {
        ...
        int posini = (int) (((float) ((Integer) command.getCommandData(POS_INI_POL_STR)).intValue()) / 0.1535);
        int posend = (int) (((float) ((Integer) command.getCommandData(POS_END_POL_STR)).intValue()) / 0.1535);
        int posfixo = (int) (((float) ((Integer) command.getCommandData(POS_FIXO_STR)).intValue()) / 0.1535);
        ...
        String commandStr = command.getCommandIdentifier() + " " + calib + " " + luzpol+ " " + posfixostr + " " +  posinistr + " " + posendstr ;
