...
public abstract class AbstractStampDriver extends BaseDriver implements
	...
	protected void loadCommandHandlers() {
		StampTranslatorProcessorManager
				.initStampProcessorTranslator("pt.utl.ist.elab.driver.serial.stamp.transproc.processors.StampStartProcessor");
		StampTranslatorProcessorManager
				.initStampProcessorTranslator("pt.utl.ist.elab.driver.serial.stamp.transproc.processors.StampConfiguredProcessor");
		StampTranslatorProcessorManager
				.initStampProcessorTranslator("pt.utl.ist.elab.driver.serial.stamp.transproc.processors.StampNotConfiguredProcessor");
		StampTranslatorProcessorManager
				.initStampProcessorTranslator("pt.utl.ist.elab.driver.serial.stamp.transproc.processors.StampIdProcessor");
	...
	public void extraValidateConfig(HardwareAcquisitionConfig config,
			HardwareInfo info) throws WrongConfigurationException {
	...
	public void init(HardwareInfo info) {
		...
		try {
			...
			LoggerUtil.logThrowable("Couldn't find port for STAMP in " + 120
					+ "s", e, Logger.getLogger(STAMP_DRIVER_LOGGER));
	...
	public IDataSource start(HardwareInfo info) throws IncorrectStateException {
		...
		if (dataSource != null) {
			try {
				...
				LoggerUtil
						.logThrowable(
								"Error on start... - rethrowing IncorrectStateException!",
								e, Logger.getLogger(STAMP_DRIVER_LOGGER));
	...
	public IDataSource startOutput(HardwareInfo info, IDataSource source)
	...
	public void stampFound(SerialPort sPort) {
		synchronized (stampFinder) {
			...
			stampIO.setApplicationNameLockPort(stampFinder
					.getApplicationNameLockPort());
	...
	public void handleStampCommand(StampCommand command) {
		...
		StampProcessor processor = command.getProcessor();
		 ...
		 Logger.getLogger(STAMP_DRIVER_LOGGER).log(Level.INFO, processor == null ? "null" : processor.getClass().getName());
		
		if (processor == null) {
			Logger.getLogger(STAMP_DRIVER_LOGGER).log(
					Level.INFO,
					"Didn't get a processor for command "
