...
public class StatSoundStampDriver extends AbstractStampDriver
{
	...
	stampConfig.addCommandData(StampConfigTranslator.STATUS_STR,
            new String(
                config.getSelectedHardwareParameterValue(StampConfigTranslator.STATUS_STR)
            )	
	);

        stampConfig.addCommandData(StampConfigTranslator.RESET_STR,
            new Integer(
                config.getSelectedHardwareParameterValue(StampConfigTranslator.RESET_STR)                
            )
	);		
	...
	if(!translator.translate(stampConfig))
        ...
        {
            config.setTotalSamples(config.getTotalSamples() + 2); //+2 for temp ini and temp final
        }
