...
//				th.stopWave();
			...
			/** FREQ EXPERIMENT */
				...
				for (int i = 0; i < Math.round(nPoints); i++) {
					...
					values[1] = PhysicsValueFactory.fromDouble(freqIni, config.getChannelsConfig(1).getSelectedScale());
					values[2] = PhysicsValueFactory.fromDouble( (short) (acqByte[i*4+1] << 8 | (255 & acqByte[i*4])) /*acqByte[i*4] + (acqByte[i*4+1])*255*/, config.getChannelsConfig(3).getSelectedScale());
