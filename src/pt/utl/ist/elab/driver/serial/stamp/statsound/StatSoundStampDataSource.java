...
//				th.stopWave();
			...
			/** FREQ EXPERIMENT */
				...
				for (int i = 0; i < Math.round(nPoints); i++) {
					...
					//if (i % 4 == 0 || i % 3 == 0) {
						values[2] = PhysicsValueFactory.fromDouble( (short) (acqByte[i*4+1] << 8 | (255 & acqByte[i*4])  / 1000) /*acqByte[i*4] + (acqByte[i*4+1])*255*/, config.getChannelsConfig(2).getSelectedScale());
