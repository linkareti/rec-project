...
//				th.stopWave();
			...
			/** FREQ EXPERIMENT */
				...
				for (int i = 0; i < Math.round(nPoints); i++) {
					...
					values[1] = PhysicsValueFactory.fromDouble(freqIni, config.getChannelsConfig(1).getSelectedScale());
					values[2] = PhysicsValueFactory.fromDouble( ((double) (acqByte[i*4+1] << 8 | (255 & acqByte[i*4])) / 1000.0   ) /*acqByte[i*4] + (acqByte[i*4+1])*255*/, config.getChannelsConfig(2).getSelectedScale());
