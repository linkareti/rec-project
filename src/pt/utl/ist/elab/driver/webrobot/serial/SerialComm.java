...
public class SerialComm implements gnu.io.SerialPortEventListener
{
    ...
    /**If the robot doesn't respond at the end of n periods launch security procedures
     */
    ...
    {       
        ...
        /**If securityPeriods==2 ...then something went wrong...Wrote 2 times without
         an answer from the robot ...Launching security procedure...
        ...
        if(serialPortEvent.getEventType()==gnu.io.SerialPortEvent.DATA_AVAILABLE)
	{
            ...
            {                
                waitingSecondPacket=true;
                firstNBytes=nBytes-2;
                if(nBytes>readBuffer.length)
                {
                    nBytes=readBuffer.length;
                }
                for(int i=2;i<nBytes;i++)
                {
                    sensorsState[i-2]=(int)readBuffer[i];
                }                
            }
            else if(waitingSecondPacket)
            {
                for(int i=0;i<(sensorsState.length-firstNBytes);i++)
                {
                    sensorsState[i+firstNBytes]=(int)readBuffer[i];
                }                                
                //sensorsState[sensorsState.length-1]=(int)readBuffer[0];
                waitingSecondPacket=false;
                setWriteMode();   
                securityPeriods=0;
