...
public class WebCamThread implements Runnable
{
    ...
    public WebCamThread()
    {
        ...
        processorDataSource = videoProcessor.getDataOutput();
        //System.out.println("DataSource:"+videoDataSource);
        
        
        System.out.println(">>> creating the videoPlayer");
        try
        {
            videoPlayer = Manager.createRealizedPlayer(videoDataSource); //or mediaLocator //or  'videoDataSource'  //or URL
            //videoPlayer = Manager.createRealizedPlayer(videoDataSource);
        }
        catch(IOException e)
        {System.out.println("Can't create Player. Check Webcam!"); System.exit(1);}
        ...
        //videoPlayer.start();
        //while (videoPlayer.getState() != videoPlayer.Started){} //wait for player to start!*/
    }
     ...
     */
    /**
     * public WebCamThread(String fileName)
     * {
     ...
     *
     * // setup output file format  ->> msvideo
     * FileTypeDescriptor outputType = new FileTypeDescriptor(FileTypeDescriptor.MSVIDEO);//isto � o INDEO5
     *
     * // setup output video and audio data format
     * Format outputFormat[] = new Format[1];
     * outputFormat[0] = new VideoFormat(VideoFormat.INDEO50);
     ...
     * ProcessorModel processorModel = new ProcessorModel(videoDataSource, outputFormat, outputType);
     * Processor videoProcessor = null;
     * try {videoProcessor = Manager.createRealizedProcessor(processorModel);}
     * catch (IOException e)
     * {
     * System.out.println(e);
     * System.exit(0);
     ...
     * //videoPlayer = Manager.createRealizedPlayer(videoDataSource);
     * }
     * catch(IOException e) {System.out.println("Can't create Player. Check Webcam!"); System.exit(1);}
     * catch(NoPlayerException e) {e.printStackTrace(); System.exit(1);}
     * catch(CannotRealizeException e) {System.out.println(e.toString()); System.exit(1);}
     * videoPlayer.start();
     * while (videoPlayer.getState() != videoPlayer.Started){} //wait for player to start!
     *
     *
     * //commented so no file is recorded
     * if (movieFileName == null)
     * movieFileName = "//pt//utl//ist//elab//Aleatorio//movie0.avi";
     *
     * // Generate the output media locator.
     * MediaLocator outputMediaLocator = new MediaLocator(movieFileName);  //� aqui, penso eu, que se diz que o output vai para o file
     *
     * try {outputDataSink = Manager.createDataSink(processorDataSource,outputMediaLocator);}
     * catch(NoDataSinkException e){}
     * try {outputDataSink.open();}
     * catch(IOException e){System.out.println("Cannot Write in " + movieFileName);}
     *
     * }
     */
    /**
    ...
    public Image getImage()
    {
        ...
        }else
        {
            ...
            java.awt.image.PixelGrabber pg = new java.awt.image.PixelGrabber(imagemCapturada, 0, 0, 640, 480, inPixels, 0, 640);
            try {
                pg.grabPixels();
    ...
    /**
     *Runs the thread.
     *This is where the actual recording is done!
     */
    public void run()
    { //man, this doesn't do anything but wait for milisecs
        recording = true;   //It's amazing how I say this is where the actual recording is done!!!
        ...
        synchronized(waiting.synchStop)
        {
            synchronized(waiting.synchStart)
            {
                waiting.start();
                try
                {waiting.synchStart.wait();} catch(Exception e)
                {}
                
            }
            try
            {waiting.synchStop.wait();} catch(Exception e)
            {}
        }
    ...
    class Waiting extends Thread
    {
        ...
        public void run()
        {
            synchronized(synchStart)
            {
                ...
                try
                {Thread.currentThread().sleep(milisecs);}
                catch(InterruptedException e)
                {}
            }
