...
public class Cargas3DDataProducer extends VirtualBaseDataSource {
    ...
    //Este � o processo que nos vai simular e criar as amostras para enviar ao cliente!
    private class ProducerThread extends Thread {
            ...
            for(x=0f;x<10;x=x+0.1f){
                for(y=0f;y<10;y=y+0.1f){
                    for(z=0f;z<10;z=z+0.1f){
                        ...
                        //limites na caixa
                        boolean minmax=true;
                        for(int i=0;i<sistema.size() && minmax;i++){ //se for uma fez falso, seja para sempre
                            ...
                            if((x<carga.getX()+0.5 && x>carga.getX()-0.5) ||
                                    ...
                                    (z<carga.getZ()+0.5 && z>carga.getZ()-0.5) ||
                                    z>8 || z<3 || y>8 || y<3 || x>8 || x<3)
                                minmax=false;
                            else minmax=true;
                        }
                        if(minmax==true){
            ...
            for(double k = 0; k<= r; k+=r/densidade ){
                for(double j = 0; j<= r; j+=r/densidade ){
                    for(double i = 0; i<= r; i+=r/densidade ){
                        ...
                        y = (y1 + j)/(norma)*r;
                        z = (z1 + k)/(norma)*r;
                        //calcula para cada ponto da superficies esferica, para cada esfera
                        linhas.add(
                                calculaLinha(carga.getX()+x,carga.getY()+y,carga.getZ()+z,
                                carga.getCharge())
                                );
                    }
                }
        ...
        public void run() {
            ...
            PhysicsVal val = PhysicsValFactory.fromByteArray(ByteUtil.getObjectAsByteArray(calculaLinhas()), "data/raw");
            ...
            addDataRow(value);
            try{ join(100); } catch(InterruptedException ie){}
            endProduction();
...
//        pt.utl.ist.elab.virtual.client.cargas3d.Sistema.novaCarga(5f, 8f, 5f, -25f);
//        pt.utl.ist.elab.virtual.client.cargas3d.Sistema.novaCarga(5f, 2f, 5f, 25f);
//        pt.utl.ist.elab.virtual.client.cargas3d.Sistema.novaCarga(1f, 2f, 3f, 10f);
