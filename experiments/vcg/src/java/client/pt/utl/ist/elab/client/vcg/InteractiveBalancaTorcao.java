...
public class InteractiveBalancaTorcao extends BalancaTorcao implements InteractionListener, ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5863548252713572036L;
	private final PopupMenu mmPopMenu;
	...
	public InteractiveBalancaTorcao() {
		...
		final Group mms = new Group();
		mm[0].setGroup(mms);
		mm[1].setGroup(mms);

		mm[0].setEnabled(InteractiveElement.TARGET_ROTATION, true);
		mm[0].setRotationAxis(new javax.vecmath.Vector3d(0, 0, 1));
		mm[0].setRotationPoint(new javax.vecmath.Vector3d(0, 0, 0));

		mm[1].setEnabled(InteractiveElement.TARGET_ROTATION, true);
		mm[1].setRotationAxis(new javax.vecmath.Vector3d(0, 0, 1));
		mm[1].setRotationPoint(new javax.vecmath.Vector3d(0, 0, 0));

		regua.setEnabled(InteractiveElement.TARGET_POSITION, true);

		setListener(this);
		mmPopMenu = new PopupMenu(this);
		mmPopMenu.addItem(ReCResourceBundle.findStringOrDefault("rec.exp.customizer.balanca.popup.title.1", "Edit"),
	...
	public void interactionPerformed(final InteractionEvent _event) {
		...
		if (-mM[0].getY() != mM[1].getY() && eventObj.getClass().toString().indexOf("InteractiveSphere") != -1) {
			final InteractiveSphere esf = (InteractiveSphere) eventObj;
	...
	public void setMasses(final double[] m) {
		mm[0].setMass(m[0]);
		mm[1].setMass(m[1]);
		mM[0].setMass(m[2]);
		mM[1].setMass(m[3]);
	...
	public void actionPerformed(final ActionEvent e) {
		if (e.getActionCommand().equalsIgnoreCase("Edit")) {
			if (eventObj.getClass().toString().indexOf("InteractivePlane") != -1) {
				regua.setSizeX(PopupMenu.dialog(ReCResourceBundle.findStringOrDefault(
						"rec.exp.customizer.balanca.title.15", "Edit Target Ruler (cm)"), ReCResourceBundle
						.findStringOrDefault("rec.exp.customizer.balanca.tip.15", "Ruler length"), ReCResourceBundle
						.findStringOrDefault("rec.exp.customizer.balanca.tip.16", "Accept the configuration"),
						(int) Math.round(regua.getSizeX()), new int[] { 100, 500, 50, 50 }));
			} else {
				((InteractiveSphereMass) eventObj).setMass(PopupMenu.dialog(ReCResourceBundle.findStringOrDefault(
						"rec.exp.customizer.balanca.title.17", "Edit Mass (g)"), ReCResourceBundle.findStringOrDefault(
						"rec.exp.customizer.balanca.tip.17", "Mass of the selected sphere"), ReCResourceBundle
						.findStringOrDefault("rec.exp.customizer.balanca.tip.16", "Accept the configuration"),
						(int) Math.round(((InteractiveSphereMass) eventObj).getMass() * 1e3), new int[] { 400, 1500,
								15, 200 }) * 1e-3);
			}
			repaint();
			updateGUI();
