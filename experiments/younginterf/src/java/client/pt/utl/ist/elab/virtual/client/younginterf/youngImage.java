 ...
 * youngImage.java
 *
 * Created on 25 de Março de 2005, 18:40
 */
...
public class youngImage extends JComponent{
    ...
    static  double largura = 1080; //number of lines to be painted, equal to the width of the spectrum
    ...
    static BufferedImage imagemBuffer = new BufferedImage((int)(largura + 2*border), (int)(altura), BufferedImage.TYPE_INT_RGB);
    ...
    public static javax.swing.ImageIcon createIcon(){
        ...
        imagemBuffer.getHeight(), Desenhar(), 0, imagemBuffer.getWidth())));
        return icon;    }
    
    public void paint(Graphics g){
        imagemBuffer.getGraphics().drawImage(java.awt.Toolkit.getDefaultToolkit().createImage(new java.awt.image.MemoryImageSource(imagemBuffer.getWidth(),
        imagemBuffer.getHeight(), Desenhar(), 0, imagemBuffer.getWidth())),0,0,null);
    ...
    public static int[] Desenhar(){
        ...
        BufferedImage imgBuf =  new BufferedImage((int)(largura + 2*border), (int)(altura), BufferedImage.TYPE_INT_RGB);
        ...
        do{//start do cicle
            ...
            while( xReal >= 0 && passa){
                if(xReal >= -dx && xReal <= dx){
                    tag = 1;
                    //                System.out.println("1 X Real é : "+ xReal);
                }
                if( tag == 1  && tag_ ==1 && (gamaAnterior > gama) ){
                    deltaX = xReal-(dx/2);
                    tag = 0;
                    //             System.out.println("2 X Real é : "+ xReal);
            ...
            // g.drawLine(xVirtual+border,100- (int)(gama*100), (int)( largura*(xReal+dx)/larguraReal + largura/2 )+border,100- (int)((CalculateGama((xReal+dx), wl, abertura, distanciaPlano))*100) );
            g.drawLine(xVirtual+border,149- roundToInt(gama*100),  roundToInt( largura*(xReal+dx)/larguraReal + largura/2 )+border,149-  roundToInt((CalculateGama((xReal+dx), wl, abertura, distanciaPlano))*100) );
        ...
        g.setFont(big);
        // String title = "\u03BB = "+wl+" nm                       " + s1 +" =  " +(float)deltaX + " \u00b1 "+ (float)dx +" mm"+"  pre-calculado: " + preCalc ;
        String title = "\u03BB = "+wl+" nm                                                                                                                " + s1 +" =  " +(float)deltaX + " \u00b1 "+ (float)dx +" mm";
        ...
        for (int conta = 0 ; conta <= (int)largura/2 ; conta = conta + roundToInt(largura/larguraReal) ) {
            ...
            g.drawLine( border + (int)(largura/2) - conta, 90+50, border + (int)(largura/2) - conta, 100+50);
            //g.drawString(s,border + (int)(largura/2) +1+ conta-(littlefm.stringWidth(s)/2) ,90+74);
            
            g.drawString(s,border + (int)(largura/2) +1+ conta-(24/2) ,90+74);
            //if(i!=0)g.drawString(s_,border + (int)(largura/2) -1- conta-(littlefm.stringWidth(s)/2) ,90+74);
     ...
     *  do eixo contido no plano de projecçao e com centro no maximo principal
     */
