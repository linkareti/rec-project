...
public class Animation extends JPanel implements ExpDataDisplay, ExpDataModelListener {
    ...
    private PlottingPanel panel = new PlottingPanel("X (m)","Y (m)",
    ReCResourceBundle.findStringOrDefault("ReCExpColisao$rec.exp.customizer.title.graph" ,"Trajectory") );
    ...
    public static void main(String args[]) {
        ReCResourceBundle.loadResourceBundle("ReCExpColisao", "recresource:///pt/utl/ist/elab/virtual/client//resources/ReCExpColisao");
    ...
    public void newSamples(NewExpDataEvent evt) {
        ...
        for(int i=evt.getSamplesStartIndex(); i<=evt.getSamplesEndIndex(); i++) {
            ...
            if(model.getValueAt(i,1) != null) {
                circulos[0].setXY(model.getValueAt(i,1).getValue().getFloatValue()
                ,model.getValueAt(i,2).getValue().getFloatValue());
                circulos[1].setXY(model.getValueAt(i,3).getValue().getFloatValue()
                ,model.getValueAt(i,4).getValue().getFloatValue());
                
                vects[0].setXY(model.getValueAt(i,1).getValue().getFloatValue()
                ,model.getValueAt(i,2).getValue().getFloatValue());
                vects[1].setXY(model.getValueAt(i,3).getValue().getFloatValue()
                ,model.getValueAt(i,4).getValue().getFloatValue());
                ...
                dataset.append(0, model.getValueAt(i,1).getValue().getFloatValue()
                ,model.getValueAt(i,2).getValue().getFloatValue());
                dataset.append(1, model.getValueAt(i,3).getValue().getFloatValue()
                ,model.getValueAt(i,4).getValue().getFloatValue());
    ...
    public void dataModelStartedNoData() {
        ...
        circulos[1] = DrawableShape.createCircle(xf - v2*(-Math.cos(Math.toRadians(a))) * dt, yf - v2*(Math.sin(Math.toRadians(a))) * dt, 2*r2);
        circulos[1].setMarkerColor((new java.awt.Color(0,0,255,90)), (new java.awt.Color(0,0,255)));
        vects[0] = new Arrow(-v1*dt, 0,v1,0);
        vects[1] = new Arrow(xf - v2*(-Math.cos(Math.toRadians(a))) * dt, yf - v2*(Math.sin(Math.toRadians(a))) * dt, v2*(-Math.cos(Math.toRadians(a))),v2*(Math.sin(Math.toRadians(a))));
    ...
    /*
        double dt = 0.01;
          ...
          //  dt=tbs/1000;
            ...
            //   esferas[1].x = xf - esferas[1].vX * dt_;
            //  esferas[1].y = yf - esferas[1].vY * dt_;
     
            //esferas[0].x = - esferas[0].vX * dt;
            //esferas[0].y = - esferas[0].vY * dt;
            System.out.println("xi1 "+ (- esferas[0].vX * dt_) +" vx "+esferas[0].vX +"  xi2 "+ (xf - esferas[1].vX * dt_) +", "+ (yf - esferas[1].vY * dt_) +" v2 "+ esferas[1].vX+", "+  esferas[1].vY+"");
            ...
            double x_ = Math.max(Math.abs(esferas[1].x),Math.abs(esferas[0].x));
            double y_ = Math.max(Math.abs(esferas[1].y),Math.abs(esferas[0].y));
            //panel.setPreferredMinMax(-esferas[0].x-1, 1+esferas[1].x,  -Math.abs(esferas[1].y)-1 ,  1 + Math.abs(esferas[1].y));
            panel.setPreferredMinMax(-x_,x_, -y_,y_);
            circulos[0] = DrawableShape.createCircle(esferas[0].x, esferas[0].y, esferas[0].raio*2);
            circulos[1] = DrawableShape.createCircle(esferas[1].x, esferas[1].y, esferas[1].raio*2);
     
            circulos[1].setMarkerColor((new java.awt.Color(0,0,255,90)), (new java.awt.Color(0,0,255)));
            panel.addDrawable(data);
            panel.addDrawable(circulos[0]);
            panel.addDrawable(circulos[1]);
