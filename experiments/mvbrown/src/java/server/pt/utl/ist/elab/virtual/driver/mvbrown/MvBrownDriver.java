...
public class MvBrownDriver extends VirtualBaseDriver {
    ...
    public void configure(HardwareAcquisitionConfig config, HardwareInfo info) throws WrongConfigurationException {
        ...
        ((MvBrownDataProducer) dataSource).initializeGraphs(new String[]{config.getSelectedHardwareParameterValue("graph1"),
        config.getSelectedHardwareParameterValue("graph2"),
        config.getSelectedHardwareParameterValue("graph3"),
        config.getSelectedHardwareParameterValue("graph4")},
        new boolean[]{config.getSelectedHardwareParameterValue("graph1Med").trim().equals("1")?true:false,
        ...
        ((MvBrownDataProducer) dataSource).initializeGenPosGraph(config.getSelectedHardwareParameterValue("x").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("y").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("z").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("posModulus").trim().equals("1")?true:false,
        true,
        config.getSelectedHardwareParameterValue("posQuad").trim().equals("1")?true:false,
        false);
        
        ((MvBrownDataProducer) dataSource).initializeGenVelGraph(config.getSelectedHardwareParameterValue("vx").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("vy").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("vz").trim().equals("1")?true:false,
        config.getSelectedHardwareParameterValue("velModulus").trim().equals("1")?true:false,
        true,
        config.getSelectedHardwareParameterValue("velQuad").trim().equals("1")?true:false,
        false);
        ...
        if (config.getSelectedHardwareParameterValue("langevin").trim().equals("1")?true:false){
            ...
            if (dim == 2){
                ...
                vel = new double[]{velMod*Math.cos(velTheta)*Math.sin(velPhi), velMod*Math.sin(velTheta)*Math.sin(velPhi), velMod*Math.cos(velPhi)};
            }
            else {
                vel = new double[]{Float.parseFloat(config.getSelectedHardwareParameterValue("velMod")),0,0};
            }
            
            ((MvBrownDataProducer) dataSource).initializeLangevin(Float.parseFloat(config.getSelectedHardwareParameterValue("mass")),
            new double[]{Float.parseFloat(config.getSelectedHardwareParameterValue("radius")),Float.parseFloat(config.getSelectedHardwareParameterValue("vis"))},
            new double[]{Float.parseFloat(config.getSelectedHardwareParameterValue("dP")),Float.parseFloat(config.getSelectedHardwareParameterValue("freq"))},
            vel,false);
            
        }
        else {
            ((MvBrownDataProducer) dataSource).initializeRandomSimul(Float.parseFloat(config.getSelectedHardwareParameterValue("d")), false);
        }
    ...
    public Object getHardwareInfo() {
        ...
        try {
            ...
            try {
                ...
                LoggerUtil.logThrowable("Unable to load resource: " + baseHardwareInfoFile,e2,Logger.getLogger(MvBrown_DRIVER_LOGGER));
            }
        }
