...
public class MvBrownDataProducer extends VirtualBaseDataSource {//implements Runnable {
    ...
    public MvBrownDataProducer(VirtualBaseDriver driver, int _numPart, byte _dim, int _w, int _h, boolean _conPts, boolean _anima, int _tbs, int _nSamples) {
        this.driver = driver;
        nSamples = _nSamples;
        tbs = _tbs;
        numPart = _numPart;
    ...
    /*
     * Inicializa um grafico do tipo (x,y), com x e y quaisquer
     * que e' renderizado num JFreeChart e cuja imagem
     * e' enviada como um array de bytes.
     */
    ...
    /*
     * Inicializa um grafico do tipo (t,vx,vy,vz,|v|)
     * que e' renderizado num JFreeChart e cuja imagem
     * e' enviada como um array de bytes.
     */
    public void initializeGenVelGraph(boolean _vx, boolean _vy, boolean _vz, boolean _vMod, boolean _vMed, boolean _vQuad, boolean _vAbs){
        ...
        if (!_vMed && !_vQuad && !_vAbs){
            ...
            int k = 0;if (_vx)k++;if (_vy)k++;if (_vz)k++;if (_vMod)k++;axis[2] = new byte[k][3];
            int i = 0;
            if (_vx)
    ...
    /*
     * Initializa um grafico do tipo (t,x,y,z,|r|)
     * que e' renderizado num JFreeChart e cuja imagem
     * e' enviada como um array de bytes.
     */
    public void initializeGenPosGraph(boolean _x, boolean _y, boolean _z, boolean _rMod, boolean _rMed, boolean _rQuad, boolean _rAbs){
        ...
        if (!_rMed && !_rQuad && !_rAbs){
            ...
            int k = 0;if (_x)k++;if (_y)k++;if (_z)k++;if (_rMod)k++;axis[1] = new byte[k][3];
            int i = 0;
            if (_x)
    ...
    /*
    //TESTE
    public void start(pt.utl.ist.elab.virtual.client.mvbrown.displays.Animation an){
        this.an = an;
    ...
    //TESTE
    pt.utl.ist.elab.virtual.client.mvbrown.displays.StaticChart ch;
    public void startChart(pt.utl.ist.elab.virtual.client.mvbrown.displays.StaticChart ch){
        this.ch = ch;
    ...
    pt.utl.ist.elab.virtual.client.mvbrown.displays.Animation an;
    ...
    public void run() {
        ...
        while(currentSample < nSamples){
                    ...
                    for (int j = 0; j < axis[i].length; j++){
                        if (axis[i][j][0] > 0){
                            if (axis[i][j][2] == 0) //x ~media
                                ...
                                ;//ADDROW CANAL : (axis[i][j][0]-1), getValue(axis[i][j], currentSample, 0)
                        }
                        if (axis[i][j][1] > 0){
                            if (axis[i][j][2] == 0) //y ~media
                                data[axis[i][j][1]] = getValue(axis[i][j], currentSample, 1);
                            else
                                ;//ADDROW CANAL : (axis[i][j][1]-1), getValue(axis[i][j], currentSample, 1);
                        }
                    }
            ...
            if (datasets != null)
                for (int i = 0; i < datasets.length; i++)
                    appendValueToSerie(datasets[i],data[Math.abs(axis[0][i][0])],data[Math.abs(axis[0][i][1])]);
            //~MEDIA - preencher serie
            if (datasetGenPos != null)
                for (int i = 0; i < datasetGenPos.getSeriesCount(); i++)
                    appendValueToSerie(((XYSeriesCollection)datasetGenPos).getSeries(i),data[Math.abs(axis[1][i][0])],data[Math.abs(axis[1][i][1])]);
            //~MEDIA - preencher serie
            if (datasetGenVel != null)
                for (int i = 0; i < datasetGenVel.getSeriesCount(); i++)
                    appendValueToSerie(((XYSeriesCollection)datasetGenVel).getSeries(i),data[Math.abs(axis[2][i][0])],data[Math.abs(axis[2][i][1])]);
        ...
        if (datasets != null){
            ...
            for (int i = 0; i < graphMed.length; i++)
                if (!graphMed[i])
                    ch.makeImage(graphImageBytes(datasets[--o]));
            //ADDROW CANAL : (18+i) graphImageBytes(datasets[i]);
        }
        //ENVIAR Grafico da posicao [~Media]
        if (datasetGenPos != null)
            //ADDROW CANAL : 22 graphImageBytes(datasetGenPos);
    ...
    */
        ...
        public void run() {
            try {
                            ...
                            for (int j = 0; j < axis[i].length; j++){
                                if (axis[i][j][0] > 0){
                                    if (axis[i][j][2] == 0) //x ~media
                                        ...
                                        value[axis[i][j][0]-1] = new PhysicsValue(PhysicsValFactory.fromFloat((float) getValue(axis[i][j], currentSample, 0)[0]),
                                        getAcquisitionHeader().getChannelsConfig(axis[i][j][0]-1).getSelectedScale().getDefaultErrorValue(),
                                        getAcquisitionHeader().getChannelsConfig(axis[i][j][0]-1).getSelectedScale().getMultiplier()
                                        );
                                }
                                if (axis[i][j][1] > 0){
                                    if (axis[i][j][2] == 0) //y ~media
                                        ...
                                        value[axis[i][j][1]-1] = new PhysicsValue(PhysicsValFactory.fromFloat((float) getValue(axis[i][j], currentSample, 1)[0]),
                                        getAcquisitionHeader().getChannelsConfig(axis[i][j][1]-1).getSelectedScale().getDefaultErrorValue(),
                                        getAcquisitionHeader().getChannelsConfig(axis[i][j][1]-1).getSelectedScale().getMultiplier()
                                        );
                                }
                            }
                ...
                if (datasets != null){
                    ...
                    for (int i = 0; i < graphMed.length; i++)
                        if (!graphMed[i])
                            value[18+i] = new PhysicsValue(PhysicsValFactory.fromByteArray(graphImageBytes(datasets[--o]), "data/raw"), null, com.linkare.rec.data.Multiplier.none);
                }
                //ENVIAR Grafico da posicao [~Media]
                if (datasetGenPos != null)
                    value[22] = new PhysicsValue(PhysicsValFactory.fromByteArray(graphImageBytes(datasetGenPos), "data/raw"), null, com.linkare.rec.data.Multiplier.none);
                
                
                //ENVIAR Grafico da velocidade [~Media]
                if (datasetGenVel != null)
                    value[23] = new PhysicsValue(PhysicsValFactory.fromByteArray(graphImageBytes(datasetGenVel), "data/raw"), null, com.linkare.rec.data.Multiplier.none);
    ...
    /*
     * Devolve um aleatorio no intervalo [0,2Pi[
     * Alterando, ou nao, a seed na inicializacao, permite
     * obter (alterando) numeros aleatorios sem correlacao, ou
     * (nao alterado) os mesmos numeros aleatorios por cada
     ...
     * com as definicoes dos argumentos:
     * _d = modulo do incremento posicional ;
     * _dim = dimensoes ;
     * _seed = mesmo conjunto de aleatorios.
     */
    public void initializeRandomSimul(double _d, boolean _seed){
        ...
        if (_seed) //Modo Aleatorio com correlacao
            rand.setSeed(seed);
        else //Modo aleatorio sem correlacao
     ...
     * com as definicoes dos argumentos:
     * _massa = massa de cada particula ;
     * [] _atrito: _atrito[0]  = raio da particula
     *             _atrito[1] = viscosidade do fluido ;
     * [] _randForce : _randForce[0] = variacao do momento linear
     *                 _randForce[1] = frequencia de colisao ;
     * [] _velcInit  = velocidade inicial X , Y, Z ;
     * _dim = dimensoes ;
     * _seed = mesmo conjunto de aleatorios.
     */
    public void initializeLangevin(double _massa, double [] _atrito, double [] _randForce, double [] _velcInit, boolean _seed){
        //resetState();
        massa = _massa;
        a = _atrito[0];
        visco = _atrito[1];
        ...
        if (execDim == 3){
            eulerTransf = new javax.media.j3d.Transform3D();
            vect3D = new javax.vecmath.Vector3d(dPRandForce/(Math.sqrt(3)*massa), dPRandForce/(Math.sqrt(3)*massa), dPRandForce/(Math.sqrt(3)*massa));
        }
    ...
    /*
     * Aplica a Forca Aleatoria, traduzida num incremento instantaneo de velocidade
     * numa direccao aleatoria
     */
    ...
    /*
     * Actualiza o estado do sistema apos a aplicao duma forca de atrito
     * durante um intervalo de tempo _dt
     */
    ...
    private byte[] graphImageBytes(IntervalXYDataset serCol){
        ...
        JFreeChart tmpChart;
        if (conPts)
            tmpChart = ChartFactory.createXYLineChart("General Chart", label, "", serCol, PlotOrientation.VERTICAL, true, true, true);
        else
            tmpChart = ChartFactory.createScatterPlot("General Chart", label, "", serCol, PlotOrientation.VERTICAL, true, true, true);
     ...
     *
     * @param ind tipo de grafico, 0 : graficos 2 vars, 1 : grafico da posicao, 2 : grafico da velocidade
    ...
    private static void detectRepetition(byte [][][] axis, byte ind, int k, int i){
        for (int j = 0; j <= ind; j++)
            for (int l = 0; l < axis[j].length; l++)
                for (int q = 0; q < 2; q++)
                    if (axis[j][l][2] == axis[ind][k][2] && axis[j][l][q] == axis[ind][k][i] && (j != ind || l != k || q != i)){
    ...
    /*
    //NAO USADO
    public void appendValueToSerie(XYSeries serie, byte [] _axis, int _currentSample){
        double x = 0;
        double y = 0;
        for (int i = 0; i < numPart; i++){
            if (_axis[0] < 8){
                ...
                if (_axis[2] == 1)
                    x += Math.pow(state[i][_axis[0]-8],2);
                else
                    x = Math.pow(state[i][_axis[0]-8],2);
            }
            else {
            ...
            if (_axis[1] < 8){
                ...
                if (_axis[2] == 1)
                    y += Math.pow(state[i][_axis[1]-8],2);
                else
                    y = Math.pow(state[i][_axis[1]-8],2);
            }
            else {
                switch (_axis[1]){
                    case 14 :
                        if (_axis[2] == 1)
                            y += Math.pow(state[i][0],2)+Math.pow(state[i][2],2)+Math.pow(state[i][4],2);
                        else
                            y = Math.pow(state[i][0],2)+Math.pow(state[i][2],2)+Math.pow(state[i][4],2);
                        break;
                    case 16 :
                        if (_axis[2] == 1)
                            y += Math.sqrt(Math.pow(state[i][0],2)+Math.pow(state[i][2],2)+Math.pow(state[i][4],2));
                        else
                            y = Math.sqrt(Math.pow(state[i][0],2)+Math.pow(state[i][2],2)+Math.pow(state[i][4],2));
                        break;
                    case 18 :
                    ...
                    case 15 :
                        if (_axis[2] == 1)
                            y += Math.pow(state[i][1],2)+Math.pow(state[i][3],2)+Math.pow(state[i][5],2);
                        else
                            y = Math.pow(state[i][1],2)+Math.pow(state[i][3],2)+Math.pow(state[i][5],2);
                        break;
                    case 17 :
                        if (_axis[2] == 1)
                            y += Math.sqrt(Math.pow(state[i][1],2)+Math.pow(state[i][3],2)+Math.pow(state[i][5],2));
                        else
                            y = Math.sqrt(Math.pow(state[i][1],2)+Math.pow(state[i][3],2)+Math.pow(state[i][5],2));
                }
            }
        ...
        if (_axis[2] == 1)
            serie.add(x/numPart,y/numPart);
     
    }
     
    //NAO USADO
    private float [][] getValues(byte [] _axis, int _currentSample){
        if (_axis[2] == 1){
            double x = 0;
            double y = 0;
            for (int i = 0; i < numPart; i++){
            ...
            return new float[][]{{(float)(x/numPart),(float)(y/numPart)}};
        }
        else {
            float [][] f = new float[numPart][2];
            double x = 0;
