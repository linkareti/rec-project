...
public class ColisaoDataProducer extends VirtualBaseDataSource {
	...
	public ColisaoDataProducer(VirtualBaseDriver driver, float v1, float v2, float r1, float r2, float m1, float m2,
			int a, int elasticCollision, int tbs, int nSamples) {
		this.driver = driver;
		this.x1 = 0;
		this.y1 = 0;
		this.x2 = 0;
			...
			try {
				...
				while (currentSample <= nSamples && !stopped) {
					...
					value[8] = new PhysicsValue(PhysicsValFactory.fromFloat((float) (esferas[1].vY * esferas[1].m)),
							...
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// pxt
					value[9] = new PhysicsValue(PhysicsValFactory
							.fromFloat((float) (esferas[0].vX * esferas[0].m + esferas[1].vX * esferas[1].m)),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getDefaultErrorValue(),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// pyt
					value[10] = new PhysicsValue(PhysicsValFactory
							.fromFloat((float) (esferas[0].vY * esferas[0].m + esferas[1].vY * esferas[1].m)),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getDefaultErrorValue(),
							getAcquisitionHeader().getChannelsConfig(1).getSelectedScale().getMultiplier());
					// Ec1
					value[11] = new PhysicsValue(PhysicsValFactory
							.fromFloat((float) (0.5 * esferas[0].m * moduloV1 * moduloV1)), getAcquisitionHeader()
					...
					// Ec2
					value[12] = new PhysicsValue(PhysicsValFactory
							.fromFloat((float) (0.5 * esferas[1].m * moduloV2 * moduloV2)), getAcquisitionHeader()
							.getChannelsConfig(1).getSelectedScale().getDefaultErrorValue(), getAcquisitionHeader()
							.getChannelsConfig(1).getSelectedScale().getMultiplier());
					// Ect
