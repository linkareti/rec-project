...
public class DrawableMolas extends InteractivePanel {
    ...
    public DrawableMolas(M3Customizer gui_) {
        ...
        mola1T = new InteractiveText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.6","Spring 1"));
        mola1T.getStyle().setEdgeColor(new Color(255,228,0));
        mola1T.setXY(5,11);
        mola3T = new InteractiveText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.8","Spring 3"));
        mola3T.getStyle().setEdgeColor(new Color(255,228,0));
        mola3T.setXY(10,-1);
        mola2T = new InteractiveText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.7","Spring 2"));
    ...
    public void fazMexer(double mousex,double mousey){
        try{
            ...
            if(gui.textX.getText().equals("5.0") && gui.textY.getText().equals("5.0")){
                ...
                gui.labelInf1.setText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.9","Please disturb the system"));
                gui.labelInf2.setText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.10","by dragging the mass."));
                gui.buttonOK.setEnabled(false);
            } else{
                gui.labelInf1.setText("");
                gui.labelInf2.setForeground(new java.awt.Color(0,125,0));
                gui.labelInf2.setText(ReCResourceBundle.findStringOrDefault("ReCExpM3$rec.exp.customizer.title.11","Ready."));
