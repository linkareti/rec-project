...
public class Pend2M extends DrawingPanel3D implements ActionListener, MouseListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5003380159117771939L;
	...
	public Pend2M(final InteractiveMenu par) {
	...
	public void setFase(final double fase) {
		this.fase = fase;
	...
	protected void keepDistances() {
		...
		bola[1].setXYZ(0, bola[0].getY() + n1 * (fio[1].getYMax() - fio[1].getYMin()),
				bola[0].getZ() + n2 * (fio[1].getZMax() - fio[1].getZMin()));
	}
	...
	public void setAmp(final double size) {
		bar.setY(-size);
		bar.setSizeY(2 * size);
	...
	public void setL1(final double len) {
	...
	public void setL2(final double len) {
		final double ang = getPhi();
	...
	public double getL1() {
		return Math.sqrt(Math.pow(fio[0].getYMax() - fio[0].getYMin(), 2)
				+ Math.pow(fio[0].getZMax() - fio[0].getZMin(), 2));
	}

	public double getL2() {
		return Math.sqrt(Math.pow(fio[1].getYMax() - fio[1].getYMin(), 2)
				+ Math.pow(fio[1].getZMax() - fio[1].getZMin(), 2));
	}
	...
	public void setPhiVecVel(final double vel) {
	...
	public void move(final double theta, final double phi, final double thetaDot, final double phiDot,
	...
	public void move(final double theta, final double phi, final double point) {
		bola[0].setXYZ(0, Math.sin(theta) * getL1(), -Math.cos(theta) * getL1());
		fio[0].clear();
		fio[0].addPoint(0, 0, 0);
		fio[0].addPoint(0, bola[0].getY(), bola[0].getZ());
	...
	public void setPhi(final double ang) {
	...
	public double getTheta() {
		final int n = (int) (bola[0].getY() / Math.abs(bola[0].getY()));
		return n * Math.acos(-bola[0].getZ() / getL1());
	}

	public double getPhi() {
		final int n = (int) ((bola[1].getY() - bola[0].getY()) / Math.abs(bola[1].getY() - bola[0].getY()));
	...
	public void config(final double theta, final double phi, final double thetaDot, final double phiDot,
			final double l1, final double l2, final double amp, final double fase) {
		bola[0].setXYZ(0, Math.sin(theta) * getL1(), -Math.cos(theta) * getL1());
		fio[0].clear();
		fio[0].addPoint(0, 0, 0);
		fio[0].addPoint(0, bola[0].getY(), bola[0].getZ());
