...
public class Chart2 extends com.linkare.rec.impl.baseUI.graph.DefaultXYExperimentGraph {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4240693120040962539L;
	...
	public void dataModelStartedNoData() {
		...
		final String graph2 = header.getSelectedHardwareParameterValue("graph2");

		if (!graph2.equalsIgnoreCase("")) { // grafico activo
			if (header.getSelectedHardwareParameterValue("graph2Med").trim().equals("1") ? true : false) {
				final byte[] channels = Chart1.nameToChannels(graph2);
		...
		if (chart != null) {
			for (int i = evt.getSamplesStartIndex(); i <= evt.getSamplesEndIndex(); i++) {
				if (model.getValueAt(i, 19) != null) {
					chart.makeImage(model.getValueAt(i, 19).getValue().getByteArrayValue().getData());
				}
			}
		} else {
