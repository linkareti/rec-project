...
public class LMFit {
	 ...
	 * Origin5.0)
	 */
	public static double[] solve(double[] x, double[] y, double mu, double sigma, double y0, double Amp, double lambda,
			double termEpsilon, int maxIter) {
		int numPts = y.length;
		int numParams = 4;
		int iter = 0;
		int term = 0;
		...
		while (!done) {
			...
			for (int parIndex1 = 0; parIndex1 < numParams; parIndex1++)
				for (int parIndex2 = 0; parIndex2 < numParams; parIndex2++)
					for (int ptIndex = 0; ptIndex < numPts; ptIndex++) {
						...
						hess[parIndex1][parIndex2] += gaussGrad(x[ptIndex], mu, sigma, y0, Amp, parIndex1)
								* gaussGrad(x[ptIndex], mu, sigma, y0, Amp, parIndex2);
					}// ptIndex
			...
			for (int parIndex = 0; parIndex < numParams; parIndex++)
				for (int ptIndex = 0; ptIndex < numPts; ptIndex++) {
					if (ptIndex == 0)
						grad[parIndex] = 0.;
					grad[parIndex] += (y[ptIndex] - gaussFunc(x[ptIndex], mu, sigma, y0, Amp))
							* gaussGrad(x[ptIndex], mu, sigma, y0, Amp, parIndex);
				}// ptIndex
	...
	private static double[] solveLin4(double[][] hess, double[] grad) {
		...
		double denominator = d * g * j * m - c * h * j * m - d * f * k * m + b * h * k * m + c * f * l * m - b * g * l
				* m - d * g * i * n + c * h * i * n + d * e * k * n - a * h * k * n - c * e * l * n + a * g * l * n + d
				* f * i * o - b * h * i * o - d * e * j * o + a * h * j * o + b * e * l * o - a * f * l * o - c * f * i
				* p + b * g * i * p + c * e * j * p - a * g * j * p - b * e * k * p + a * f * k * p;
	...
	private static double[] solveLin(double[][] hess, double[] g, int numParams) {
		if (numParams == 4)
			return solveLin4(hess, g);
		else if (numParams == 2)
			return solveLin2(hess, g);
		else
			return null;
	}
}// LMFit
