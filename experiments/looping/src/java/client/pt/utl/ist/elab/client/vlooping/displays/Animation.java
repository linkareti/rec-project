...
public class Animation extends JPanel implements ExpDataDisplay, ExpDataModelListener {
    ...
    private PlottingPanel panel = new PlottingPanel("X (m)","Y (m)", ReCResourceBundle.findStringOrDefault("ReCExpLooping$rec.exp.customizer.title.graph" ,"Trajectory") );
    ...
    public void paintTracks(double h1, double h2, double r1){
        ...
        if(Math.max(h1,h2)> Math.max(h1,r1)){
            ...
            if(Math.max(h1,r1)==r1){panel.setPreferredMinMaxY(-1, Math.max(h1,r1)*2+2);}
            else{
    ...
    public void newSamples(NewExpDataEvent evt) {
        ...
        for(int i=evt.getSamplesStartIndex(); i<=evt.getSamplesEndIndex(); i++) {
            ...
            if(model.getValueAt(i,1) != null) {
                ...
                moveBall(model.getValueAt(i,1).getValue().getFloatValue(),
