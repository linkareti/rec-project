...
public class RobotStateMachine extends BaseDataSource implements pt.utl.ist.elab.driver.webrobot.interfaces.AuxVars,
		pt.utl.ist.elab.driver.webrobot.interfaces.Sensors, pt.utl.ist.elab.driver.webrobot.interfaces.InputOutput

{
	...
	/**
	 * The sensitivity of the IR sensors: If I > sensitivity it implies it is
	 * over a reflecting surface. If the user wants to do something when it is a
	 * reflecting surface, he must say I=0, otherwise say I=1...for more
	 * informations consult the help from prograf or JPrograf
	 */
	...
	/**
	 * The file the user will send...actually he sends a String...a file is
	 * constructed...next version upgrade this pasticio
	 */
	...
	/**
	 * Use to yield the state machine, only used when the robot is "walking"
	 * with a component of type 3,4,5 or 6
	 */
	...
	/**
	 * The two threads! One, generalThread, keeps the program alive and change
	 * the states in the state machine. The other is for the blocks of type
	 * 3,4,5 and 6.
	 */
	...
	/** REC */
	/**
	 * This running is different from the runningGeneralThread!! When
	 * running=false, it stops sending samples...the exp has not endend because
	 * I need to park the robot first...but it will end soon...Conclusion:don't
	 * send more packets...
	 */
	...
	/**
	 * sensorsValues are the ones received from the robot, the problem is that
	 * the digital values are encoded in one number, channelsValues are the
	 * values actually sent to the client!
	 */
	...
	public RobotStateMachine(final RobotDriver hardware) {
		...
		LOGGER.log(Level.INFO, "State Machine Started!");
	}
	...
	private void readProps() {
		...
		try {
			...
			LOGGER.log(Level.INFO, "Couldn't found the file...\n" + fnfe);
		...
		try {
			...
			// EXP_DURATION = Integer.parseInt(props.getProperty("expDuration"),
			// 10);
			// I0STATE_PARKED = Integer.parseInt(props.getProperty("i0parked"),
			// 10);
			// I1STATE_PARKED = Integer.parseInt(props.getProperty("i1parked"),
			// 10);
			// I2STATE_PARKED = Integer.parseInt(props.getProperty("i2parked"),
			// 10);
			// I3STATE_PARKED = Integer.parseInt(props.getProperty("i3parked"),
			// 10);
			// I4STATE_PARKED = Integer.parseInt(props.getProperty("i4parked"),
			// 10);
			// I5STATE_PARKED = Integer.parseInt(props.getProperty("i5parked"),
			// 10);
			// I6STATE_PARKED = Integer.parseInt(props.getProperty("i6parked"),
			// 10);
			// I7STATE_PARKED = Integer.parseInt(props.getProperty("i7parked"),
			// 10);
			batDownVal = Integer.parseInt(props.getProperty("batDownVal"), 10);
			// batFullVal = Integer.parseInt(props.getProperty("batFullVal"),
			// 10);
			...
			LOGGER.log(Level.INFO, "Erro ao ler as propriedades\n" + e);
		}
	}
	...
	private synchronized void askSerial() {
		...
		/** CHANGED */
		/*
		 * try { Thread.currentThread().sleep(15); } catch(InterruptedException
		 * ie) { }
		 */
	}
	...
	private class dataProducerThread extends Thread {
		public void run() {
			while (runningDataThread) {
					...
					synchronized (channelsValues) {
						...
						for (int i = 0; i < 8; i++) {
							values[i] = new PhysicsValue(PhysicsValFactory.fromInt(channelsValues[i]),
									getAcquisitionHeader().getChannelsConfig(i).getSelectedScale()
											.getDefaultErrorValue(), getAcquisitionHeader().getChannelsConfig(i)
						...
						values[21] = new PhysicsValue(PhysicsValFactory.fromInt(rowToSend), getAcquisitionHeader()
								.getChannelsConfig(21).getSelectedScale().getDefaultErrorValue(),
								getAcquisitionHeader().getChannelsConfig(21).getSelectedScale().getMultiplier());

						/** The current column in the main matrix */
						values[22] = new PhysicsValue(PhysicsValFactory.fromInt(colToSend), getAcquisitionHeader()
								.getChannelsConfig(22).getSelectedScale().getDefaultErrorValue(),
								getAcquisitionHeader().getChannelsConfig(20).getSelectedScale().getMultiplier());

						/**
						 * If the state machine has reseted I'll block the
						 * actualization of the row to send and column to send,
						 * since now I've sent the values, new values are very
						 * welcome
						 */
						restarted = false;

						LOGGER.log(Level.INFO, "Sending sample number:" + nSamples);
				...
				/** Park the robot */
				LOGGER.log(Level.INFO, "Parking the robot");
				parkRobot();
				...
				while (stoping) {
					...
					testPark();
					/**
					 * You say it's parked...but please be sure...ask again If
					 * you've asked 2 times and the answer is always the same,
					 * then I guess it's parked...
					 */
					if (parked && counterParked > 2) {
						LOGGER.log(Level.INFO, "The robot is Parked!!!");
						stoping = false;
						counterParked = 0;
						sendPWM(127, 127, false);
						killGeneralThread();
				...
				/**
				 * If the robot has the battery low...I'm sorry but I can't
				 * change the state, it will remain stoping till the battery has
				 * a value good enough
				 */
				int batCounter = 0;
				LOGGER.log(Level.INFO, "The min value for the bat is:" + batDownVal);

				while (batCounter < 4) {
					LOGGER.log(Level.INFO, "Current battery value is:" + a1State);
					if (!isBatteryDown()) {
						batCounter++;
					} else {
						batCounter = 0;
			...
			try {
				...
				LOGGER.log(Level.INFO, "Error in the thread\n" + ie);
			}
			killGeneralThread();
			serialTimer.cancel();
		}
	...
	/**
	 * Very important thread, it's the key of the experiment...controls the
	 * incrementation of blocks in the stateMachine, and controls the pre-valued
	 * actuation blocks (frente, direita,marchAtras e Esquerda
	 */
		...
		public synchronized void run() {
			while (runningGeneralThread) {
				...
				if (!stoping && isOverBorder(init)) {
					sendPWM(127, 127, false);
					/**
					 * Let's wait to send to the user the reason why it
					 * overborded..
					 */
					try {
						Thread.sleep(acqTime);
				...
				while (runningGeneralThread && !finished && !walking) {
					...
					/**
					 * CHANGED try { generalThread.sleep(5);//original 30 }
					 * catch(InterruptedException ie) {
					 * Logger.getLogger(WR_DS_LOGGER
				...
				if (runningGeneralThread && walking) {
					while (runningGeneralThread && numberWalks > 0) {
						...
						/**
						 * If the robot has walked for 1/2 second, then ask for
						 * serial data, I'll use it to control the border... and
						 * to send to the user...
						 */
						// askSerial();
						sensorsValues = getSensorsValues();
				...
				/*** CHANGED */
				/*
				 * try { generalThread.sleep(10);//original 25 } catch
	...
	public void startStateMachine(final String flowString) {
		...
		try {
			...
			LOGGER.log(Level.INFO, "Error when writing the flow file\n" + ioe);
		}
	...
	public void startStateMachine(final java.io.File file) {
		...
		/**
		 * Restart the state machine (set the state machine in the beggining
		 * positions
		 */
		restartStateMachine();
	}
	...
	private void stateMachine() {
		 ...
		 * there isn't a connection) 2-If there's a connection increment the
		 * column
		 */
		if (stateBlock == null) {
		...
		if (stateBlock.getTipo() == 1 || stateBlock.getTipo() == 2 || stateBlock.getTipo() == 12
			...
			/**
			 * If the flag is equal to 1, then the comparation is to be done
			 * with a number, otherwise it will be done with a variable
			 */
			if (flag == 1) {
				d3 = valor;
			}
			...
			currentRow++;
			return;
		} else if (stateBlock.getTipo() == 3 || stateBlock.getTipo() == 4 || stateBlock.getTipo() == 5
				|| stateBlock.getTipo() == 6) {
	...
	private void sendPWM(int valor, int valor2, final boolean waitSerial) {
		...
		/** CHANGED */
		/*
		 * try { Thread.currentThread().sleep(50);//original 50 }
		 * catch(InterruptedException ie) {
	...
	private void restartStateMachine() {
		...
		/** CHANGED */
		/*
		 * try { Thread.currentThread().sleep(20); } catch(InterruptedException
		 * ie) { }
		 */
		finished = true;
		currentRow = 2;
	...
	private boolean isOverBorder(final boolean init) {
		...
		if ((i2State > I2BORDER || i3State > I3BORDER || i4State > I4BORDER || i5State > I5BORDER)) {
			...
			if (!init) {
				sendPWM(127, 127, false);
				LOGGER.log(Level.INFO, "The border was crossed!!!!");
			}
	...
	public void setAtStartPosition() {
		...
		while (counter < 3) {
			...
			counterTime++;
			if (counterTime > 40) {
				LOGGER.log(Level.INFO, "Exit because of timeout");
				break;
			}
			try {
				Thread.sleep(1500);
	...
	public void startProduction() {
		...
		LOGGER.log(Level.INFO, "acqTime=" + acqTime);
		// counter = 0;
		runningGeneralThread = true;
		generalThread = new generalThread();
		generalThread.setPriority(Thread.MAX_PRIORITY);
