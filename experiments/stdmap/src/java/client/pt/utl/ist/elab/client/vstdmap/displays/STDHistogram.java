...
public class STDHistogram extends PlottingPanel implements ExpDataDisplay, ExpDataModelListener {
    ...
    private String statusStr = java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.statusStr.recData");
    ...
    public STDHistogram() {
        ...
        //Anima+Hist+Map
        //new pt.utl.ist.elab.virtual.driver.stdmap.STDMAPDataProducer(null, 0, 1, 5, 1, -5, 1000,10000).startHist(this);
        
        //MAP+Hist static
        /*float [] c = STDMAPImage.byteArrayToFloatArray(new pt.utl.ist.elab.virtual.driver.stdmap.STDMAPDataProducer(null, 1, 0, 1, 1000000, .5f, 800, 800, (byte)1, false).getIMapaData());
        ...
        //MAP+Hist non-static
        /*float [] mData = STDMAPImage.byteArrayToFloatArray(new pt.utl.ist.elab.virtual.driver.stdmap.STDMAPDataProducer(null, 1, 0, 1, 1000000, .5f, 800, 800, (byte)1, false).getMapaData());
    ...
    public void dataModelEnded() {
        if (statusStr.equalsIgnoreCase(java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.statusStr.recData"))){
            statusStr = "";
            repaint();
        }
    }
    ...
    public void dataModelStartedNoData() {
        ...
        //    setVisible(false);
        if (simulType != 1){
            setTitle(java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.display.stdmap.title.2"));
            setPreferredMinMaxX(0, 2*Math.PI);
            hist = new Histogram();
            hist.setBinWidth(2*Math.PI/1000);
            if (simulType == 2){
                setXLabel("dTheta/dt");
                hist.setXYColumnNames("dTheta/dt", java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.histogram.title.1"));
            }
            else {
                setXLabel("I");
                hist.setXYColumnNames("I", java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.histogram.title.1"));
            }
            setYLabel(java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.histogram.title.1"));
            addDrawable(hist);
        }
        else
            statusStr = java.util.ResourceBundle.getBundle("pt/utl/ist/elab/virtual/client/stdmap/resources/ReCExpSTDMAP").getString("rec.exp.displays.inactive");
        repaint();
    }
        ...
        if (hist != null)
            for(int i=evt.getSamplesStartIndex(); i<=evt.getSamplesEndIndex(); i++) {
                ...
                if (model.getValueAt(i, model.getChannelIndex("iMapaBytes")) != null){ //static
                    float [] tmp = ByteUtil.byteArrayToFloatArray(model.getValueAt(i, model.getChannelIndex("iMapaBytes")).getValue().getByteArrayValue().getData());
