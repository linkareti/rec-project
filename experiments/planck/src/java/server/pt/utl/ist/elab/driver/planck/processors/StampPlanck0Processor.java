...
public class StampPlanck0Processor extends AbstractStampProcessor {
	...
	public StampPlanck0Processor() {
		super(StampPlanck0Processor.COMMAND_IDENTIFIER);
	}
	...
	public boolean process(final StampCommand command) {
		...
		if (command.getCommandIdentifier().equalsIgnoreCase(StampPlanck0Processor.COMMAND_IDENTIFIER)
				&& splitedStr[0] != null) {
			try {
				...
				command.addCommandData(StampPlanck0Processor.TETA, oTeta);
				...
				command.addCommandData(StampPlanck0Processor.FOTOCEL, oPotential);
