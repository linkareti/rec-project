...
public class QuantumDriver extends VirtualBaseDriver {
	...
	public void configure(HardwareAcquisitionConfig config, HardwareInfo info) throws WrongConfigurationException {
		...
		boolean wraparoundKS = config.getSelectedHardwareParameterValue("wraparoundKS").trim().equals("1") ? true
				: false;
		boolean wraparoundXS = config.getSelectedHardwareParameterValue("wraparoundXS").trim().equals("1") ? true
				: false;
		...
		try {
			...
			try {
				...
				LoggerUtil.logThrowable("Unable to load resource: " + baseHardwareInfoFile, e2, Logger
						.getLogger(Quantum_DRIVER_LOGGER));
			}
		}
