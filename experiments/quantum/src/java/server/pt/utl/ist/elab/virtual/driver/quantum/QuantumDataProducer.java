...
public class QuantumDataProducer extends VirtualBaseDataSource implements Runnable {
     ...
     *
     *  (min, max) -> Angstroms
     *  potential[0] -> inicio da primeira barreira de potencial (Angstroms)
     *  potential[1] -> fim da primeira barreira de potencial (Angstroms)
     *  potential[2] -> espaco entre potenciais (Angstroms)
     *  potential[3] -> valor do potencial (eV)
     *  _nPotentials -> numero de potenciais
     *  _energy -> energia da particula (eV)
     */
    public QuantumDataProducer(VirtualBaseDriver driver, double _dX0, double _x0, double _energy, int log2N, double _deltaX, double tol, double _dt, double _tbs, int _nSamples, boolean _wraparoundKS, boolean _wraparoundXS, boolean _tunneling) {
        this.driver = driver;
        nSamples = _nSamples;
        ...
        if (isMedio){
            ...
            for (int i = 0; i < N; i++)
                if (i*dx >= x-(x0-(N-1)*dx/2) && i*dx <= x+width-(x0-(N-1)*dx/2)){
                    ExpV[i] = Complex.expI(new Complex(0, -(vFunction.evaluate(i*dx-x+(x0-(N-1)*dx/2)))*dt/(hbar*2)));
                }
    ...
    /*
     * Devolve o y para em funcao do x, da gaussiana inicial que representa a particula
     */
     ...
     *
     * Este avanco contempla a funcao de onda, as caracteristicas de tunneling
     * e os coeficientes
     *
     * Inicia o calculo dos coeficientes entre o fim dum tempo de tunel e o inicio
     * da contagem de outro
     *
     */
    public double[] step(){
        ...
        while (time - timeInicial < dtFrame){
            ...
            if (wraparoundKSCheckup && wraparoundCheckUpKSpace()){
                wraparoundKS = 1;
                //se chegar aos limites e nao tiver coeficiente e ja tiver iniciado, fica com o actual
            ...
            if (tunnelingSettings){
                ...
                //verifica se inicio da contagem do tunneling time e se sim e no caso de ainda nao ter obtido coefs, finaliza com os valores actuais
     ...
     *
     * Avalia o integral dum lado e doutro do potencial actual, esperando
     * obter um maximo local, que equivalera ao coeficiente
     *
     * No caso em que o comprimento do potencial ultrapasse a grelha, e considerado como transmissao
     * o calculo entre a primeira face do potencial e o fim da grelha.
     */
    private void coefs(){
    ...
    /*
     * Faz o calculo do tempo que a funcao de onda demora a atravessar
     * o potencial. No caso de nao atravessar nao faz qualquer definicao
     *
     * Inicio da contagem quando se obtem um maximo local na primeira face
     * po potencial actual e fim da contagem quando o mesmo acontece para o
     * a segunda face do mesmo potencial. Este maximo local tem de ser maior
     * que um valor de tolerancia.
     *
     * [t,v,w] -> ]s,m/s,A]
     */
    private double[] tunnelingTimeSpeed(){
        if (potentialLookUp + 2 <= potentials.length){
            ...
            if (principioTunneling)
                ...
                if (ini >= psi.length) //final para la da grelha = final no ultimo ponto da grelha
                    ini = psi.length-1;
            }
            ...
            if (ini < psi.length){
                ...
                if (psiLookedUp > modPsiSq[ini] && psiLookedUp > tolerancia){
                    ...
                    if (!principioTunneling)
                        ...
                        tunneling[potentialLookUp][1] = potentials[potentialLookUp][1]*1e-10/(time-potentialInitTime);//(ini-xIPotential-(potentialLookUp-1)*(xFPotential - xIPotential + potentialGap))*dx*1e-10/(time-potentialInitTime);
                        tunneling[potentialLookUp][2] = potentials[potentialLookUp][1];//(ini-xIPotential-(potentialLookUp-1)*(xFPotential - xIPotential + potentialGap))*dx*1e-10;
                        return new double[]{tunneling[potentialLookUp][0], tunneling[potentialLookUp][1], tunneling[potentialLookUp][2]};
                    }
                }
            }
        }
    ...
    /*
     * Calcula a FFT da funcao de onda e re-ordena os pontos
     * segundo o limite de nyquist
     */
    ...
    /*
     * Verifica se pelo menos 0.5% da funcao de onda esta nos ultimos
     * 5% dos extremos do espaco coordenado
     */
    ...
    /*
     * Verifica se pelo menos 0.5% da FFT da funcao de onda esta nos ultimos
     * 5% dos extremos do espaco dos k's
     */
                ...
                while(currentSample < nSamples && !stopped){
                    ...
                    while (time - timeInicial < dtFrame){
                        ...
                        if (wraparoundXS == 0 && wraparoundXSCheckup && wraparoundCheckUpCoordinateSpace()){
                            wraparoundXS = 1;
                            value[8] = new PhysicsValue(PhysicsValFactory.fromBoolean(true),
                        ...
                        if (tunnelingSettings){
                            ...
                            if (tunnelingTimeSpeed != null && potentialLookUp < potentials.length){
                                ...
                                value[6] = new PhysicsValue(PhysicsValFactory.fromFloat((float)(tunnelingTimeSpeed[0]*1e21)),
                            ...
                            if ((modalRC && modalTC || currentSample == nSamples-2) && potentialLookUp < potentials.length){
                                modalRC = false;
                                modalTC = false;
                                ...
                                value[2] = new PhysicsValue(PhysicsValFactory.fromFloat((float)coefR[potentialLookUp]),
                                ...
                                value[3] = new PhysicsValue(PhysicsValFactory.fromFloat((float)coefT[potentialLookUp]),
                    ...
                    value = new PhysicsValue[NUM_CHANNELS];
                    value[0] = new PhysicsValue(PhysicsValFactory.fromByteArray(ByteUtil.getObjectAsByteArray(psi), "data/raw"), null, com.linkare.rec.data.Multiplier.none);       
                    addDataRow(value);
                    currentSample++;
                    sleep(tbs);
                }
