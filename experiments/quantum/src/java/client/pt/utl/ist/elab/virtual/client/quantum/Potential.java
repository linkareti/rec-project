...
public class Potential implements Bounded {
    ...
    public Potential(double xI, double vWidth, String function, boolean _enabled, boolean _isMedio, Color _fillColor, Color _highlightColor){
        xIPotential = xI;
        potentialWidth = vWidth;
        try{
            vFunction = new ParsedFunction(function);
    ...
    private void buildPath(){
        ...
        positivePath.transform(new AffineTransform(1, 0, 0, Math.abs(5e-1/Math.max(Math.abs(yminHere), Math.abs(ymaxHere))), 0, 0));
        negativePath.transform(new AffineTransform(1, 0, 0, Math.abs(5e-1/Math.max(Math.abs(yminHere), Math.abs(ymaxHere))), 0, 0));
    ...
    public Interactive findInteractive(DrawingPanel panel, int _xpix, int _ypix) {
        if (enabled){
            ...
            ntmpPath.closePath();
            if (positiveShape != null && negativeShape != null && (tmpPath.createTransformedShape(panel.getPixelTransform()).contains(_xpix,_ypix) || ntmpPath.createTransformedShape(panel.getPixelTransform()).contains(_xpix,_ypix)))
                return this;
        }
        return null;
    }
    ...
    public boolean intersect(Rectangle2D rect){
        ...
        if (positiveShape != null && negativeShape != null && (tmpPath.createTransformedShape(null).getBounds2D().intersects(rect) || ntmpPath.createTransformedShape(null).getBounds2D().intersects(rect)))
            return true;
        return false;
    }
    ...
    public Rectangle2D getBounds(double x){
        ...
        return tmpPath.createTransformedShape(null).getBounds2D().createUnion(ntmpPath.createTransformedShape(null).getBounds2D());
    }
