...
public class PoissonCalculator {
	...
	private double[][][] doCalculation() {
		...
		try {
			...
			do {
				...
				if (i == 0)
					firstDiff = diff[0];
				// if ( myRank == 0) dialog.updateResultField( "(iteration " + i
				// + "): delta = " + diff[0] + " (aim = " + Error + ")" );
				i++;
			...
			if (myRank == 0) {
				...
				// dialog.updateResultField( "" );
				// dialog.updateResultField( "After " + i +
				// " iterations: delta = " + diff[0] + "\n\n" );
				// endTime = (new Date()).getTime();
				// dialog.updateResultField( "" );
				// dialog.updateResultField( "Time elapsed: " +
		...
		if (myRank == 0) {
			for (int x = startx; x <= endx; x++) {
				for (int j = 0; j < Ny + 2; j++)
					System.arraycopy(u[x][j], 0, solution[x][j], 0, Nz + 2);
			}
		}
		...
		try {
			for (int proc = 1; proc < numprocs; proc++) {
				...
				for (int x = sx; x <= ex; x++) {
					if (myRank == proc) {
						for (int j = 0; j < Ny + 2; j++)
							System.arraycopy(u[x - sx + 1][j], 0, msgSend, j * (Nz + 2), Nz + 2);
					...
					if (myRank == 0) {
						...
						for (int j = 0; j < Ny + 2; j++)
							System.arraycopy(msgRecv, j * (Nz + 2), solution[x][j], 0, Nz + 2);
					}
				}
			}
		}
	...
	// evaluates these functions in the portion of the grid belonging to each
	// process
	...
	private void gridInit() {
		...
		// These objects represent the mathematical funtions that define the
		// boundary conditions
		...
		try {
			...
			// initialization of boundary at z and -z surfaces (face1Function
			// and face3Function)
			for (i = 0; i < (endx - startx + 3); i++)
				for (j = 0; j < Ny + 2; j++) {
					d2[0] = hx * (startx + i - 1);
			...
			// initialization of boundary at y and -y surfaces (face2Function
			// face4Function)
			for (i = 0; i < (endx - startx + 3); i++)
				for (k = 0; k < Nz + 2; k++) {
					d2[0] = hx * (startx + i - 1);
	...
	// exchange ghost bottom (startx-1) rows with bottom neighbour and ghost top
	// (endx+1) rows with top neighbour.
	private void exchange(double[][][] a) throws MPIException {
		try {
			if (topNeighbour != MPI.PROC_NULL) {
				for (int j = 0; j < Ny + 2; j++)
					System.arraycopy(a[endx - startx + 1][j], 0, msgSend, j * (Nz + 2), Nz + 2);
			...
			if (bottomNeighbour != MPI.PROC_NULL) {
				...
				for (int j = 0; j < Ny + 2; j++)
					System.arraycopy(msgRecv, j * (Nz + 2), a[0][j], 0, Nz + 2);
			}

			if (bottomNeighbour != MPI.PROC_NULL) {
				for (int j = 0; j < Ny + 2; j++)
					System.arraycopy(a[1][j], 0, msgSend, j * (Nz + 2), Nz + 2);
			...
			if (topNeighbour != MPI.PROC_NULL) {
				...
				for (int j = 0; j < Ny + 2; j++)
					System.arraycopy(msgRecv, j * (Nz + 2), a[endx - startx + 2][j], 0, Nz + 2);
			}
		...
		for (int i = 1; i < endx - startx + 2; i++)
			for (int j = 1; j < Ny + 1; j++)
				for (int k = 1; k < Nz + 1; k++)
					out[i][j][k] = r
							* ((in[i + 1][j][k] + in[i - 1][j][k]) / hx2 + (in[i][j + 1][k] + in[i][j - 1][k]) / hy2
									+ (in[i][j][k + 1] + in[i][j][k - 1]) / hz2 + rho[i][j][k] / Epsilon0);
	}
