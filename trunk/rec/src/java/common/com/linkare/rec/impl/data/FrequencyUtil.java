...
public class FrequencyUtil {
    ...
    public static double toMillis(final Frequency freq) {
        double retVal = 0;
        if (freq.getFrequencyDefType().getValue() == FrequencyDefType.SamplingIntervalType.getValue()) {
            retVal = freq.getFrequency();
        } else {
            retVal = ((double) 1.) / freq.getFrequency();
        }
        
        retVal=retVal*freq.getMultiplier().getExpValue()/(double)1000.;
