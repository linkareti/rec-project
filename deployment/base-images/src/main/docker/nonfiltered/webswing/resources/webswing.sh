...
case "$1" in
    run)
        ...
        if [ ! -f $PID_PATH_NAME ] || [ `ps -axo pid | grep "$(cat $PID_PATH_NAME)" | wc -l` -eq 0 ]; then
            ...
            $JAVA_HOME/bin/java $JAVA_OPTS -jar $HOME/webswing-server.war $OPTS &
            echo $! > $PID_PATH_NAME
            echo "Webswing started with pid : $(cat $PID_PATH_NAME)"
    ...
    start)
        ...
        if [ ! -f $PID_PATH_NAME ] || [ `ps -axo pid | grep "$(cat $PID_PATH_NAME)" | wc -l` -eq 0 ]; then
            ...
            if [ -a /tmp/.X99-lock ]; then
                ...
                rm -f /tmp/.X99-lock
                if [ $? -ne 0 ]; then
                    echo "Unable to remove /tmp/.X99-lock"
                fi
            fi
            xvfb-run $SCRIPTPATH$0 run
            echo "Webswing FINISHED"
        else
            echo "Webswing is already running with pid $(cat $PID_PATH_NAME)"
        fi
        ;;
