...
public class SAnimation extends JPanel implements com.linkare.rec.impl.client.experiment.ExpDataDisplay,
	...
	public SAnimation() {
		...
		title = BorderFactory.createTitledBorder("Animacao"); // Cria��o da
		// border e do
		// t�tulo
		...
		title = BorderFactory.createTitledBorder("Variaveis"); // Cria��o da
		// border e do
		// t�tulo
		title.setTitleJustification(TitledBorder.CENTER); // Posicionamento do
		// t�tulo
		...
		title = BorderFactory.createTitledBorder("Parametros"); // Cria��o da
		// border e do
		// t�tulo
		title.setTitleJustification(TitledBorder.CENTER); // Posicionamento do
		// t�tulo
	...
	private void setW(double w) {
		this.w = w;
		dWidth = (int) (w * ((pWidth + nWidth - 10) / (0.01)));
	}
	...
	private void setVBI(double vBI) {
		this.vBI = vBI;
	}

	private void setEG(double eG) {
		this.eG = eG;
	}

	private void setNA(double nA) {
		this.nA = nA;
	}

	private void setND(double nD) {
		this.nD = nD;
	}
	...
	private void headerAvailable(HardwareAcquisitionConfig header) {
		...
		if (header != null) {
			// Valores iniciais
			/*
			 * setLength1(Integer.parseInt(header.getSelectedHardwareParameterValue
			 * ("P1 length")));
			 * setLength2(Integer.parseInt(header.getSelectedHardwareParameterValue
			 * ("P2 length")));
			 * setMass1(Float.parseFloat(header.getSelectedHardwareParameterValue
			 * ("P1 Mass"))/1000f);
			 * setMass2(Float.parseFloat(header.getSelectedHardwareParameterValue
			 * ("P2 Mass"))/1000f);
			 */
	...
	public void newSamples(NewExpDataEvent evt) {
		...
		for (int i = evt.getSamplesStartIndex(); i <= evt.getSamplesEndIndex(); i++) {
			if (model.getValueAt(i, 0) != null)
				setW(model.getValueAt(i, 0).getValueNumber().doubleValue());
			if (model.getValueAt(i, 1) != null)
				setEG(model.getValueAt(i, 1).getValueNumber().doubleValue());
			if (model.getValueAt(i, 2) != null)
				setVBI(model.getValueAt(i, 2).getValueNumber().doubleValue());
			if (model.getValueAt(i, 3) != null)
				setE(model.getValueAt(i, 3).getValueNumber().doubleValue());
			if (model.getValueAt(i, 4) != null)
				setT(model.getValueAt(i, 4).getValueNumber().doubleValue());
			if (model.getValueAt(i, 6) != null)
				setND(model.getValueAt(i, 6).getValueNumber().doubleValue());
			if (model.getValueAt(i, 7) != null)
				setNA(model.getValueAt(i, 7).getValueNumber().doubleValue());
			if (model.getValueAt(i, 8) != null)
				setV(model.getValueAt(i, 8).getValueNumber().doubleValue());
	...
	class PanelAnimacao extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			corP = new Color(corP.getRed() / 255f, corP.getGreen() / 255f, corP.getBlue() / 255f, (float) (0.5 + (0.5)
					/ (Math.pow(10, 19) - Math.pow(10, 13)) * (nA - Math.pow(10, 13))));
			corN = new Color(corN.getRed() / 255f, corN.getGreen() / 255f, corN.getBlue() / 255f, (float) (0.5 + (0.5)
					/ (Math.pow(10, 19) - Math.pow(10, 13)) * (nD - Math.pow(10, 13))));
			corV = new Color(corV.getRed() / 255f, corV.getGreen() / 255f, corV.getBlue() / 255f,
					(float) (0.5 + (0.5) / (20) * (v + 10)));
